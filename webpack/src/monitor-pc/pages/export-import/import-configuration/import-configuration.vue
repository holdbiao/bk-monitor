<template>
  <article class="import-config" v-bkloading="{ isLoading: loading }">
    <!--折叠内容-->
    <section class="import-config-content" :style="{ marginBottom: isScroll ? '34px' : '' }" ref="collapse">
      <bk-collapse
        ...
        <bk-collapse-item
          ...
          <template #default>
            ...
            <div class="collapse-item-left">
              <i class="bk-icon icon-play-shape collapse-item-icon" :class="{ 'icon-rotate': collapse.activeName.includes(item.name) }"></i>
              <span class="collapse-item-title" v-if="item.name === 'plugin'">{{ getItemTitle(item) }}</span>
              <span class="collapse-item-title" v-else>{{ selectedTitle(item) }}</span>
              <span class="collapse-item-mark" v-if="item.markName">{{ item.markName }}</span>
            </div>
            <!--右侧状态-->
            <div class="collapse-item-right" v-show="table.statistics && table.statistics[item.name]">
              <!-- eslint-disable-next-line vue/no-v-html -->
              <span v-html="statusHtml(item.name)"></span>
            </div>
          </template>
          <!--折叠表格-->
          <template #content>
            <bk-table
              ...
              <bk-table-column :label="$t('配置名称')" prop="name" width="205" align="left" header-align="left"></bk-table-column>
              <bk-table-column :label="$t('监控对象')" prop="label" width="190"></bk-table-column>
              <bk-table-column :label="$t('任务状态')" width="153" :render-header="renderHeader">
    ...
    <!--底部按钮-->
    <section class="import-config-footer" v-if="collapseList.length">
      <!--背景占位-->
      <div :class="{ 'footer-banner': isScroll }"></div>
      <bk-button
        theme="primary"
        class="mr10"
        :class="{ 'footer-button1': isScroll }"
        @click="handleImportClick"
        :disabled="disabledConfirmBtn"> {{ $t('开始导入') }} </bk-button>
      <bk-button
        class="button-cancel"
        :class="{ 'footer-button2': isScroll }"
        @click="handleImportCancel"> {{ $t('取消') }} </bk-button>
    </section>
    <!--空数据-->
    <section class="config-empty" v-if="!collapseList.length">
      <content-empty :title="$t('无数据')" :sub-title="$t('导入数据为空')"></content-empty>
    </section>
    <!--筛选数据-->
    <template>
      <div v-show="false">
        <div class="label-menu-wrapper" ref="labelMenu">
          <ul class="label-menu-list" v-if="filterHeader[currentStatus]">
            <li class="item" v-for="(item, index) in filterHeader[currentStatus].list" :key="index" @click="handleSelectLabel(item)">
          ...
          <div class="footer">
            <div class="btn-group">
              <bk-button :text="true" @click="handleStatusChange"> {{ $t('确定') }} </bk-button>
...
export default {
  ...
  computed: {
    // 开始导入按钮禁用状态
    disabledConfirmBtn() {
      return !(this.table.list && this.table.list.some(item => item.checked))
    },
    // 当前表格已选总数
    selectedTitle() {
      return (collapse) => {
        const checkedCount = this.table.list.filter(item => item.type === collapse.name && item.checked).length
        return `${collapse.title}（${this.$t('已选')} ${checkedCount} ${this.$t('个')}）`
      }
    },
    // 需要显示的 collapse (无数据的不显示)
    collapseList() {
      return this.collapse.list.filter(item => this.table.statistics[item.name]
        && this.table.statistics[item.name].total)
    },
    // 状态列是否勾选
    hasSelectStatus() {
      return collapseName => this.filterHeader[collapseName]
        && this.filterHeader[collapseName].list.filter(item => item.value)
          .map(item => item.value)
          .join(',')
    }
  },
  ...
  methods: {
    ...
    async handleInitImportConfigData() {
      ...
      this.table.list = data.configList ? data.configList.map((item) => {
        item.checked = item.type !== 'plugin' && item.fileStatus === 'success' // 默认勾选所有成功项
    ...
    handleSelectAll(selection, name) {
      this.table.list.forEach((item) => {
        if (item.type === name && item.status === 'success' && item.type !== 'plugin') {
    ...
    async handleImportClick() {
      const uuids = this.table.list.filter(item => item.checked && item.type !== 'plugin').map(item => item.uuid)
      this.loading = true
      const data = await this.handleImportConfig({ uuids })
      this.loading = false
      // 等待状态设置为importing后跳转
    ...
    /**
             ...
             */
    getItemTitle(item) {
      if (this.table.statistics[item.name] && this.table.statistics[item.name].total) {
        return `${item.title}（${this.$t('共')} ${this.table.statistics[item.name].total} ${this.$t('个')}）`
      }
      return `${item.title}（${this.$t('共')} 0 ${this.$t('个')}）`
    },
    /**
             * 自定义渲染表头（状态列）
    ...
    renderHeader(h, data) {
      ...
      return <span onClick={e => this.handleShow(e, collapseName)}
    ...
    handleShow(e, collapseName) {
      ...
      status.instance && status.instance.show(100)
    },
    /**
             * 勾选状态事件
             */
