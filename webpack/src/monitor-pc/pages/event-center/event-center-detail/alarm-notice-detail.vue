<template>
  <monitor-dialog :value="show" @change="handleShowChange"
                  :title="$t('通知状态明细')"
                  header-position="left"
                  :show-footer="false"
                  :show-confirm-btn="false"
                  width="620">
    <div class="notice-detail" v-bkloading="{ isLoading: loading }">
      <div class="notice-detail-select">
        <span class="select-lable"> {{ $t('通知次数：') }} </span>
        <bk-select class="select-input" v-model="value" @change="handleSelectChange" :clearable="false">
      ...
      <div class="notice-detail-table" v-bkloading="{ isLoading: table.loading }">
        <bk-table
          ...
          <bk-table-column
            ...
            <template slot-scope="scope">
              <div
                ...
                :title="scope.row.noticeInfo">{{scope.row.receiver}}
                <span class="col-notice" v-show="scope.row.noticeGroup">（{{scope.row.noticeGroup}}）</span>
              </div>
              <template v-else>
                <div v-if="scope.row[column.id]">
                  <bk-popover placement="top" width="200" v-if="scope.row[column.id].status === 0 && scope.row[column.id].message">
                    <div :class="'notice-' + scope.row[column.id].status"></div>
                    <div slot="content" style="word-break: break-all">
                      {{ scope.row[column.id].message }}
                    </div>
                  </bk-popover>
                  <div :class="'notice-' + scope.row[column.id].status" v-else></div>
                </div>
                <div v-else>
                  <div>--</div>
                </div>
          ...
          <template #empty>
            <i class="bk-table-empty-icon bk-icon" :class="tableAbnormalIcon"></i>
