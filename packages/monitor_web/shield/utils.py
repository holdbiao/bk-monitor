...
class ShieldDetectManager(object):
    ...
    def get_shield_list(self, bk_biz_id, category):
        ...
        if category == ShieldCategory.STRATEGY:
            shield_list = list(shield_list.filter(category=category, is_quick=True))

        # 告警事件列表页判断是否被屏蔽，需要看这个时间点有没有被屏蔽
        if category == ShieldCategory.EVENT:
    ...
    def check_shield_status(self, match_info):
        ...
        for shield in self.shield_list:
            # 遍历屏蔽的维度进行匹配
            for key, value in list(shield.dimension_config.items()):
                # 以'_'开头的key不去进行校验
                if key.startswith("_"):
                    continue
                ...
                return {"is_shielded": True, "id": shield.id}
        else:
            return {"is_shielded": False}
