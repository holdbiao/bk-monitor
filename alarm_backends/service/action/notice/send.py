...
class Sender(object):
    ...
    def __init__(self, context=None, title_template_path="", content_template_path=""):
        ...
        try:
            ...
            # 如果渲染失败，则尝试使用默认模板重新渲染
            context_dict["content_template"] = getattr(context, "DEFAULT_TEMPLATE", "")
            self.is_use_default = True
            self.title = title_template.render(context_dict)
            self.content = content_template.render(context_dict)

        # 如果消息长度超过限制，尝试使用缩短部分信息
        length_limit = settings.NOTICE_MESSAGE_MAX_LENGTH.get(getattr(context, "notice_way", None))
    ...
    def send(self, notice_way: str, notice_receivers: list):
        ...
        if notice_way not in settings.ENABLED_NOTICE_WAYS:
            notice_result = {}
            message = _("发送失败，全局配置中该消息渠道被禁用。")
