<template>
  <monitor-dialog
    :value.sync="show" :title="$t('添加日志关键字')"
    width="850"
    @change="handleValueChange"
    @on-confirm="handleConfirm">
    <div class="retrieval-set" v-bkloading="{ isLoading: loading }">
      ...
      <div class="retrieval-set-header">
        <bk-input :placeholder="placeholder"
                  :clearable="true"
      ...
      <div class="retrieval-content">
        ...
        <ul class="retrieval-content-left">
          <li v-for="(item, index) in origin.list" :key="index" class="left-item" @click="handleOrginChange(item.id)"
              :class="{ 'item-active': item.id === origin.value }">
            <span class="left-item-name">{{ item.name }}</span>
            <span class="left-item-num" :class="{ 'num-active': item.id === origin.value }">{{ item.count }}</span>
          </li>
        </ul>
        <!-- 右侧表格 -->
          ...
          <bk-table :data="table.data"
            ...
            <template v-if="origin.value === 'bkMonitor'">
              <bk-table-column width="52">
                <template v-slot="{ row }">
                  <bk-radio :value="row.checked"></bk-radio>
                </template>
              </bk-table-column>
              <bk-table-column :label="$t('采集项ID')" prop="related_id"></bk-table-column>
              <bk-table-column min-width="200" :label="$t('采集项名称')" prop="related_name"></bk-table-column>
            </template>
            <template v-else>
              <bk-table-column width="52">
                <template v-slot="{ row }">
              ...
              <bk-table-column :label="$t('索引集')" prop="metric_field"></bk-table-column>
              <bk-table-column min-width="200" :label="$t('索引')" prop="result_table_id"></bk-table-column>
              <bk-table-column width="100" :label="$t('数据源')" prop="scenario_name"></bk-table-column>
            </template>
          </bk-table>
        </div>
      </div>
...
export default class StrategyConfigRetreval extends Vue {
  ...
  get placeholder(): TranslateResult {
    return this.origin.value === 'logData' ? this.$t('支持索引/索引集名称搜索') : this.$t('支持监控项名称搜索')
  }
  ...
  .retrieval-set {
    ...
    .retrieval-content {
      ...
      &-left {
        ...
        font-size: 14px;
        background-image: linear-gradient(180deg, #dcdee5 1px, rgba(0, 0, 0, 0) 1px, rgba(0, 0, 0, 0) 100%), linear-gradient(90deg, #dcdee5 1px, rgba(0, 0, 0, 0) 1px, rgba(0, 0, 0, 0) 100%), linear-gradient(-90deg, #dcdee5 1px, rgba(0, 0, 0, 0) 1px, rgba(0, 0, 0, 0) 100%);
