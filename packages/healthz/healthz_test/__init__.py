...
# 根据不同的环境变量，引入不同的测试用例
ENV = settings.BKAPP_DEPLOY_PLATFORM


def load_yaml_by_name_and_env(name, env):
    """
    ...
    :return:
    """
    filename = "{name}_{env}.yml".format(name=name, env=env)
...
for component in component_to_test:
    load_yaml_by_name_and_env(component, ENV)
