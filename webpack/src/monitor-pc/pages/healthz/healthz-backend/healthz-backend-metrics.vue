<template>
  <div class="main-component-content">
    <p> {{ $t('后台服务器性能指标') }} </p>
    <div id="panelBody" style="margin-bottom: 10px;">
      <template>
        <el-table :data="filteredHostDataList" @selection-change="selectionChange"
                  ref="multipleTable" tooltip-effect="dark" style="width: 70%;">
          ...
          <el-table-column :label="$t('CPU使用率')"
                           align="center">
            <template slot-scope="scope" v-if="scope.row['system.cpu.percent']">
              <el-tooltip effect="dark" placement="right">
                <div slot="content">
                  <div style="min-width: 200px">
                    <p style="color: #fff"> {{ $t('单核CPU使用率') }} </p>
                    <div v-if="scope.row['system.cpu.percent.all'].value.length > 0">
                      <div v-for="(item, index) in scope.row['system.cpu.percent.all'].value"
                           :key="index" style="margin: 4px">
                        <span style="float: left;margin-left: 2px">{{index + 1}}</span>
                        <div style="margin-left: 26px;">
                          <el-progress :percentage="item"
                                       :color="getColor(item)"></el-progress>
                        </div>
                      </div>
                    </div>
                    <div v-else>
                ...
                <div style="display: inline-block;cursor: pointer;">
                  <el-progress type="circle"
                               ...
                               :width="40" :stroke-width="3" v-if="scope.row['system.cpu.percent'] && scope.row['system.cpu.percent'].value">
                  </el-progress>
          ...
          <el-table-column v-for="(item, index) in displayedMetricsExclude"
                           :key="index" :render-header="renderHeader" align="center">
            <template slot-scope="scope">
              <el-progress type="circle" :percentage="floatFix(scope.row[item].value)"
                           :color="scope.row[item].value > 80 ? '#ff6666' : '#46c37b'" :width="40"
                           :stroke-width="3" v-if="scope.row[item] && scope.row[item].hasOwnProperty('value')">
              </el-progress>
              <span v-else> {{ $t('无数据') }} </span>
            </template>
          </el-table-column>
...
import { Table, TableColumn, Tooltip, Progress } from 'element-ui'
export default {
    ...
    hostDataList() {
      ...
      for (let i = 0; i < this.globalData.length; i++) {
        ...
        if (tmpData.node_name === 'system'
            && Object.prototype.hasOwnProperty.call(tmpData, 'server_ip') && tmpData.server_ip !== '') {
