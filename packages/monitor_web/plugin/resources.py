...
import yaml
from monitor_web.plugin.constant import PluginType, MAX_METRIC_NUM
...
class SaveMetricResource(Resource):
    class RequestSerializer(MetricJsonBaseSerializer):
        plugin_id = serializers.RegexField(
            required=True, regex=r"^[a-zA-Z][a-zA-Z0-9_]*$", max_length=30, label=_("插件ID")
        )
        ...
        config_version = serializers.IntegerField(required=True, label=_("插件版本"))
        info_version = serializers.IntegerField(required=True, label=_("插件信息版本"))
        need_upgrade = serializers.BooleanField(required=False, label=_("是否升级"), default=False)
    ...
    def perform_request(self, validated_request_data):
        ...
        if plugin_type == CollectorPluginMeta.PluginType.SNMP:
            if len(validated_request_data["metric_json"]) > MAX_METRIC_NUM:
                raise SNMPMetricNumberError
...
class PluginRegisterResource(Resource):
    ...
    def register_package(self, file_name):
        ...
        while to_be_continue > 0:
            ...
            # 轮询超时
            raise RegisterPackageError({"msg": "轮询插件包注册任务超时，请检查节点管理celery是否正常运行"})
...
class PluginImportResource(Resource):
    ...
    def get_plugin(self):
        ...
        meta_dict = yaml.load(meta_content)
        # 检验plugin_type
...
class CheckPluginIDResource(Resource):
    class RequestSerializer(serializers.Serializer):
        plugin_id = serializers.CharField(required=True, label=_lazy("插件ID"))
...
class PluginUpgradeInfoResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        config_id = serializers.CharField(required=True, label=_("配置id"))
        config_version = serializers.IntegerField(required=True, label=_("插件版本"))
        info_version = serializers.IntegerField(required=True, label=_("插件信息版本"))
