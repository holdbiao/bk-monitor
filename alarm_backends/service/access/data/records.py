...
from constants.strategy import SYSTEM_PROC_PORT_DYNAMIC_DIMENSIONS, SYSTEM_PROC_PORT_METRIC_ID
...
class DataRecord(base.BaseRecord):
    ...
    def value(self):
        ...
        2. 四舍五入，保留两位
        """
        metric = self._item.data_source.metrics[0]
        metric_field = metric.get("alias") or metric["field"]
        value = self.raw_data.get(metric_field)
        if value is None:
            return
    ...
    def values(self):
        values = {}
        for metric in self._item.data_source.metrics:
            metric_field = metric.get("alias") or metric["field"]
            value = self.raw_data.get(metric_field)
            values[metric_field] = self._convert(value)
        values[self._item.data_source.time_field] = self.time
    ...
    def record_id(self):
        ...
        if self._item.metric_id == SYSTEM_PROC_PORT_METRIC_ID:
    ...
    def _origin_dimension(self):
        ...
        获取原始数据的维度
        """
        dimension_fields = self._item.rt_query_config.get("agg_dimension", [])[:]
        dimensions = {}
