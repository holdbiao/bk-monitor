...
class StandardCollector(six.with_metaclass(abc.ABCMeta, GenericCollector)):
    ...
    def fetch_content(self, result):
        ...
        for success_item in result["success"]:
            if success_item["log_content"].find(divide_symbol) != -1:
                success_item["log_content"] = success_item["log_content"].split(divide_symbol, 1)[1]
        return result
    ...
    def upgrade_or_install_then_deposit(self, hosts):
        ...
        检查更新并安装采集器，最后启动托管
        """
        upgrade_result = self.upgrade_or_install(hosts)

        deposit_result = self.start_deposit(upgrade_result["success"])

        result = {
            "success": deposit_result["success"],
            "pending": deposit_result["pending"] + upgrade_result["pending"],
            "failed": deposit_result["failed"] + upgrade_result["failed"],
        }
        return result
    ...
    def _execute_deposit(self, hosts, operate_type):
        ...
        执行托管程序
        """
        result = self.job_client.gse_process_operate(
            hosts=hosts,
            proc_name=self.get_linux_proc_name(),
            setup_path=self.get_linux_setup_path(),
            pid_path=self.get_linux_pid_path(),
            operate_type=operate_type,
            system_special=dict(
                windows=dict(
                    proc_name=self.get_windows_proc_name(),
                    setup_path=self.get_windows_setup_path(),
                    pid_path=self.get_windows_pid_path(),
                ),
                aix=dict(
                    proc_name=self.get_aix_proc_name(),
                    setup_path=self.get_aix_setup_path(),
                    pid_path=self.get_aix_pid_path(),
                ),
            ),
        )
        return result

    def start_deposit(self, hosts, *args, **kwargs):
        """
        启动托管程序
        """
        result = self._execute_deposit(hosts, settings.PROC_OP_TYPE_RESTART)
        result = self.label_failed_ip(result, _("托管程序启动失败"))
        return result

    def stop_deposit(self, hosts):
        """
        停用托管程序
        """
        result = self._execute_deposit(hosts, settings.PROC_OP_TYPE_STOP)

        failed_list = []

        for host in result["failed"]:
            # 进程不存在也可以认为停用成功
            if host["exit_code"] == settings.PROC_NON_EXIST:
                result["success"].append(host)
            else:
                failed_list.append(host)

        result["failed"] = failed_list
        result = self.label_failed_ip(result, _("托管程序停用失败"))
        return result
    ...
    def check_deposit(self, hosts):
        ...
        查询托管状态
        """
        result = self._execute_deposit(hosts, settings.PROC_OP_TYPE_CHECK)
        result = self.label_failed_ip(result, _("查询托管状态失败"))

        success_list = []
        for host in result["success"]:
            try:
                log_data = json.loads(host["log_content"])
    ...
    def reload_deposit(self, hosts):
        ...
        重载托管进程
        """
        result = self._execute_deposit(hosts, settings.PROC_OP_TYPE_RELOAD)
        result = self.label_failed_ip(result, _("查询托管状态失败"))
        return result

    def reload_or_restart_process(self, hosts):
        """
        检查进程托管状态，若托管则reload，若没有托管则restart
        """
        check_result = self.check_deposit(hosts)
        reload_result = self.reload_deposit(check_result["success"])
        restart_result = self.start_deposit(check_result["failed"])

        result = {
            "success": reload_result["success"] + restart_result["success"],
            "pending": reload_result["pending"] + reload_result["pending"] + check_result["pending"],
            "failed": reload_result["failed"] + restart_result["failed"],
        }
        return result
    ...
    def deploy_config_and_deposit(self, hosts, config, is_delete_mode=False):
        ...
        :param hosts: IP列表
        :param config: 新的配置
        :param is_delete_mode: 是否为删除模式
        :return:
        """
        deploy_result = self.deploy_config(hosts, config, is_delete_mode)
        deposit_result = self.start_deposit(deploy_result["success"])
        result = {
            "success": deposit_result["success"],
            "pending": deposit_result["pending"] + deploy_result["pending"],
    ...
    def uninstall(self, hosts):
        stop_deposit_result = self.stop_deposit(hosts)
        return stop_deposit_result
...
class UptimeCheckCollector(StandardCollector):
    ...
    def test(self, task, hosts=None):
        ...
        # 如果是动态拓扑，转化为主机
        task["config"]["hosts"] = resource.uptime_check.topo_template_host(task["config"].get("hosts", []))
