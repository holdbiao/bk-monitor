<template>
  <div class="upload-container">
    <div class="upload-view">
      <div class="file-icon">
        <span v-if="system === 'linux_aarch64'" class="item-icon icon-arm">ARM</span>
        <span v-else :class="['item-icon', 'icon-monitor', `icon-${system}`]">
          <!-- <span class="set-mark">
                        <span class="set-mark-font">64</span>
                    </span> -->
        </span>
      </div>
      <div class="upload-operator" @mouseover="handleMouseOver" @mouseleave="showClearIcon = false">
        <span v-if="!fileName" class="upload-btn"> {{ $t('点击上传文件') }} </span>
        <div v-else class="file-name">
          <span class="name">{{fileName}}</span>
          <div class="icon-wrapper">
            <span v-show="progress === 100 || isEdit"
          ...
          <div v-show="fileName" class="progress"><div :style="{ width: `${progress}%` }"></div></div>
        </div>
      </div>
    </div>
    <input v-show="!fileName" ref="uploadFile" type="file" :accept="accept" @change="handleSelectFile">
  </div>
</template>
<script>
  ...
  computed: {
    ...
    monitorIcon() {
      ...
      if (this.progress === 100 && !this.showClearIcon) {
        return 'icon-monitor icon-check-circle'
