...
def translate_field(apps, schema_editor):
    ...
    temp_field = AESTextField()
    temp_cipher = temp_field.cipher
    ...
    for cluster in cluster_info_model.objects.all():
        ...
        # 如果密码已经被加密过了，则不要处理了
        # 因为此处可能会先更新后台，这样后台的crontab任务其实会将密码改为了加密的存储了
        if cluster.password.startswith(temp_field.prefix):
            continue

        cluster.password = "".join([temp_field.prefix, temp_cipher.encrypt(cluster.password).decode("utf-8")])
        cluster.save()
    ...
    for host in influxdb_host_model.objects.all():
        # 如果密码已经被加密过了，则不要处理了
        # 因为此处可能会先更新后台，这样后台的crontab任务其实会将密码改为了加密的存储了
        if host.password.startswith(temp_field.prefix):
            continue

        host.password = "".join([temp_field.prefix, temp_cipher.encrypt(host.password).decode("utf-8")])
        host.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.AlterField(
            ...
            name="password",
            field=bkmonitor.utils.db.fields.AESTextField(default="", max_length=128, verbose_name="\u5bc6\u7801"),
        ),
        migrations.AlterField(
            model_name="influxdbhostinfo",
            name="password",
            field=bkmonitor.utils.db.fields.AESTextField(default="", verbose_name="\u5bc6\u7801"),
        ),
    ]
