<template>
  <article v-show="false">
    <div class="label-menu-wrapper" ref="labelMenuWrapper">
      <ul class="label-menu-list" v-if="filterType === 'checkbox'">
        <!-- <li class="item" v-for="(item, index) in label.list" :key="index" @click="handleSelectLabel(item)"> -->
        <li class="item" v-for="(item, index) in label.list" :key="index" @click="handleSelectCheckbox(item)">
          <!-- <bk-checkbox :value="item.value" :true-value="item.checked" :false-value="item.cancel"></bk-checkbox> -->
          <bk-checkbox :checked="item.checked" :true-value="true" :false-value="false"></bk-checkbox>
          <span class="name">{{ item.name }}</span>
        </li>
      </ul>
      <bk-radio-group v-model="radio.value" v-else-if="filterType === 'radio'" class="radio-group">
        <bk-radio
          ext-cls="radio-item"
      ...
      <div class="footer">
        <div class="btn-group">
          ...
          <span class="monitor-btn" @click="handleResetLable"> {{ $t('重置') }} </span>
        </div>
      </div>
    </div>
  </article>
...
export default {
  ...
  methods: {
    // 销毁
    handleDestroy() {
      this.label.instance && this.label.instance.destroy()
    ...
    handleShow() {
      ...
      this.label.instance && this.label.instance.show(100)
    },
    /**
             * 确定筛选
             */
