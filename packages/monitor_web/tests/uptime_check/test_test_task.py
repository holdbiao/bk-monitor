...
def mock_node_model(mocker):
    node = UptimeCheckNode(bk_biz_id=2, ip="10.0.1.10", plat_id=0, is_common=False, update_user="admin")
...
def mock_node_model_common(mocker):
    node = UptimeCheckNode(bk_biz_id=2, ip="10.0.1.10", plat_id=0, is_common=True, update_user="admin")
    all_nodes = FilterList()
    all_nodes.append(node)
    filter_func = mocker.patch("monitor_web.models.uptime_check.UptimeCheckNode.objects.filter", return_value=all_nodes)
    return filter_func
...
def mock_uptimecheck_collector_test(mocker):
    ...
    content = f.read()
    success_result = {"bk_cloud_id": 0, "ip": "10.0.1.10", "log_content": content, "plat_id": 0}
    result = {
        "failed": [],
        "pending": [],
...
def mock_uptimecheck_collector_test_job_fail(mocker):
    success_result = {"bk_cloud_id": 0, "ip": "10.0.1.10", "log_content": "", "plat_id": 0, "errmsg": "failed"}
    result = {
