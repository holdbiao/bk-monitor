<template>
  <div class="alarm-shield-event">
    <div class="event-item" v-if="bizList.length">
      <div class="event-item-label"> {{ $t('所属') }} </div>
    ...
    <div class="event-detail" v-if="shieldData.dimension_config">
      <div class="event-detail-label"> {{ $t('告警内容') }} </div>
      <div class="event-detail-content">
        <!-- <div class="column-item">
                    <div class="item-label"> {{ $t('告警级别：') }} </div><div class="item-content">{{shieldData.dimension_config.level}}</div>
                </div> -->
        <div class="column-item">
          <div class="item-label"> {{ $t('维度信息：') }} </div><div class="item-content">{{shieldData.dimension_config.dimensions}}</div>
        </div>
        <div class="column-item" style="margin-bottom: 18px">
          <div class="item-label"> {{ $t('检测算法：') }} </div><div class="item-content">{{shieldData.dimension_config.event_message}}</div>
        </div>
      </div>
    </div>
    <shield-date-config ref="noticeDate"></shield-date-config>
    <div class="event-desc">
      <div class="event-desc-label"> {{ $t('屏蔽原因') }} </div>
      <div class="event-desc-content">
        <bk-input class="content-desc" type="textarea" v-model="desc" :maxlength="100"></bk-input>
      </div>
    </div>
    ...
    <div class="event-btn">
      ...
      <bk-button class="button" :theme="'default'" @click="$router.push({ name: 'alarm-shield' })"> {{ $t('取消') }} </bk-button>
    </div>
  </div>
</template>
...
<style lang="scss" scoped>
  .alarm-shield-event {
    ...
    /deep/ .alarm-shield-notice .notice-config .notice-item {
      &-label {
