...
from alarm_backends.core.cache.cmdb import HostIDManager
...
class ServiceInstanceManager(RefreshByBizMixin, CMDBCacheManager):
    ...
    def refresh_by_biz(cls, bk_biz_id):
        ...
        for instance in instances:
            ...
            # 从缓存中补全主机IP信息
            host_key = HostIDManager.get(instance.bk_host_id)
            ip = None
            bk_cloud_id = None
            if host_key:
                ip, bk_cloud_id = host_key.split("|")
                bk_cloud_id = int(bk_cloud_id)
                host_to_service_instance[host_key].append(instance.service_instance_id)
            instance.ip = ip
            instance.bk_cloud_id = bk_cloud_id

        for host_key in host_to_service_instance:
            cls.cache.hset(
                cls.HOST_TO_SERVICE_INSTANCE_ID_CACHE_KEY, host_key, json.dumps(host_to_service_instance[host_key])
            )
    ...
    @classmethod
    def get_service_instance_id_by_host(cls, ip, bk_cloud_id):
        """
        获取主机下的服务实例id列表
        :param ip:
        :param bk_cloud_id:
        :return:
        """
        host_key = "{}|{}".format(ip, bk_cloud_id)
        result = cls.cache.hget(cls.HOST_TO_SERVICE_INSTANCE_ID_CACHE_KEY, host_key) or "[]"
