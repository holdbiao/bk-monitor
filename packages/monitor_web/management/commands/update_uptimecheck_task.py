...
class Command(BaseCommand):
    ...
    def do_job(self, node, script_content, script_type, account):
        ...
        if result[0]["status"] == 3 and result[0]["step_results"][0]["ip_status"] == 9:
            logger.info(_("节点%s拨测配置文件迁移完成") % host_key(ip=ip_list[0]["ip"], bk_cloud_id=ip_list[0]["bk_cloud_id"]))
            print(_("节点%s拨测配置文件迁移完成") % host_key(ip=ip_list[0]["ip"], bk_cloud_id=ip_list[0]["bk_cloud_id"]))
        else:
            logger.error(_("节点拨测配置文件迁移失败：%s") % result)
        ...
        for node in nodes:
            if host_key(ip=node.ip, plat_id=node.plat_id) in list(self.os_type_dict.keys()):
                ...
                for task in tasks:
                    ...
                    task.deploy()
                    logger.info(_("任务 %s 增加订阅流程，创建订阅任务ID:%d") % (task.id, task.subscription_id))
                    print(_("任务 %s 增加订阅流程，创建订阅任务ID:%d") % (task.id, task.subscription_id))
                # 逐个node迁移旧拨测配置文件
                self.move_old_conf(node)
            else:
                logger.info(_("不存在节点%s对应的主机，请联系节点管理进行处理") % host_key(ip=node.ip, plat_id=node.plat_id))
