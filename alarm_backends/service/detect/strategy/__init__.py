...
from core.errors.alarm_backends.detect import HistoryDataNotExists, InvalidAlgorithmsConfig, InvalidDataPoint
...
class Algorithms(object):
    ...
    def _detect(self, data_point):
        ...
        expr = self.gen_expr()
        byte_code = compile(expr, "<string>", "eval")
        return eval(byte_code, {}, self.get_context(data_point))
    ...
    def anomaly_message_template_tuple(self, data_point):
        ...
        :return: 前缀和后缀 -> tuple
        """
        prefix = data_point.item.data_source.metric_display
        unit = load_unit(data_point.unit)
        value, suffix = unit.fn.auto_convert(data_point.value)
    ...
    def query_history_points(self, data_points):
        ...
        for offset in offsets:
            ...
            from_timestamp, until_timestamp = (
                ...
                sorted_data_points[-1].timestamp - start + item.rt_query_config["agg_interval"],
            )

            accessed = None
            for history_timestamp in range(from_timestamp, until_timestamp, item.rt_query_config["agg_interval"]):
                if accessed is None:
                    accessed = True
