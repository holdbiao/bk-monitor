...
import { CellGroup, Cell, Collapse, CollapseItem, Col, Row } from 'vant'
...
export default class AlarmDetail extends Vue {
  ...
  handleGetData() {
    ...
    Promise.all([
      this.handleGetEventDetail(),
      this.handleGetSeriesData(),
  ...
  async handleGetSeriesData() {
    ...
    if (chartSeries) {
      const coverList = []
      const upBoundary = data.find(item => item.metric.metric_field.includes('upper_bound'))
            ?.datapoints?.map(item => ([item[1], item[0]])) || []
      const lowBoundary = data.find(item => item.metric.metric_field.includes('lower_bound'))
            ?.datapoints?.map(item => ([item[1], item[0]])) || []
