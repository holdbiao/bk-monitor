...
from alarm_backends.service.action.notice.utils import get_business_roles
from alarm_backends.service.action.shield.display_manager import DisplayManager
from bkmonitor.utils import time_tools
from bkmonitor.utils.range import TIME_MATCH_CLASS_MAP, load_field_instance
from bkmonitor.utils.range.conditions import AndCondition, EqualCondition
...
class ShieldObj(object):
    ...
    def is_match(self, alarm):
        ...
        if "bk_target_service_instance_id" in dimension and "service_instance_id" not in dimension:
            dimension["service_instance_id"] = dimension["bk_target_service_instance_id"]
    ...
    def send_notice(self, notice_type):
        ...
        for notice_way in self.config["notice_config"]["notice_way"]:
            ...
            logger.debug(_("[屏蔽通知] shield({}) 通知方式：{}, 内容：{}").format(self.config["id"], notice_way, sender.content))
