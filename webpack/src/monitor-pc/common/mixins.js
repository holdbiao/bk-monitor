...
const memberSelectorMixin = {
  methods: {
    ...
    renderPublicCode(node, parentClass, textClass, avatarClass, type) {
      return (
                <div class={parentClass}>
                    { node.logo ? <img class={avatarClass} src={node.logo} />
                      : <i class={node.type === 'group'
                        ? 'icon-monitor icon-mc-user-group only-img'
                        : 'icon-monitor icon-mc-user-one only-img'}></i> }
                    { type === 'list'
  ...
  methods: {
    ...
    handleConfig2Threshold(config) {
      if (Array.isArray(config)) {
        return config.reduce((pre, { method, threshold, condition }) => {
          if (condition === 'or') {
            pre.push([{
              method,
              threshold
    ...
    handleThreshold2Config(threshold) {
      if (Array.isArray(threshold)) {
        return threshold.reduce((pre, cur, index) => {
          pre.push(...cur.map((item, setIndex) => {
