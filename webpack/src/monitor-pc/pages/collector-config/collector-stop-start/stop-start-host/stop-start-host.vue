<template>
  <div class="collector-host" ref="collectorHost">
    <div v-if="tables && tables.contents && tables.contents.length">
      <config-deploy :all-data="tables" :is-running="isRunning" @can-polling="handlePolling"></config-deploy>
    </div>
    <div v-else>
      <div>
        <div class="empty-data">
          <div style="text-align: center;">
            <i class="icon-monitor icon-tishi hint-icon"></i>
            <p> {{ $t('未选择采集目标，无需下发') }} </p>
          </div>
        </div>
      </div>
    </div>
    <div class="footer" v-if="!openDetail">
      <bk-button v-show="tables" theme="primary" :disabled="btnType === 'RUNNING'" @click="btnType !== 'RUNNING' && handleConfirm(btnType)">{{textObj[btnType]}}</bk-button>
      <!-- <bk-button v-show="type !== 'UPGRADE' && btnType !== 'RUNNING'" @click="handleCancel"> {{ $t('取消') }} </bk-button> -->
      <bk-button v-show="type === 'UPGRADE' && !rollBackSuccess" :disabled="btnType === 'RUNNING' || btnType === 'UPGRADE'" @click="handleRollback"> {{ $t('回滚') }} </bk-button>
    </div>
  </div>
</template>
<script>
import configDeploy from '../../config-deploy/config-deploy'
import { collectTargetStatus, collectRunningStatus,
  toggleCollectConfigStatus, upgradeCollectPlugin,
  rollbackDeploymentConfig, isTaskReady } from '../../../../../monitor-api/modules/collecting'
export default {
  name: 'StopStartHost',
  components: {
    configDeploy
  ...
  methods: {
    ...
    handleRollback() {
      this.$bkInfo({
        ...
        confirmFn: () => {
          ...
          rollbackDeploymentConfig({ id: this.id }).then(() => {
    ...
    getHosts(id, type, ajaxFn) {
      return new Promise((resolve, reject) => {
        ...
        ajaxMethod({ id, is_auto: this.data.autoStatus }).then((data) => {
    ...
    getHostData(id) {
      return new Promise((resolve, reject) => {
        this.ajaxMark = false
        collectTargetStatus({ id, is_auto: this.data.autoStatus }).then((data) => {
          if (this.ajaxMark) {
            reject(data)
          } else {
            resolve(data)
    ...
    async handlePolling(v) {
      ...
      if (v) {
        clearTimeout(this.timer)
        await this.getHostData(this.id, this.nodeType).then((data) => {
          if (this.ajaxMark) {
            this.tables = this.handleData(data)
            const hasRunning = data.contents.some(item => item.child.some(set => (set.status === 'RUNNING'
            || set.status === 'PENDING')))
