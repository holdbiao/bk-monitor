    ...
    def set_value(self, key, value, timeout=60):
        if self.compress:
            try:
                ...
                logger.exception(_("[Cache]不支持序列化的类型: %s") % type(value))
                return False
        ...
        try:
            ...
            try:
                ...
                request_path = ""
            # 缓存出错不影响主流程
            logger.exception(_("存缓存[key:%s]时报错：%s\n value: %r\nurl: %s") % (key, e, value, request_path))
...
class CacheType(object):
    ...
    BIZ = CacheTypeItem(key="biz", timeout=settings.CACHE_BIZ_TIMEOUT, label=_lazy("业务及人员相关"), user_related=True)

    HOST = CacheTypeItem(key="host", timeout=settings.CACHE_HOST_TIMEOUT, label=_lazy("主机信息相关"), user_related=False)

    CC = CacheTypeItem(key="cc", timeout=settings.CACHE_CC_TIMEOUT, label=_lazy("CC模块和Set相关"), user_related=True)

    DATA = CacheTypeItem(key="data", timeout=settings.CACHE_DATA_TIMEOUT, label=_lazy("计算平台接口相关"), user_related=False)
    OVERVIEW = CacheTypeItem(
        key="overview", timeout=settings.CACHE_OVERVIEW_TIMEOUT, label=_lazy("首页接口相关"), user_related=False
    )
    ...
    LOG_SEARCH = CacheTypeItem(key="log_search", timeout=60 * 5, label=_lazy("日志平台相关"), user_related=False)
