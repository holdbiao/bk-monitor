    ...
    <ip-select
      ...
      <template #static-ip-panel>
        <slot name="static-ip-panel"
          ...
          <bk-table
            ...
            :data="selector.staticTableData" :empty-text="$t('查询无数据')">
            <bk-table-column prop="ip" label="IP" min-width="190"></bk-table-column>
            <bk-table-column :label="$t('状态')" width="135">
              <template slot-scope="scope">
                <div :class="['col-status', { 'success': scope.row.agentStatus === 'normal',
            ...
            <bk-table-column prop="cloudName" :label="$t('云区域')"></bk-table-column>
            <bk-table-column :label="$t('操作')" align="center" width="100">
              <template slot-scope="scope">
                <bk-button text @click="handleDeleteStaticIp(scope.row, scope.$index)"> {{ $t('移除') }} </bk-button>
              </template>
            </bk-table-column>
          </bk-table>
        </slot>
      ...
      <template #dynamic-topo-panel>
        <slot
          ...
          <bk-table
            ...
            :data="selector.dynamicTopoTableData" :empty-text="$t('查询无数据')">
            <bk-table-column prop="nodePath" :label="$t('节点名称')" min-width="200">
              <template slot-scope="scope">
                <!-- {{scope.row.nodePath}} -->
                <div class="col-text">
                  {{scope.row.nodePath }}
            ...
            <bk-table-column prop="count" :label="$t('Agent异常')" width="100">
              <template slot-scope="scope">
                <!-- <div>
                ...
                <div>
                  <span :class="[scope.row.agentErrorCount ? 'error' : 'not-exist']">{{scope.row.agentErrorCount}}</span>/<span>{{scope.row.count}}</span>
                </div>
              </template>
            </bk-table-column>
            <bk-table-column :label="$t('分类')" min-width="120">
              <template slot-scope="scope">
                <div class="col-label">
            ...
            <bk-table-column :label="$t('操作')" align="center" width="100">
              <template slot-scope="scope">
                <bk-button text @click="handleDeleteDynamicTopo(scope.row, scope.$index)"> {{ $t('移除') }} </bk-button>
              </template>
            </bk-table-column>
          </bk-table>
        </slot>
      </template>
      <template v-slot:change-input>
        <div v-if="!!changeInput && isRigthIp && $refs.ipSelect.curItem.type === 'staticInput'" class="err-tips"> {{ $t('IP格式有误或不存在，检查后重试！') }} </div>
        <div v-if="!!changeInput && isRigthIpExtranet && $refs.ipSelect.curItem.type === 'staticExtranet'" class="err-tips"> {{ $t('IP格式有误或内网IP，检查后重试！') }} </div>
      </template>
      <template v-slot:search-noData>
        <span> {{ $t('搜索结果为空！') }} </span>
      ...
      <template #static-extranet-panel="staticExtranet">
        <slot name="static-extranet-panel">
          <bk-table
            class="static-ip-table"
            :data="staticExtranet.data" :empty-text="$t('查询无数据')">
            <bk-table-column prop="ip" label="IP" min-width="190"></bk-table-column>
            <bk-table-column :label="$t('操作')" align="center" width="100">
              <template slot-scope="scope">
                <bk-button text @click="handleDeleteExtranetIp(scope.row, scope.$index)"> {{ $t('移除') }} </bk-button>
              </template>
            </bk-table-column>
          </bk-table>
        </slot>
...
<script>
import { getTopoTree, getHostInstanceByIp,
  getHostInstanceByNode, getServiceInstanceByNode } from '../../../../../monitor-api/modules/commons'
import IpSelect from '../../../../components/ip-select/ip-select'
import { createNamespacedHelpers } from 'vuex'
const { mapGetters } = createNamespacedHelpers('collector-config')
  ...
  methods: {
    ...
    async getDefaultData(type) {
      ...
      if ((type === 'static-topo' || type === 'static-ip') && !selector.staticTreeData.length) {
        await this.getTopoTree(selector.apiMap[type]).then((data) => {
    ...
    async handleBackDisplayData() {
      const { selector } = this
      await this.getSelectorData(selector.type, this.checkedData, selector.treeData).then((data) => {
    ...
    async getFetchData(type, payload) {
      ...
      if (type === 'static-topo') { // 静态拓扑
        ...
        if (Object.prototype.hasOwnProperty.call(node, statciNodeKey)) { // IP 节点
          ...
          if (node.children && node.children.length) {
            ...
            tmp.forEach((item) => { // 将有相同 IP 的节点筛选出来
              nodes = isService
      ...
      } else if (type === 'dynamic-topo') { // 动态拓扑
        if (payload.checked) { // 勾选
          // 父子去重，若某个节点其父节点被选中，
      ...
      if (['static-ip', 'static-topo'].includes(type)) {
        ...
        if (this.instanceType !== 'service' && (selector.checkedData.length || ipList.length)) {
          ...
          await this.getSelectorTableData(type, ip).then((data) => {
            if (data.length) {
              selector.tableData = data
        ...
        if (selector.checkedData.length) {
          // selector.loading = true
          this.$emit('loading-change', true)
          await this.getSelectorTableData(type, selector.dynamicTopoMap).then((data) => {
            selector.tableData = data
    ...
    getSameServiceInstance(serviceInstId, treeData, nodes = []) {
      treeData.forEach((item) => {
        if (Object.prototype.hasOwnProperty.call(item, 'service_instance_id')
    ...
    async getSelectorTableData(type, nodesMap) {
      ...
      if (['static-ip', 'static-topo'].includes(type)) {
        ...
        await this.getHostInstanceByIp(params).then((data) => {
        ...
        if (!this.isInstance) {
          await this.getHostInstanceByNode(this.getNodesByMap(type, nodesMap)).then((data) => {
            tableData = this.handleNodeData(data)
          })
            .catch(() => {})
        } else {
          await this.getServiceInstanceByNode(this.getNodesByMap(type, nodesMap)).then((data) => {
            tableData = this.handleNodeData(data)
          })
            .catch(() => {})
        }
    ...
    findChildren(parent) {
      return parent.children.reduce((prev, cur) => prev.concat(cur.children && cur.children.length
        ? this.findChildren(cur)
        : cur), [])
    },
    async getSelectorData(type, checkedData, treeData) {
      ...
      if (['static-ip', 'static-topo'].includes(type)) {
        ...
        await this.getSelectorTableData(this.selector.type, staticMap).then((data) => {
          selector.staticTableData = data
        })
          .catch(() => {})
      } else {
        ...
        await this.getSelectorTableData(this.selector.type, dynamicTopoMap).then((data) => {
          selector.dynamicTopoTableData = data
        })
          .catch(() => {})
      }
    ...
    // 重置虚拟滚动
    resize() {
      this.$refs.ipSelect && this.$refs.ipSelect.resize()
    }
  }
}
</script>
