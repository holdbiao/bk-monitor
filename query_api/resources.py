...
class GetTSDataResource(Resource):
    class RequestSerializer(serializers.Serializer):
        sql = serializers.CharField(required=True, label=_("SQL查询语句"))
...
class GetEsDataResource(Resource):
    class RequestSerializer(serializers.Serializer):
        index_name = serializers.CharField(required=True, label=_("索引名"))
        doc_type = serializers.CharField(required=True, label=_("文档类型"))
        query_body = serializers.DictField(required=True, label=_("查询内容"))
        datasource_info = serializers.DictField(required=True, label=_("链接信息"))
    ...
    def perform_request(self, validated_request_data):
        try:
            ...
            raise APIException(_("connect hosts:{}:{} error, message is {}.".format(domain_name, port, conn_err)))
