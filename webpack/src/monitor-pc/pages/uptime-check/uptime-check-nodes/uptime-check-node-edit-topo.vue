<template>
  <bk-dialog
    width="850"
    :value="isShow"
    :close-icon="false" :ok-text="$t('添加')" :cancel-text="$t('取消')"
    :auto-close="false"
    ...
    <ip-selector
      ...
      <template #static-ip-panel="{ data }">
        <bk-table :data="data">
          ...
          <bk-table-column prop="agentStatus" :label="$t('状态')">
          ...
          <bk-table-column prop="isBuilt" :label="$t('是否配置任务')">
            <template #default="{ row }">
              {{ row.isBuilt ? $t('是') : $t('无') }}
            </template>
          </bk-table-column>
          <bk-table-column :label="$t('操作')" align="center" width="80">
            <template #default="rowData">
              <bk-button text @click="handleDeleteStaticIp(rowData)"> {{ $t('移除') }} </bk-button>
            </template>
          </bk-table-column>
        </bk-table>
      </template>
    ...
    <template #footer>
      <bk-button theme="primary" :title="$t('添加')" class="mr10" :disabled="disabledAddBtn" @click="handleAddTargetIp"> {{ $t('添加') }} </bk-button>
      <bk-button :title="$t('取消')" @click="handleCancel"> {{ $t('取消') }} </bk-button>
    </template>
  </bk-dialog>
</template>
<script>
