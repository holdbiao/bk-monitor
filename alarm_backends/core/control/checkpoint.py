...
class Checkpoint(object):
    ...
    def get(self, min_last_checkpoint=0):
        """
        获取监控最后一个处理时间
        ...
        if (now - last_check_point) > settings.MIN_DATA_ACCESS_CHECKPOINT:
            return max(min_last_checkpoint, now - settings.MIN_DATA_ACCESS_CHECKPOINT)
