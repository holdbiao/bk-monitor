...
cp -R ${PROJECT_DIR}/conf/platform/community.py "${CURRENT_DIR}"/tmp/conf/platform/community.py
...
cp -R ${PROJECT_DIR}/conf/web/development/community.py "${CURRENT_DIR}"/tmp/conf/web/development/community.py
...
cp -R ${PROJECT_DIR}/conf/web/production/community.py "${CURRENT_DIR}"/tmp/conf/web/production/community.py
...
cp -R ${PROJECT_DIR}/conf/web/testing/community.py "${CURRENT_DIR}"/tmp/conf/web/testing/community.py
...
cp -R ${PROJECT_DIR}/version/web/community/app.yml "${CURRENT_DIR}"/tmp
cp -R ${PROJECT_DIR}/version/web/community/release.md "${CURRENT_DIR}"/tmp
...
sed -i '/BKAPP_DEPLOY_PLATFORM/s/ieod/community/g' "${CURRENT_DIR}"/tmp/settings.py
sed -i '/_choices/s/\[.*\]/\["community",\]/g' "${CURRENT_DIR}"/tmp/settings.py
sed -i 's/BKAPP_DEPLOY_PLATFORM =.*/BKAPP_DEPLOY_PLATFORM = "community"/g' "${CURRENT_DIR}"/tmp/settings.py
sed -i '/BKAPP_DEPLOY_PLATFORM/s/ieod/community/g' "${CURRENT_DIR}"/tmp/conf/platform/default_settings.py
#sed -i '/ieod\|tencent\|qcloud\|bkclouds\|enterprise/d' "${CURRENT_DIR}"/tmp/conf/platform/default_settings.py
...
sed -i 's/ee =.*/ee = False/g' "${CURRENT_DIR}"/tmp/packages/common/context_processors.py
sed -i 's/ce =.*/ce = True/g' "${CURRENT_DIR}"/tmp/packages/common/context_processors.py

# 3.2 clean dir&file
rm -rf "${CURRENT_DIR}"/tmp/packages/monitor/resource/bkclouds
rm -rf "${CURRENT_DIR}"/tmp/packages/monitor/resource/tencent
rm -rf "${CURRENT_DIR}"/tmp/packages/monitor/resource/enterprise
...
find "${CURRENT_DIR}"/tmp/packages/healthz/healthz_test/ -type f -a ! -name '*init*' -a ! -name '*community*' -delete
