<template>
  <div class="config-deploy">
    <div class="config-deploy-description" v-if="$route.name === 'collect-config-operate-detail'">
      <i class="icon-monitor icon-tips item-icon"></i><span> {{ $t('该内容是对') }} {{config.name}}{{ $t('进行') }} "<span class="operation-step-name">{{operationStepName}}</span>" {{ $t('操作的执行详情') }}</span>
    </div>
    <div class="config-deploy-header" v-if="isRunning">
      <div class="bk-button-group">
        ...
        <bk-button @click="header.status = 'SUCCESS'" :class="{ 'is-selected': header.status === 'SUCCESS' }" size="normal"> {{ $t('成功') }}({{header.data.successNum}})</bk-button>
        <bk-button @click="header.status = 'FAILED'" :class="{ 'is-selected': header.status === 'FAILED' }" size="normal"> {{ $t('失败') }}({{header.data.failedNum}})</bk-button>
      ...
      <bk-button :icon="header.batchRetry ? 'loading' : ''" :disabled="header.batchRetry || !(header.data.failedNum > 0 && header.data.pendingNum === 0)" class="header-retry" hover-theme="primary" @click="handleBatchRetry">
        <i v-if="!header.batchRetry" class="icon-monitor icon-mc-retry"></i>
        {{ $t('失败批量重试') }}
      </bk-button>
      <bk-button :icon="disBatch ? 'loading' : ''" :disabled="!haveDeploying || disBatch" class="header-retry" hover-theme="primary" @click="handleBatchStop">{{ $t('批量终止') }} </bk-button>
    </div>
    <div class="config-deploy-content" v-if="content.length">
      <right-panel class="content-panel"
                   ...
                   :style="{ borderBottomWidth: item.expand ? '0' : '1px', marginBottom: index === content.length - 1 ? '20px' : '10px' }">
        <bk-table class="content-panel-table" :data="item.table" :empty-text="$t('查询无数据')" max-height="450">
          ...
          <bk-table-column width="120" :label="$t('状态')">
            <template slot-scope="props">
              <div class="col-status">
                <img class="col-status-img" v-if="isRunning && (props.row.status === 'RUNNING' || props.row.status === 'PENDING')" src="../../../static/images/svg/spinner.svg" />
                <div class="col-status-radius" v-if="['SUCCESS', 'FAILED'].includes(props.row.status)"
                     :style="{ background: statusMap[props.row.status].color , 'border-color': borderMap[props.row.status] }"></div>
          ...
          <bk-table-column width="80" label="">
            <template slot-scope="props">
              <div class="col-retry" v-if="isRunning && props.row.status === 'FAILED'" @click="props.row.status === 'FAILED' && handleRetry(props.row, item)"> {{ $t('重试') }} </div>
              <div class="col-retry" v-if="isRunning && ['DEPLOYING','RUNNING','PENDING'].includes(props.row.status)" @click="['DEPLOYING','RUNNING','PENDING'].includes(props.row.status) && handleRevoke(props.row, item)"> {{ $t('终止') }} </div>
            </template>
          </bk-table-column>
        </bk-table>
        ...
        <div slot="title" class="panel-title">
          ...
          <div class="total">
            ...
            <span class="num" v-else>{{ $t('共') }} {{item.successNum + item.failedNum + item.pendingNum}} {{config.target_object_type === 'HOST' ? $t('台主机') : $t('个实例')}}</span>
          </div>
        </div>
      </right-panel>
    </div>
  ...
  methods: {
    ...
    handleStatusChange(status) {
      this.content.forEach((item) => {
        ...
        item.child.forEach((set) => {
          if ((status === 'RUNNING'
          && (set.status === 'RUNNING' || set.status === 'PENDING'))
          || set.status === status
          || status === 'ALL') {
            item.table.push(set)
          }
