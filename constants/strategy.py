...
class SourceType(object):
    BKMONITOR = ("BKMONITOR", _lazy("监控采集"))
    BASEALARM = ("BASEALARM", _lazy("系统事件"))
    BKDATA = ("BKDATA", _lazy("计算平台"))
    CUSTOMEVENT = ("CUSTOMEVENT", _lazy("自定义事件"))
    CUSTOMTIMINGDATA = ("CUSTOMTIMINGDATA", _lazy("自定义时序数据"))
...
# 系统时间主机重启、进程端口对应检测算法
EVENT_DETECT_LIST = {
    "os_restart": [{"algorithm_type": "OsRestart", "algorithm_config": []}],
    "proc_port": [{"algorithm_type": "ProcPort", "algorithm_config": []}],
    "ping-gse": [{"algorithm_type": "PingUnreachable", "algorithm_config": []}],
}
...
GSE_PROCESS_EVENT_NAME = {
    "process_restart_success": _lazy("进程重启成功"),
    "process_restart_failed": _lazy("进程重启失败"),
    "process_cancel_restart": _lazy("进程不再托管"),
    "process_resource_limit_exceed": _lazy("进程资源超限（单进程维度）"),
    "process_group_resource_limit_exceed": _lazy("进程资源超限（进程组维度）"),
}
