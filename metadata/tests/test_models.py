...
from metadata.models import Label
from metadata.models.storage import ClusterInfo
from metadata.utils import consul_tools

from .utils import any_return_model
...
class TestDataSource(object):
    ...
    def create_base_cluster(self):
        ...
        Label.objects.bulk_create(
            [
                Label(label_id=self.data_source_label, label_type=Label.LABEL_TYPE_SOURCE),
                Label(label_id=self.data_type_label, label_type=Label.LABEL_TYPE_TYPE),
                Label(label_id=self.result_table_label, label_type=Label.LABEL_TYPE_RESULT_TABLE),
                Label(label_id=Label.RESULT_TABLE_LABEL_OTHER, label_type=Label.LABEL_TYPE_RESULT_TABLE),
                Label(label_id="applications", label_type=Label.LABEL_TYPE_RESULT_TABLE),
                Label(label_id="hosts", label_type=Label.LABEL_TYPE_RESULT_TABLE),
            ]
        )

    def delete_base_cluster(self):
        ...
        models.InfluxDBStorage.objects.all().delete()
        models.DataSourceResultTable.objects.all().delete()
        models.KafkaTopicInfo.objects.all().delete()
    ...
    def test_create_new_data_source(self, mocker):
        ...
        self.mock_outer_ralay(mocker)
        mocker.patch("consul.base.Consul.KV.delete", return_value=True)

        self.create_base_cluster()
        ...
        assert models.InfluxDBStorage.objects.get(table_id=new_table.table_id).source_duration_time == "90d"
        ...
        assert models.ResultTableField.objects.all().count() == 8
        ...
        # 判断字段的修改是否生效
        assert models.ResultTableField.objects.filter(table_id=new_table.table_id).count() == 7
        ...
        # 1. 创建ES存储
        default_storage_table_id = "{}_es_defaul_storage".format(new_data_source.data_name)

        def es_get_index_stat(index_name, *args, **kwargs):
            # 先找到原本的table_id
            table_id = index_name[:-1].replace(".", "_")
            new_date = datetime.datetime.utcnow().strftime("%Y%m%d%H")

            return {
                "indices": {
                    "{}{}_0".format(table_id, new_date): {
                        "primaries": {"store": {"size_in_bytes": 600 * 1024 * 1024 * 1024}}
                    }
                }
            }

        stat_mock = mocker.patch(  # noqa
            "elasticsearch5.client.indices.IndicesClient.stats", side_effect=es_get_index_stat
        )
        ...
        new_table = models.ResultTable.create_result_table(
            ...
            default_storage_config={
                "slice_gap": 120,
                "retention": 20,
                "index_settings": {"number_of_shards": 30, "number_of_replicas": 1},
                "mapping_settings": {"external_mappings": "test"},
            },
            field_list=(
                {
                    "field_name": "log",
                    ...
                    "alias_name": "log",
                    "description": "日志内容",
                    "option": {"es_type": "text", "es_include_in_all": True, "es_doc_values": False, "es_index": True},
                },
            ),
            bk_biz_id=2,
            label=self.result_table_label,
        ...
        # 确保有足够的index创建次数，验证遍历创建功能
        now_datetime = datetime.datetime.utcnow().strftime("%Y%m%d%H")
        # 默认只会创建两个index：当前周期，和下一个周期，防止接口等待超时
        assert self.create_index_mock.call_count == 2

        # 确认创建index的内容是否符合预期
        self.create_index_mock.assert_any_call(
            # index名
            index="{}_{}_0".format(default_storage_table_id.replace(".", "_"), now_datetime),
            body={
                "mappings": {
                    "properties": {
                        "log": {"doc_values": False, "include_in_all": True, "index": True, "type": "text"},
                        "time": {},
                    },
                    "external_mappings": "test",
                },
                "settings": {"number_of_replicas": 1, "number_of_shards": 30},
            },
            params={"request_timeout": 30},
        )
        assert models.ESStorage.objects.filter(table_id=new_table.table_id).exists()
        es_storage = models.ESStorage.objects.get(table_id=new_table.table_id)
        assert es_storage.retention == 20

        # ES7兼容
        now_datetime = datetime.datetime.utcnow().strftime("%Y%m%d%H")
        es_storage_cluster = ClusterInfo.objects.get(cluster_type=ClusterInfo.TYPE_ES, is_default_cluster=True)
        es_storage_cluster.cluster_id = None
        es_storage_cluster.cluster_name = "es7"
        es_storage_cluster.is_default_cluster = False
        es_storage_cluster.version = "7.2.0"
        es_storage_cluster.save()
        storage_table_id = default_storage_table_id + "_es7"
        es_7 = models.ResultTable.create_result_table(
            bk_data_id=new_data_source.bk_data_id,
            ...
            default_storage=models.ClusterInfo.TYPE_ES,
            default_storage_config={
                "cluster_id": es_storage_cluster.cluster_id,
                "slice_gap": 120,
                "retention": 20,
                "index_settings": {"number_of_shards": 30, "number_of_replicas": 1},
                "mapping_settings": {"external_mappings": "test"},
            ...
            field_list=(
                {
                    ...
                    "alias_name": "log",
                    "description": "日志内容",
                    "option": {"es_type": "text", "es_doc_values": False, "es_index": True},
                },
            ),
            bk_biz_id=2,
            label=self.result_table_label,
        ...
        self.create_index_mock.assert_any_call(
            # index名
            index="{}_{}_0".format(storage_table_id.replace(".", "_"), now_datetime),
            body={
                "mappings": {
                    "properties": {"log": {"doc_values": False, "index": True, "type": "text"}, "time": {}},
                    "external_mappings": "test",
                },
                "settings": {"number_of_replicas": 1, "number_of_shards": 30},
            },
        ...
        # 测试删除能力
        clean_count = es_storage.clean_index()
        assert clean_count == 1
        out_date = (datetime.datetime.utcnow() - datetime.timedelta(days=365 * 3)).strftime("%Y%m%d%H")
        self.delete_index_mock.assert_called_once_with(
            "{}_{}_0".format(default_storage_table_id.replace(".", "_"), out_date)
        )

        # 测试index大小切片能力
        self.delete_index_mock.reset_mock()
        self.create_index_mock.reset_mock()
        self.put_alias_mock.reset_mock()

        def es_get_mapping(index, *args, **kwargs):
        ...
        count_mock = mocker.patch(  # noqa
            "elasticsearch5.client.Elasticsearch.count",
            return_value={"_shards": {"failed": 0, "successful": 0, "total": 0}, "count": 100},
        )

        mapping_mock = mocker.patch(  # noqa
            "elasticsearch5.client.indices.IndicesClient.get_mapping", side_effect=es_get_mapping
        )

        es_storage.create_index(ahead_time=0)

        now_date = datetime.datetime.utcnow().strftime("%Y%m%d%H")
        # self.delete_alias_mock.assert_called_with(
        #     index='2_system_cpu_es_defaul_storage_{}_0'.format(now_date),
        #     name="{}_{}_write".format(default_storage_table_id.replace('.', '_'), now_date)
        # )
        self.delete_alias_mock.assert_called_with(
            index="2_system_cpu_es_defaul_storage_{}_0".format(now_date),
            name="write_{}_{}".format(now_date, default_storage_table_id.replace(".", "_")),
        )

        self.create_index_mock.assert_called_once_with(
            index="{}_{}_1".format(default_storage_table_id.replace(".", "_"), now_date),
            body={
                "mappings": {
                    "properties": {
                        "log": {"doc_values": False, "include_in_all": True, "index": True, "type": "text"},
        ...
        self.put_alias_mock(
            index="{}_{}_1".format(default_storage_table_id.replace(".", "_"), now_date),
            name="{}_{}".format(default_storage_table_id.replace(".", "_"), now_date),
        )

        # 结果表启用测试
        new_table.modify(is_enable=False, operator="admin")
        es_7.modify(is_enable=False, operator="admin")
        new_data_source.refresh_consul_config()
        ...
        if not IS_CONSUL_MOCK:
            ...
            consul_config = json.loads(consul_client.kv.get(new_data_source.consul_config_path)[1]["Value"])
            # 仅保留了一个kafka的存储结果表
            assert len(consul_config["result_table_list"]) == 1
    ...
    def test_standard_fields_discover(self, mocker):
        """测试字段自动发现的逻辑能力"""
        ...
        field_path = "{}_{}_{}/metadata/data_id/{}/fields".format(
    ...
    def test_init_influxdb_config(self, mocker):
        """测试influxdb的配置初始化能力"""
        # 写入环境变量
        os.environ["INFLUXDB_BKMONITORV3_PORT"] = "5290"
        for index in range(2):
            host_name = "INFLUXDB_BKMONITORV3_IP%s" % index
            os.environ[host_name] = "10.0.0.%s" % index

        os.environ["BKMONITORV3_INFLUXDB_PROXY_HOST"] = "test.influxdb.name"
        os.environ["BKMONITORV3_INFLUXDB_PROXY_PORT"] = "10203"

        os.environ["ES7_REST_PORT"] = "10004"
        os.environ["ES7_HOST"] = "test.es7.name"
        os.environ["ES7_NAME"] = "username"
        os.environ["ES7_PASS"] = "password"

        os.environ["KAFKA_HOST"] = "test.kafka.name"
        os.environ["KAFKA_PORT"] = "9200"
        ...
        assert models.ESStorage.objects.all()[0].index_body == {
            "mappings": {
                "properties": {
                    ...
                    "time": {"type": "date_nanos", "format": "epoch_millis"},
                },
                "dynamic_templates": [
    ...
    def test_time_series(self, mocker):
        ...
        assert new_group.table_id == "bkmonitor_time_series_{}.base".format(new_group.bk_data_id)

        # 数据清理
        self.delete_base_cluster()

    def test_es_storage(self, mocker):
        """
        ...
        mock_es_get_alias.assert_called_once_with(
        ...
        #             }
        #         },
        #         f"{new_es_storage.index_name}_{now_datetime.strftime(new_es_storage.date_format)}_0": {
        #             "aliases": {
        #                 f"{new_es_storage.index_name}_{now_datetime.strftime(new_es_storage.date_format)}_write": {},
        #                 f"write_{now_datetime.strftime(new_es_storage.date_format)}_{new_es_storage.index_name}": {},
        #             }
        #         },
        #     },
        # )
        ...
        # )
        #
        # mock_es_delete_index = mocker.patch("elasticsearch5.client.indices.IndicesClient.delete", return_value=True)
        # mock_es_get_alias.assert_called_once_with(index=f"*{new_es_storage.index_name}_*_*")
        # mock_es_delete_alias.assert_called_once_with(
        #     index=f"{new_es_storage.index_name}_2020010306_0", name=f"write_2020010306_{new_es_storage.index_name}"
        # )
        # mock_es_delete_index.assert_called_once_with(index=f"{new_es_storage.index_name}_2020010306_0")

    def test_es_reallocate_index(self, mocker):
        self.create_base_cluster()
    ...
    def test_es_clear_index(self, mocker):
        self.create_base_cluster()
        ...
        mocker.patch(
            ...
            return_value={
                f"{new_es_storage.index_name}_2020010306_0": {
                    "aliases": {
                        f"{new_es_storage.index_name}_{datetime1_str}_write": {},
                ...
                f"{new_es_storage.index_name}_2020010304_0": {"aliases": {"invalid_alias": {}}},
            },
        )
        ...
        mock_es_delete_alias.assert_called_once_with(
            index=f"{new_es_storage.index_name}_2020010306_0", name=f"{new_es_storage.index_name}_{datetime1_str}_write"
        )
        mock_es_delete.assert_called_once_with(index=f"{new_es_storage.index_name}_2020010305_0")
