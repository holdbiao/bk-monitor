<template>
  <bk-dialog
    ...
    <div class="strategy-config-metric" v-bkloading="{ 'isLoading': loading }">
      ...
      <bk-search-select :show-popover-tag-change="false" ref="searchSelect" :popover-zindex="2600" class="metric-search" @change="handleSearch" :data="searchObj.data" v-model="searchObj.keyWord" :placeholder="$t('搜索')"></bk-search-select>
      <div class="metric-content">
        <ul class="metric-content-left">
          <template v-for="(item, index) in monitorSource[scenarioType]">
            <li
              v-if="(item.data_type_label === 'event' && mericType === 'event') || ( item.data_type_label !== 'event' && item.data_type_label !== 'log' && mericType !== 'event')"
              class="left-item"
              :key="index"
              ...
              <span class="left-item-num" :class="{ 'num-active': item.source_type === left.active }">{{item.count}}</span>
            </li>
          </template>
        </ul>
        <div class="metric-content-right">
          <bk-table ref="metricTable"
                    :class="{ 'hidden-header': mericType === 'common' }"
            ...
            <template>
              <bk-table-column
                ...
                v-for="(item, index) in right.columns" :key="index" :label="item.label">
                <template slot-scope="scope">
                  <bk-radio v-if="item.type === 'radio'" :disabled="scope.row.disabled" :value="scope.row.select" name="metric" @change="handleRadioChange(scope.row)"></bk-radio>
                  <span class="col-name" v-if="item.type === 'related_id'" @mouseenter="handleNameEnter($event, scope.row)" @mouseleave="handleNameLeave($event, scope.row)">
                    {{scope.row.related_id}}
                  </span>
                  <span class="col-name" v-if="item.type === 'result_table_id'" @mouseenter="handleNameEnter($event, scope.row)" @mouseleave="handleNameLeave($event, scope.row)">
                    {{scope.row.result_table_id}}
                  </span>
                  <span v-if="item.type === 'result_table_name'" @mouseenter="handleNameEnter($event, scope.row)" @mouseleave="handleNameLeave($event, scope.row)">
                    {{scope.row.result_table_name}}
                  </span>
                  <span v-if="item.type === 'related_name'" @mouseenter="handleNameEnter($event, scope.row)" @mouseleave="handleNameLeave($event, scope.row)">
                    {{scope.row.related_name}}
                  </span>
                  <span v-if="item.type === 'metric_field'" style="cursor: pointer;" @mouseenter="handleNameEnter($event, scope.row)" @mouseleave="handleNameLeave($event, scope.row)">
                    {{scope.row.metric_field}}
                  </span>
                  <span v-if="item.type === 'metric_field_name'" style="cursor: pointer;" @mouseenter="handleNameEnter($event, scope.row)" @mouseleave="handleNameLeave($event, scope.row)">
                    <template v-if="mericType === 'common'">
                      <template v-if="handleMetricFieldName(scope.row).alias">
                        {{handleMetricFieldName(scope.row).alias}}<span class="result-table-id">( {{handleMetricFieldName(scope.row).id}} )</span>
                      </template>
                      <template v-else>
            ...
            <template #append>
              <div v-show="right.scrollLoading" class="footer-loading">
    ...
    <template #footer>
      <bk-button theme="primary" :disabled="!right.value" @click="handleConfirm"> {{ $t('添加') }} </bk-button>
      <bk-button @click="handleCancel"> {{ $t('取消') }} </bk-button>
    </template>
    <div v-show="false">
      <div @mouseleave="handleTipsLeave" class="uptimecheck-tips" ref="uptimecheckTips"> {{ $t('该指标需设置期望返回码/期望响应信息后才可选取') }} <span style="color: #3a9eff; cursor: pointer;" class="set-uptimecheck" @click="handleToUptimcheck"> {{ $t('前往设置') }} </span>
      </div>
    </div>
  </bk-dialog>
</template>
...
export default {
  ...
  props: {
    ...
    mericType: {
      type: String,
      validator(v) {
        return ['event', 'common'].includes(v)
      },
      required: true
    }
  },
  ...
  beforeDestroy() {
    this.popoverInstance && this.popoverInstance.detory()
    this.scrollEl.removeEventListener('scroll', this.throttledScroll)
  },
  methods: {
    // 清理缓存的搜索条件
    ...
    getSearchOptions(sourceType) {
      ...
      const searchObj = {
        ...
        log_time_series: [
          ...options,
          { id: 'releated_id', name: this.$t('索引集'), children: [] }
        ],
        custom_event: [
    ...
    handleAfterLeave() {
      if (this.$refs.searchSelect && this.$refs.searchSelect.popperMenuInstance) {
    ...
    async handleSelectSource(type) {
      ...
      if (this.metricList[this.scenarioType][type] && this.monitorSource[this.scenarioType].length) {
        this.right.columns = this.mericType === 'common'
          ? this.handleTableHeader(this.COLUMNS[type]) : this.COLUMNS[type]
    ...
    async handleScroll(e) {
      ...
      if (isEnd && !this.right.scrollLoading
    ...
    handleSourceMetric(data, page, scenarioType, sourceType) {
      ...
      this.right.columns = this.mericType === 'common'
        ? this.handleTableHeader(this.COLUMNS[sourceType])
        : this.COLUMNS[sourceType]
    ...
    handleStaticParams(staticParams) {
      ...
      return {
        ...
        data_source_label: this.isEdit && this.requestCount === 1
          ? this.metric.dataSourceLabel : source.data_source_label || 'bk_monitor',
        data_type_label: this.isEdit && this.requestCount === 1
    ...
    getMonitorSource(scenarioType, sourceType, staticObj) {
      ...
      return getMetricList(params).then((data) => {
