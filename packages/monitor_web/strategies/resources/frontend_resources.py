...
from bkmonitor.models import StrategyModel, ItemModel, QueryConfigModel
...
from monitor_web.models import CollectConfigMeta, CustomEventGroup, CustomEventItem, DataTargetMapping
...
from constants.strategy import UPTIMECHECK_ERROR_CODE_MAP
from bkmonitor.strategy.new_strategy import get_metric_id, Strategy
from monitor_web.strategies.metric_list_cache import DEFAULT_DIMENSIONS_MAP, FILTER_DIMENSION_LIST, DefaultDimensions
...
class GetUnitInfoResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        unit_id = serializers.CharField(required=True, label=_("二级标签"))
...
class GetMetricListResource(Resource):
    class RequestSerializer(serializers.Serializer):
        ...
        data_source_label = serializers.CharField(default="", label=_("指标数据来源"), allow_blank=True)
        data_type_label = serializers.CharField(default="", label=_("指标数据类型"), allow_blank=True)
        result_table_label = serializers.CharField(default="", label=_("对象类型"), allow_blank=True)
        tag = serializers.CharField(default="", label=_("标签"), allow_blank=True)

        search_fields = serializers.DictField(default=lambda: {}, label=_("查询字段"))
        is_exact_match = serializers.BooleanField(default=False, label=_("是否精确匹配"))
        search_value = serializers.CharField(default="", label=_("查询关键字"), allow_blank=True)
        page = serializers.IntegerField(required=False, label=_("页码"))
        page_size = serializers.IntegerField(required=False, label=_("每页数目"))
    ...
    def get_metric_remarks(metric: dict) -> list:
        ...
        if metric["metric_field"] == "disk-full-gse":
            ...
            return [
                ...
                _("依赖exceptionbeat采集器, 在节点管理安装,会自动根据core_pattern监听文件目录"),
            ]
            ...
            return [
                _(
                    "gse每隔60秒检查一次agent心跳数据。"
                    "gse从agent最后一次心跳数据更新开始，第5分钟、第10分钟、第12小时各触发一次告警事件。"
                    "从第24小时起，如果心跳依然未更新则重复上述第5分钟、第10分钟、第12小时各触发一次告警事件的过程。"
                )
            ]
        elif metric["metric_field"] == "oom-gse":
            return [
                _("通过调用内核syslog接口获取系统日志，对ut of memory:关键字匹配告警，应用进程触发的OOM告警"),
                _("通过对/proc/vmstat的oom_kill计数器进行判断告警，如递增则判断产生OOM告警，操作系统触发的OOM告警"),
            ]
        elif metric["metric_field"] == "os_restart":
            return [
                _("依赖basereport采集器的安装 ，在节点管理进行安装"),
                _("检测原理：通过最近2次的uptime数据对比，满足cur_uptime < pre_uptime，则判断为重启"),
            ]
        elif metric["metric_field"] == "ping-gse":
            return [
                _("依赖bkmonitorproxy采集器的安装，直连区域请联系管理员进行安装，非直连区域自动安装"),
                _("由监控后台bkmonitorproxy去探测目标IP是否存活"),
            ]
        elif metric["metric_field"] == "proc_port":
            return [_("依赖processbeat采集器的安装，在节点管理进行安装"), _("对CMDB中的进程端口存活状态判断，如不满足预定义数据状态，则产生告警")]
    ...
    def get_metric_list(cls, metrics: QuerySet, params):
        ...
        for metric in metrics:
            ...
            # 默认触发条件
            if data["data_type_label"] == DataTypeLabel.EVENT:
                data["default_trigger_config"] = (
                    DEFAULT_TRIGGER_CONFIG_MAP.get(data["result_table_label"], {})
                    .get(data["data_type_label"], {})
                    .get(data["metric_field"], GLOBAL_TRIGGER_CONFIG)
                )
            else:
                data["default_trigger_config"] = (
                    DEFAULT_TRIGGER_CONFIG_MAP.get(data["result_table_label"], {})
                    .get(data["data_type_label"], {})
                    .get(data["result_table_id"], {})
                    .get(data["metric_field"], GLOBAL_TRIGGER_CONFIG)
                )
    ...
    def perform_request(self, params):
        ...
        metrics = self.search_filter(metrics, params)

        result["tag_list"] = self.get_tag_list(metrics, params)
        result["count_list"] = self.get_count_list(metrics)

        metrics = self.category_filter(metrics, params)
        metrics = self.tag_filter(metrics, params)
        metrics = self.page_filter(metrics, params)

        result["metric_list"] = self.get_metric_list(metrics, params)
        return result
    ...
    def perform_request(self, params):
        ...
        if params["result_table_id"] == SYSTEM_EVENT_RT_TABLE_ID:
            ...
            if params["metric_field"] == "corefile-gse" and params["field"] == "signal":
                return [{"id": item, "name": item} for item in CORE_FILE_SIGNAL_LIST]
            elif params["metric_field"] in EVENT_QUERY_CONFIG_MAP:
                params.update(EVENT_QUERY_CONFIG_MAP[params["metric_field"]])
            else:
                return []

        if params["result_table_id"] == "uptimecheck.http":
            params["metric_field"] = "available"

        # 反查指标的数据源
        result_table_id_backend = str(params["bk_biz_id"]) + "_" + params["result_table_id"]
        metric: MetricListCache = MetricListCache.objects.filter(
            result_table_id__in=[params["result_table_id"], result_table_id_backend],
            bk_biz_id__in=[0, params["bk_biz_id"]],
            metric_field=params["metric_field"],
        ).first()
        if not metric or metric.data_type_label in [DataTypeLabel.EVENT, DataTypeLabel.LOG]:
            return []
        ...
        values = data_source.query_dimensions(dimension_field=params["field"], start_time=start_time)

        names = {}
        # 获取采集配置名称
        if params["field"] == "bk_collect_config_id":
            collect_configs = CollectConfigMeta.objects.filter(id__in=values).values("id", "name")
            for config in collect_configs:
                names[str(config["id"])] = config["name"]

        # 获取拨测任务任务名
        if params["field"] == "task_id":
            task_list = UptimeCheckTask.objects.filter(id__in=values).values("id", "name")
            result = []
        ...
        tips = ""
        if len(result) == TS_MAX_SLIMIT:
            tips += f"[{params['field']}]查询条件的值过多，为了避免查询问题只显示{TS_MAX_SLIMIT}个。"

        if tips:
            result = {"data": result, "message": "", "tips": tips, "result": True, "code": 200}
...
class StrategyConfigListResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        service_category = serializers.CharField(required=False, label=_("服务分类"), allow_blank=True)
        ...
        bk_event_group_id = serializers.IntegerField(required=False, label=_("事件分组ID"))
        data_source_list = serializers.ListField(required=False, label=_("数据来源列表"))
        conditions = serializers.ListField(label=_("搜索条件"), required=False)
        only_using = serializers.IntegerField(required=False, label=_("仅统计在用的类型"), default=0)

        def validate_data_source_list(self, value):
            try:
                return json.loads(value)
            except Exception as e:
                logger.exception(_("data_source_list参数错误, %s") % e)
                return []
...
class StrategyConfigDetailResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        id = serializers.IntegerField(required=True, label=_("策略ID"))
...
class DeleteStrategyConfigResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        ids = serializers.ListField(required=False, label=_("策略ID列表"))
...
class StrategyConfigResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        class ItemListSerializer(serializers.Serializer):
            class DetectAlgorithmSerializer(serializers.Serializer):
                class AlgorithmSerializer(serializers.Serializer):
                    algorithm_config = serializers.JSONField(required=True, label=_("检测算法配置"))
                    algorithm_type = serializers.ChoiceField(
                        required=True, choices=DETECT_ALGORITHM_CHOICES, label=_("检测算法")
                    )
                    algorithm_unit = serializers.CharField(
                        required=False, allow_blank=True, default="", label=_("算法单位")
                    )
                ...
                algorithm_list = AlgorithmSerializer(many=True, label=_("检测算法列表"))
            ...
            metric_field = serializers.CharField(required=True, label=_("监控指标别名"))
            ...
            detect_algorithm_list = DetectAlgorithmSerializer(required=True, many=True, label=_("检测算法列表"))
            trigger_config = serializers.DictField(default={}, label=_("告警触发配置"))
            recovery_config = serializers.DictField(default={}, label=_("告警恢复配置"))
            rule = serializers.CharField(required=False, allow_blank=True, label=_("组合方式"))
            extend_fields = serializers.DictField(default={}, label=_("扩展字段"))
        ...
        class ActionListSerializer(serializers.Serializer):
            ...
            notice_group_list = serializers.ListField(required=False, label=_("通知组ID列表"))
        ...
        item_list = ItemListSerializer(required=True, many=True, label=_("监控算法配置"))
        no_data_config = serializers.DictField(required=True, label=_("无数据告警配置"))
        message_template = serializers.CharField(required=False, allow_blank=True, label=_("告警模板"))
        action_list = ActionListSerializer(required=True, many=True, label=_("触发动作"))
        id = serializers.IntegerField(required=False, label=_("策略ID"))
        source_type = serializers.CharField(required=True, label=_("策略配置来源"))
...
class BulkEditStrategyResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        id_list = serializers.ListField(required=True, label=_("批量修改的策略ID列表"))
        edit_data = serializers.DictField(required=True, label=_("批量修改的值"))

        def validate_edit_data(self, value):
            if value.get("target"):
                is_validate_target(value.get("target"))
                value["target"] = handel_target(value.get("target"))
            return value
...
class GetDimensionListResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        data_source_label = serializers.CharField(required=False, allow_blank=True, label=_("数据来源"))
        data_type_label = serializers.CharField(required=False, allow_blank=True, label=_("数据类型"))
        result_table_id = serializers.CharField(required=False, allow_blank=True, label=_("表名"))
        custom_event_id = serializers.IntegerField(required=False, label=_("自定义事件ID"))
        bk_event_group_id = serializers.IntegerField(required=False, label=_("自定义事件组ID"))
        show_all = serializers.BooleanField(default=False, label=_("是否展示全部维度"))
    ...
    def perform_request(self, validated_request_data):
        ...
        elif data_source_label == DataSourceLabel.CUSTOM and data_type_label == DataTypeLabel.EVENT:
            # 获取自定义字段的维度信息
...
class StrategyInfo(StrategyConfigDetailResource):
    ...
    def perform_request(self, params):
        ...
        for item in strategy_config["item_list"]:
            ...
            for condition in item["agg_condition"]:
...
class GetIndexSetListResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        index_set_id = serializers.IntegerField(default=-1, label=_("查询索引集"))
    ...
    def index_filter(self, data):
        ...
        for index_set in data:
            ...
            for index in index_set.get("indices", []):
                ...
                time_field = index.get("time_field")
...
class GetLogFields(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        index_set_id = serializers.IntegerField(required=True, label=_("索引集ID"))
...
class RenderNoticeTemplate(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        scenario = serializers.CharField(required=True, label=_("场景"))
        template = serializers.CharField(required=True, label=_("消息模板"), allow_blank=True)

    def perform_request(self, validated_request_data):
        template_path_template = "/notice/anomaly/{}/{}_content.jinja"
        ...
        multi_content = defaultdict(
            ...
            },
            {
                ...
                "sms": {
                    ...
                    "target_type": "",
                    "data_source": "",
                    "content": _("比如: [致命]磁盘使用率 10:10告警,已持续10分钟,sum(in_user) > 10,当前值 15%"),
                    "biz": "",
                    "target": _("目标: 蓝鲸[2] 127.0.0.1,127.0.0.2 (2)"),
                    "dimension": _("维度: 磁盘=C"),
                    "detail": _("告警ID: 12345"),
        ...
        for notice_way in notice_ways:
            ...
            try:
                single = AlarmNoticeTemplate(template_path_template.format("DIMENSION", notice_way["type"]))
            ...
            try:
                multi = AlarmNoticeTemplate(template_path_template.format("MULTI_STRATEGY", notice_way["type"]))
...
class StrategyLabelResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=False, default=0, label=_("业务ID"))
        strategy_id = serializers.IntegerField(required=False, default=0, label=_("策略ID"))
        id = serializers.CharField(required=False, label=_("标签ID"), default=None)
        label_name = serializers.CharField(required=True, label=_("标签名"))
        ...
        def validate(self, attrs):
            ...
            if StrategyLabel.objects.filter(
                ...
                raise ValidationError(f"标签{label_name}已存在")
            return attrs
    ...
    def perform_request(self, validated_request_data):
        ...
        bk_biz_id = validated_request_data["bk_biz_id"]
        # if bk_biz_id == strategy_id == 0:
        # 全局标签
        return self.edit_label(label_name, label_id, strategy_id=strategy_id, bk_biz_id=bk_biz_id)
        # 策略标签
        # 向上层级覆盖
        # global_label = self.get_global_label(label_name)
        # if global_label:
        #     obj = StrategyLabel.objects.create(label_name=label_name, bk_biz_id=bk_biz_id, strategy_id=strategy_id)
        #     return obj.id
        # else:
        #     return self.edit_label(label_name, label_id, strategy_id=strategy_id, bk_biz_id=bk_biz_id)
...
class DeleteStrategyLabelResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=False, default=0, label=_("业务ID"))
        strategy_id = serializers.IntegerField(required=False, default=0, label=_("策略ID"))
        label_name = serializers.CharField(required=False, label=_("标签名"), default="")
        ...
        def validate(self, attrs):
            if attrs["bk_biz_id"] == attrs["strategy_id"] == 0:
                if not attrs["label_name"]:
                    raise ValidationError("参数缺少label_name")
            return attrs
...
class FetchItemStatus(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        metric_ids = serializers.ListField(required=True, label=_("指标ID"))
    ...
    def perform_request(self, validated_request_data):
        ...
        # 获得业务下的Item
        query_configs = QueryConfigModel.objects.filter(metric_id__in=validated_request_data["metric_ids"])
        ...
        alert_status = {}
        for query_config in query_configs:
            if query_config.strategy_id in strategy_alert_num:
                alert_status[query_config.metric_id] = {
                    "status": 2,
                    "alert_number": strategy_alert_num[query_config.strategy_id],
                    "strategy_number": len(strategy_numbers[query_config.metric_id]),
                }
            else:
                alert_status[query_config.metric_id] = {
                    "status": 1,
