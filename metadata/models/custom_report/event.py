...
class EventGroup(CustomGroupBase):
    ...
    event_group_id = models.AutoField(verbose_name=_("分组ID"), primary_key=True)
    event_group_name = models.CharField(verbose_name=_("事件分组名"), max_length=255)
    ...
    def update_event_dimensions_from_es(self):
        """
        从ES更新事件及维度信息等内容
        ...
        for event_info in result:
            try:
                ...
                logger.error(
                    "failed to update event_group->[{}] for event->[{}] for failed to query "
                    "event from es.".format(self.event_group_name, event_info["key"])
                )
                continue
    ...
    def create_event_group(
        cls, bk_data_id, bk_biz_id, event_group_name, label, operator, event_info_list=None, table_id=None
    ):
        """
        创建一个新的自定义分组记录
    ...
    @atomic(config.DATABASE_CONNECTION_NAME)
    def modify_event_group(self, operator, event_group_name=None, label=None, is_enable=None, event_info_list=None):
        """
        修改一个事件组
        :param operator: 操作者
...
class Event(models.Model):
    ...
    event_id = models.AutoField(verbose_name=_("事件ID"), primary_key=True)
    event_group_id = models.IntegerField(verbose_name=_("事件所属分组ID"))
    event_name = models.CharField(verbose_name=_("事件名称"), max_length=255)
    dimension_list = JsonField(verbose_name=_("维度列表"), default=[])
    ...
    class Meta:
        ...
        verbose_name = _("事件描述记录")
        verbose_name_plural = _("事件描述记录表")
