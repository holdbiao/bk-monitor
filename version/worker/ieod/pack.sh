...
find "${CURRENT_DIR}"/tmp/conf -name '*.py' -a ! -name '*init*' -a ! -name '*default_settings*' -a ! -name '*'${PLATFORM}'*' -delete
...
rm -rf "${CURRENT_DIR}"/tmp/packages/account/components/bk_token
...
sed -i "/BKAPP_DEPLOY_PLATFORM/s/tencent/${PLATFORM}/g" "${CURRENT_DIR}"/tmp/settings.py
