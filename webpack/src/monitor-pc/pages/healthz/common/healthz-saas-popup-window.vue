<template>
  ...
  <el-dialog :title="tips.name + $t('接口列表')" :visible.sync="isShow" @closed="closeDialog"
             :before-close="handleClose">
    <div>
      <el-button type="primary" :loading="saasDataToTest.loading" size="mini" @click="runTest">
        {{ msg }}
      </el-button>
      <el-tooltip placement="top" :content="$t('测试进行时，请勿关闭弹窗！')">
        <i class="el-icon-warning"></i>
      </el-tooltip>

      <el-card class="box-card" style="margin-top: 10px; overflow: scroll; height: 200px">
        <el-tree :data="saasDataToTest.data" :props="treeProps" default-expand-all>
          <span class="custom-tree-node" slot-scope="{ node, data }">
            <i class="el-icon-loading" v-if="data.loading"></i>
            <span :class="data.class">{{ node.label }}</span>
            <el-tooltip placement="top">
              <div slot="content"> {{ $t('接口描述：') }} {{ data.description }}
                <br /> {{ $t('测试参数：') }} {{ data.args }}
                <br /> {{ $t('测试结果：') }} {{ data.result }}
                <br />
                <!--错误信息，只有请求出错时渲染-->
...
import { Tooltip, Button, Card, Tree, Dialog } from 'element-ui'
export default {
  ...
  methods: {
    ...
    runTest() {
      ...
      for (let i = 0; i < tmp.data.length; i++) {
        // 检查是否存在 loading 属性
        if (Object.prototype.hasOwnProperty.call(tmp.data[i], 'loading')
              ...
              apiTask.then(function (res) {
                ...
                if (res.result && res.data.status) {
                    ...
                    const childTmpTask = new Promise(((resolve) => {
                      ...
                      if (childApiTest) {
                        ...
                        childApiTestTask.then((childRes) => {
                          ...
                          const childApiIndex = self.findIndexOf(childRes.data.api_name, self.saasDataToTest.data[parentApiIndex].children)
                          // 更新子接口的loading状态
                          ...
                          self.saasDataToTest.data[parentApiIndex].children[childApiIndex].result = childRes.data.result
                  ...
                  for (let childIndex = 0; childIndex < childrenList.length; childIndex++) {
                    ...
                    self.saasDataToTest.data[j].children[childIndex].message = this.$t('父接口测试失败')
