...
class ScaledUnits(object):
    ...
    def unit_series(self):
        ...
        return [
            {
                "unit_conversion": self.factor ** (idx - self.suffix_idx),
    ...
    def convert(
        self, value: Union[int, float], target_suffix: str, current_suffix: str = None, decimal: Optional[int] = None
    ) -> Union[int, float]:
        """
        转换为指定单位
        :param value: 数值
        ...
        if current_suffix_id >= 0 and target_suffix_id >= 0:
            value /= self.factor ** (target_suffix_id - current_suffix_id)
    ...
    def convert_to_max(self, value: Union[int, float], suffix: str = None):
        """
        转换为最大值
        """
        ...
        return self.convert(value, target_suffix=self.suffix_list[0], current_suffix=suffix, decimal=5), suffix
