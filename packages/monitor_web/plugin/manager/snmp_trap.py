...
from monitor_web.plugin.constant import DEFAULT_TRAP_CONFIG, DEFAULT_TRAP_V3_CONFIG, PluginType
...
class SNMPTrapPluginManager(LogPluginManager):
    ...
    def get_deploy_steps_params(self, plugin_version, param, target_nodes):
        oids_list = []
        for item in list(yaml.load(param["snmp_trap"]["yaml"]["value"]).values()):
            oids_list.extend(item["metrics"])
        collector_params = {
            "tasks": [
                {
