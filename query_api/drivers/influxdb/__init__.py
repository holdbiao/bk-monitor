...
class InfluxDBDriver(DriverProxy):
    ...
    def _process_result_table(self):
        ...
        if "biz_id" in match_info:
            if table_name in CUSTOM_RT_MAP:
                is_custom_series_rt = CUSTOM_RT_MAP[table_name]
            else:
                is_custom_series_rt = TimeSeriesGroup.objects.filter(table_id=self.__rt_instance.table_id).exists()
                CUSTOM_RT_MAP[table_name] = is_custom_series_rt
        ...
        if self.__rt_instance.schema_type == ResultTable.SCHEMA_TYPE_FREE:
            # 动态字段解析
            if "time_series" not in self.db_name and not ResultTable.is_disable_metric_cutter(
                self.__rt_instance.table_id
            ):
    ...
    def _process_where_condition(self, where_token=None):
        ...
        tidx = -1
        while True:
            tidx, token = where_token.token_next_by(m=(T.Keyword, "LIKE"), idx=tidx)
            if token:
                _, like_value_token = where_token.token_next(tidx)
                if like_value_token:
                    self._process_like_value(like_value_token)

                idx, not_token = where_token.token_prev(tidx)
                if not_token.is_keyword and not_token.normalized == "NOT":
                    like_token_idx_list.append((idx, tidx))
                else:
                    like_token_idx_list.append((tidx, tidx))
            else:
                break
        if like_token_idx_list:
            self._process_like_key(like_token_idx_list, where_token)

    def _process_like_key(self, like_token_idx_list, where_token=None):
        where_token = where_token or self.q.where_token
        # traverse from the back to the front
        for sidx, eidx in reversed(like_token_idx_list):
            if sidx != eidx:
                # not like
                like_token = S.Token(T.Token.Operator.Comparison, "!~")
            else:
                # like
                like_token = S.Token(T.Token.Operator.Comparison, "=~")
            # fmt: off
            self._replace_token(where_token.tokens[sidx: eidx + 1], like_token)
            # fmt: on
    ...
    def _process_limit(self):
        ...
        limit_value = min(limit_nums[-1], MAX_LIMIT)
