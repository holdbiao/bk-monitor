...
from alarm_backends.core.storage.kafka import KafkaQueue, KafkaOffsetManager
from alarm_backends.core.storage.redis import Cache
from bkmonitor.models import StrategyModel
from bkmonitor.utils.common_utils import get_local_ip
from metadata.models import DataSource
from core.drf_resource import api
from alarm_backends.management.story.base import BaseStory, CheckStep, register_step, register_story, ResolvedProblem
...
class DetectServiceQueueCheck(CheckStep):
    name = "check detect service signal queue"
...
class PollEventDelayCheck(CheckStep):
    ...
    def check(self):
        topic_info = api.metadata.get_data_id(bk_data_id=settings.GSE_BASE_ALARM_DATAID)
        ...
        if local_offset + 10000 <= last_remote_offset:
            ...
            problem = Kafka1000Delay(
                f"GSE EVENT拉取存在处理瓶颈，topic[{topic}]已堵塞{last_remote_offset-local_offset}条消息", self.story, topic=topic
            )
        else:
...
class DurationSpace(CheckStep):
    ...
    def check(self):
        start = time.time()
        sql = """select last(usage) from system.cpu_summary where time>'10m' order by time desc limit 1"""
        try:
