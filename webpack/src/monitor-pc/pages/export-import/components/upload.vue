  ...
  <article class="config-upload" :class="{ 'content-hover': beforeUpload, 'isdrag': isdrag }">
    ...
    <section class="config-upload-content" v-if="beforeUpload">
      <slot name="upload-content" v-bind="{ acceptTips }">
        <span class="content-icon"><i class="icon-monitor icon-upload-cloud"></i></span>
    ...
    <section class="config-upload-file" v-else>
      <slot name="upload-file" v-bind="{ beforeAbortUpload, file }">
        <span class="file-abort"><i class="icon-monitor icon-mc-close" @click="beforeAbortUpload"></i></span>
        <div class="file-info">
          <span class="info-icon"><i class="icon-monitor icon-mc-file"></i></span>
    ...
    <section class="config-upload-footer">
      <slot name="upload-footer" v-bind="{ importConfigExplain }">
        <div class="footer-explain">
          <!-- <span> {{ $t('标准的配置格式详见') }} </span>
                    <span @click="importConfigExplain" class="footer-explain-button"> {{ $t('配置格式说明') }} </span> -->
        </div>
      </slot>
    </section>
  </article>
...
export default {
  ...
  props: {
    ...
    acceptTips: {
      type: String,
      default() {
        return `${this.$t('仅支持导入')}".gz .tgz .bz2 .tbz2" ${this.$t('gzip或bzip2压缩')} ${this.$t('格式文件')}`
      }
    },
    // URL
    action: {
