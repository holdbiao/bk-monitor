...
from core.errors.custom_report import CustomEventNameValidationError
...
class TestResource(TestCase):
    ...
    def test_validate_custom_event_group_name_1(self, query_event_group):
        ...
        with pytest.raises(CustomEventNameValidationError):
...
class TestTSResource(TestCase):
    ...
    @mock.patch.object(APIResource, "perform_request")
    def test_create_custom_ts(self, mock_api):
        post_data = {"bk_biz_id": 2, "name": "test_ts", "scenario": "uptimecheck"}
        mock_api.side_effect = [{"bk_data_id": 123}, TS_GROUP_RETURN_VALUE]
        path = "/rest/v2/custom_report/create_custom_time_series/"
        response = self.client.post(path=path, data=json.dumps(post_data), content_type="application/json")
        content = json.loads(response.content)
        self.assertEqual(content["data"]["time_series_group_id"], 1002)
