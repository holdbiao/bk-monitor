    ...
    def extra_context(self, context):
        ...
        if floor_history_data_points:
            env["floor_history_value"] = round(
                sum([abs(p.value) for p in floor_history_data_points]) * 1.0 / len(floor_history_data_points), 2
            )
        ...
        if ceil_history_data_points:
            env["ceil_history_value"] = round(
                sum([abs(p.value) for p in ceil_history_data_points]) * 1.0 / len(ceil_history_data_points), 2
            )
