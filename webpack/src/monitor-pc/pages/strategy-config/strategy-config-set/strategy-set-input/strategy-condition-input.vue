<template>
  <div class="strategy-set-input">
    <component v-for="(item,index) in alarmConditionList"
...
import { random, deepClone } from '../../../../../monitor-common/utils/utils'
...
export default {
  ...
  computed: {
    ...
    getConditions() { // 兼容后端返回number类型,导致无法正确展示条件名称
      ...
      if (item.value.id !== data.id || item.value.name !== data.name) {
        ...
        if (item.type === 'key') {
          ...
          if (this.dataTypeLabel !== 'log') {
            ...
            if (!(preValueIndex > 0 && prevValue?.type === 'value')) { // 将method恢复对应的维度默认值
        ...
        this.alarmConditionList.forEach((item) => {
          ...
          if (['key', 'method', 'value', 'condition'].includes(type)) {
            if (type === 'condition') {
              ...
              data[type] = data[type] || (Array.isArray(item.value)
