<template>
  <article class="import-config" v-bkloading="{ isLoading: loading }">
    <!--按钮组（统计信息）-->
    <section class="import-config-tag">
        ...
        <bk-collapse-item
          ...
          <template #default>
            ...
            <div class="collapse-item-right" v-show="table.statistics && table.statistics[item.name]">
              ...
              <span v-html="statusHtml(item.name, tag.active)"></span>
            </div>
          </template>
          <!--折叠表格-->
          <template #content>
            <bk-table
              ...
              <bk-table-column :label="$t('任务状态')" width="153">
                ...
                <template #default="{ row }">
                  <div class="detail-col">
                    ...
                    <bk-button
                      ...
                      v-show="row.status === 'failed' && item.name !== 'plugin'"
                      text
    ...
    <!--空数据-->
    <section class="import-config-empty" v-show="isShowEmpty">
      <content-empty :title="$t('无数据')" :sub-title="$t('筛选数据为空')"></content-empty>
    </section>
    <!--底部按钮-->
    <section class="import-config-footer">
      ...
      <div class="button1-wrap"
        ...
        <bk-button
          ...
          :disabled="disabledConfirmBtn"> {{ $t('前往添加统一监控目标') }} </bk-button>
      </div>
      <bk-button
        class="button-cancel"
        :class="{ 'footer-button2': isScroll }"
        :style="{ marginLeft: isScroll ? '182px' : '' }"
        @click="handleImportCancel"> {{ $t('暂不添加') }} </bk-button>
    </section>
  </article>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    disabledBatchRetryBtn() {
      return this.table.runingQueue.length !== 0
            || this.table.list.filter(item => item.status === 'failed' && item.type !== 'plugin').length === 0
    },
    // 统一添加监控目标按钮禁用状态
    disabledConfirmBtn() {
    ...
    countData() {
      return (collapse) => {
        ...
        return `${collapse.title}（${this.$t('共')} ${checkedCount} ${this.$t('个')}）`
      }
    },
    // 是否显示空界面
    isShowEmpty() {
  ...
  methods: {
    ...
    handleDetectionTarget() {
      const hasRepeat = {}
      this.isRepeat = this.table.list.filter(item => item.type !== 'plugin' && item.status === 'success')
        .every((item) => {
    ...
    handlePushRuningTask() {
      const uuids = this.table.list.filter((item) => {
    ...
    async handleBatchRetry() {
      ...
      const uuids = this.table.list.filter(item => item.status === 'failed' && item.type !== 'plugin').map((item) => {
