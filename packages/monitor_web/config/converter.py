...
def convert_field(config):
    ...
    try:
        ...
        logger.exception(_("全局配置 [{}] 加载异常: {}").format(config.key, e))
...
class BaseFieldConverter(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def get_form_item_props(self):
        return {
            "label": _(self.serializer.label or self.config.description or ""),
            "required": self.serializer.required,
            "property": self.config.key,
            "help_text": _(self.serializer.help_text or ""),
        }

    def get_form_child_props(self):
        return {
            "placeholder": _(self.serializer.help_text or ""),
        }
