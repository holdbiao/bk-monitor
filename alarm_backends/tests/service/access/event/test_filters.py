...
from alarm_backends.tests.service.access.data.config import EVENT_RAW_DATA, EVENT_STRATEGY_CONFIG, CORE_FILE_RAW_DATA
...
class TestConditionFilter(object):
    def test_filter(self, mocker):
        ...
        strategy_id = 1
        strategy = Strategy(strategy_id)
        strategy.items[0].rt_query_config["agg_condition"] = [
            {"key": "corefile", "method": "include", "value": ["/data/bkee"]}
        ]
        strategy.config["item_list"][0]["metric_id"] = "bk_monitor.corefile-gse"
        strategies = {2: {1: strategy}}
