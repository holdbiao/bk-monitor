...
export default class FilterPanel extends tsc<FilterPanelProps, FilterPanelEvents, FilterPanelScopedSlots> {
  ...
  // 默认展开项
  @Prop({ default: () => ['data_source_list', 'scenario'], type: Array }) defaultActiveName!: string[]
  // 勾选节点的数据
  ...
  render(): VNode {
    return (
      <transition name="slide">
        <section class="filter-panel" v-show={this.show}>
  ...
  collapseItemContentSlot(item: IGroupData) {
    ...
    return (
      <BigTree data={data}
        ...
        scopedSlots={{
          default: ({ data }) => (<span class="check-label-content">
            <span class="label-text" title={data.name}>{data.name}</span>
            <span>{data.count || 0}</span></span>)
        }}
        on-check-change={(id, node) => this.handleTreeCheckChange(id, node, item)}></BigTree>
    )
  }
