                ...
                if key not in performance_point_info and point[item] is not None:
                    ...
                    with ignored(TypeError, log_exception=False):
                        if isinstance(val, int) and int(conversion) == 1:
                            ...
                            val = round(val, 5) if val < 0.05 else round(val, 2)
...
class TSDBData(DataBase):
    ...
    @classmethod
    def get_data(
        cls, table_name, select_field, group_by_field=None, filter_dict=None, limit=50000, order_by_field=None
    ):
        if order_by_field:
        ...
        if "bk_cloud_id" in group_by_field:
            for d in data:
                d["bk_cloud_id"] = resource.cc.plat_id_gse_to_cc(cls.get_plat_id(d))
        return data
...
class TSDataBase(object):
    ...
    def get_data(self, table_name, select_field, group_by_field=None, filter_dict=None, limit=50000):
