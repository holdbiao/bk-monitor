...
class SQLCompiler(compiler.SQLCompiler):
    ...
    TIME_AGG_FIELD_RE = re.compile(r"minute(?P<minute>\d+)")
    ...
    def _get_dimensions(self):
        ...
        for idx, dim in enumerate(dimensions):
            time_agg_field = self.TIME_AGG_FIELD_RE.match(dim)
            if time_agg_field:
                minute = time_agg_field.groupdict().get("minute") or 1
                dimensions.pop(idx)
                break
        return minute, dimensions
    ...
    def _get_aggregations(self, agg_interval, dimensions, agg_method, metric_field, metric_alias):
        ...
        aggragations = {
            time_field: {
                "date_histogram": {"field": time_field, "interval": "%sm" % agg_interval},
