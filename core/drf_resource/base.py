...
from core.drf_resource.tools import format_serializer_errors, get_serializer_fields, render_schema
...
class Resource(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def validate_request_data(self, request_data):
        ...
        if isinstance(request_data, models.Model):
            ...
            if not is_valid_request:
                logger.error(
                    _("Resource[{}] 请求参数格式错误：%s").format(self.get_resource_name()),
    ...
    def validate_response_data(self, response_data):
        ...
        # model类型的数据需要特殊处理
        if isinstance(response_data, models.Model):
