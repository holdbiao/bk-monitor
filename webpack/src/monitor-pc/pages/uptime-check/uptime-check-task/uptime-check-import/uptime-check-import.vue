<template>
  <div class="uptime-check-import">
    <bk-sideslider :title="$t('批量导入')"
                   :width="640"
                   :is-show.sync="options.isShow"
                   @hidden="handleHidden">
      <div class="import-content" slot="content">
        <div class="import-content-body">
          <div class="file-container">
            <div :class="['file-wrapper', { 'is-drap': isDrap }]" v-show="status === 'import'">
              <i class="icon-monitor icon-upload-cloud upload-icon"></i>
              <div class="upload-text">
                <span class="drop-upload"> {{ $t('拖拽到此处上传或') }} </span>
            ...
            <div class="file-result" v-show="status === 'uploading' || status === 'parsing'">
              <div class="icon-monitor icon-xls xlsx-icon"></div>
              <div class="file-name">{{file.name}}</div>
              <div class="upload-process" v-show="status === 'uploading'">
                <div class="process-done" :style="{ width: uploadProcess + '%' }"></div>
              </div>
              <div class="file-size" v-show="status === 'parsing'">{{file.size|fileSizeFormat}}</div>
              <i class="bk-icon icon-close" @click="deleteFile"></i>
            </div>
            <div class="file-desc" v-show="status === 'import' || status === 'uploading'">
              <div class="file-text"> {{ $t('只支持“xlsx”格式的文件') }} </div>
              <div class="file-download">
                <div class="download-text">{{ $t('模板下载') }}</div>
                <bk-select :value="'模板下载'" style="width: 150px;" :clearable="false" @change="handleDloadTem">
                  <bk-option v-for="option in downloadList"
                             :key="option.id"
                             :id="option.id"
                             :name="option.name">
                  </bk-option>
            ...
            <div class="file-parsing" v-show="status === 'parsing'">
              <div class="parsing-loading" v-show="parsingRes === 'parsing'">
                ...
                <span class="parsing-text">{{ $t('正在解析文件') }}...</span>
              </div>
              <div class="parsing-result" v-show="parsingRes === 'error'">
                <i class="bk-icon icon-exclamation-circle-shape icon-error"></i>
                <span class="error-text"> {{ $t('文件已损坏') }} </span>
              </div>
            </div>
          </div>
          <div class="data-preview" v-show="taskList.length > 0">
            <div class="preview-title"> {{ $t('数据预览') }} </div>
            <div class="preview-content">
              <bk-table
                class="task-table"
                :data="taskList">
                <bk-table-column :label="$t('任务名')" prop="name" show-overflow-tooltip></bk-table-column>
                <bk-table-column :label="$t('类型')" prop="protocol"></bk-table-column>
                <bk-table-column :label="$t('创建状态')">
                  <template slot-scope="scope">
                    <bk-popover v-if="scope.row.status === 'fail'">
                      ...
                      <div slot="content">
                        <pre class="hint-content">{{scope.row.errMsg}}</pre>
                      </div>
                    </bk-popover>
        ...
        <div class="import-content-footer">
          <bk-button class="button-complete" theme="primary" :disabled="!canComplete" @click="complete"> {{ $t('完成') }} </bk-button>
...
import { fileParse, fileImportUptimeCheck } from '../../../../../monitor-api/modules/uptime_check'
export default {
  ...
  methods: {
    ...
    complete() {
      ...
      this.$bkMessage({
        theme: 'success',
        message: `${this.$t('恭喜你，成功导入 ')}${res}${this.$t('个')}${this.$t('任务')}`
      })
      this.close()
      this.$emit('complete')
    },
    ...
    validateFile(file) {
      ...
      if (file.size > this.maxFileSize * 1024) {
        valid.success = false
        valid.message = `${this.$t('文件不能超过')}${this.maxFileSize}${this.$t('MB')}`
      }
    ...
    requestRules(type) {
      return new Promise(resolve => fileParse({ protocol: type }, { needRes: true }).then((res) => {
        if (res.result) {
    ...
    getExcelRowData(rules, row) {
      const data = {}
      rules.forEach((item) => {
        if (item.is_dict && row[item.cnkey]) {
          data[item.enkey] = JSON.parse(row[item.cnkey])
        } else if (item.default && !row[item.cnkey]) {
    ...
    validateExcelRowData(rules, row) {
      ...
      for (const item of rules) {
        if (item.required) { // 必填项校验
          if (!row[item.enkey]) {
            row.status = 'fail'
            row.errMsg = `${this.$t('字段')}”${item.cnkey}“${this.$t('必填不能为空')}`
            break
          }
          if (item.regex && !new RegExp(item.regex).test(row[item.enkey])) { // 正则校验
            row.status = 'fail'
            row.errMsg = `${this.$t('字段')}“${item.cnkey}“${this.$t('正则匹配错误，正则为：')}${item.regex}`
            break
          }
        } else {
          if (item.regex && !new RegExp(item.regex).test(row[item.enkey])) { // 正则校验
