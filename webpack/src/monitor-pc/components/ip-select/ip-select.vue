<template>
  <div class="ip-select" :style="{ minWidth: minWidth + 'px',height: height + 'px', maxWidth: maxWidth + 'px' }" v-bind="$attrs">
    <!--IP选择器左侧tree区域-->
    <div class="ip-select-left"
      ...
      <div class="left-tab">
        <span
          ...
          <span ref="staticTab"><slot name="left-tab"> {{ $t('静态') }} </slot></span>
        </span>
        <span class="left-tab-item "
          ...
          <span ref="dynamicTab"><slot name="left-tab"> {{ $t('动态') }} </slot></span>
        </span>
      </div>
      <!--静态/动态 tree-->
      ...
      <!--tree搜索-->
      <div class="left-footer" :class="{ 'input-focus': search.focus }" v-show="curActive !== 0 && curActive !== 4">
    ...
    <div class="ip-select-right">
      <slot name="right-wrap">
        <div :key="staticIp.type" class="right-wrap"
          ...
          <right-panel v-model="staticIp.expand" type="staticIp" @change="handleCollapseChange" :title="{ num: staticTopo.tableData.length }">
            <slot name="static-ip-panel"
                  v-bind="{
                    data: staticTableData,
                    deleteClick: handleDeleteStaticIp
                  }">
              <bk-table :data="staticTableData" :empty-text="$t('查询无数据')">
                <bk-table-column prop="ip" label="IP" min-width="210">
                </bk-table-column>
                <bk-table-column prop="agent" :label="$t('状态')">
                </bk-table-column>
                <bk-table-column prop="cloud" :label="$t('云区域')">
                </bk-table-column>
                <bk-table-column :label="$t('操作')" align="center" width="80">
                  <template slot-scope="scope">
                    <bk-button text @click="handleDeleteStaticIp(scope)"> {{ $t('移除') }} </bk-button>
                  </template>
                </bk-table-column>
              </bk-table>
            </slot>
        ...
        <div :key="dynamicTopo.type" class="right-wrap"
          ...
          <right-panel v-model="dynamicTopo.expand" @change="handleCollapseChange" type="dynamicTopo" :title="{ num: dynamicTopo.tableData.length, 'type': $t('节点') }">
            <slot name="dynamic-topo-panel" v-bind="{
              ...
              <ul class="topo-list">
                <li class="topo-list-item" v-for="(item,index) in dynamicTopo.tableData" :key="index">
                  ...
                  <div class="item-desc"> {{ $t('现有主机') }} <span class="status-host">{{item.host}}</span>，<span class="status-unusual">{{item.unusual}}</span> {{ $t('台主机Agent异常') }} </div>
                  <bk-button text class="item-btn" @click="handleDelDynamicTopo(index,item)"> {{ $t('移除') }} </bk-button>
                </li>
              </ul>
            </slot>
          </right-panel>
        ...
        <div :key="staticExtranet.type" class="right-wrap"
          ...
          <right-panel v-model="curComp.expand" @change="handleCollapseChange" type="staticExtranet" :title="{ num: staticExtranet.tableData.length, 'type': $t('外网IP') }">
        ...
        <div key="right-empty" class="right-empty" v-if="isNoData">
          ...
          <div class="right-empty-title"> {{ $t('未选择任何内容') }} </div>
...
export default {
  ...
  props: {
    ...
    defaultEmptyDesc: {
      type: String,
      default() {
        return this.$t('请在左侧选择主机/节点/动态分组')
      }
    },
    inputIpSplit: {
      type: String,
  ...
  data() {
    return {
      ...
      // 搜索框placeholder
      searchPlaceholder: this.$t('请输入IP'),
      left: {
        width: 240
      },
      resizeState: {
  ...
  computed: {
    ...
    isNoData() {
      return !this.staticTableData.length
  ...
  methods: {
    ...
    async handlerCurActiveChange(v, old) {
      if (old !== 4) {
        try {
          this.active === 0
            ? this.searchPlaceholder = this.$t('请输入IP')
            : this.searchPlaceholder = this.$t('搜索节点') // 静态和动态搜索款Placeholder
            ...
            if ((v > 0 && this.curComp.treeData.length === 0) || (v === 0 && !this.curComp.mark)) {
              ...
              if (data) {
                if (v > 0) {
                  ...
                  this.curComp.defaultText = (data.defaultText || '').replace(new RegExp(
                    `\\${this.inputIpSplit}`
                    , 'gm'
                  ), '\n')
                }
