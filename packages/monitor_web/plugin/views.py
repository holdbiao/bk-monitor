...
from monitor_web.models.plugin import CollectorPluginMeta, OperatorSystem, PluginVersionHistory
...
from monitor_web.plugin.serializers import ReleaseSerializer, StartDebugSerializer, TaskIdSerializer
...
class PermissionMixin(IAMPermissionMixin):
    iam_read_actions = ActionEnum.VIEW_PLUGIN
    iam_write_actions = ActionEnum.MANAGE_PLUGIN
...
def assert_manage_public_plugin_permission():
    """
    断言当前用户必须要有管理公共插件的权限
    """
...
class CollectorPluginViewSet(PermissionMixin, viewsets.ModelViewSet):
    ...
    def get_iam_actions(self):
        ...
        if self.request.method not in permissions.SAFE_METHODS and not bk_biz_id:
            ...
            return [ActionEnum.MANAGE_PUBLIC_PLUGIN]
        return super(CollectorPluginViewSet, self).get_iam_actions()
    ...
    def list(self, request, *args, **kwargs):
        ...
        # 获取全量的插件数据（包含外键数据）
        all_versions = PluginVersionHistory.objects.exclude(
            plugin__plugin_type__in=CollectorPluginMeta.VIRTUAL_PLUGIN_TYPE
        ).select_related("plugin", "config", "info")
        if bk_biz_id:
            all_versions = all_versions.filter(plugin__bk_biz_id__in=[0, bk_biz_id])
        else:
            assert_manage_public_plugin_permission()
    ...
    def delete(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            for plugin_id in plugin_ids:
                ...
                if plugin.first():
                    PluginVersionHistory.origin_objects.filter(plugin=plugin).delete()
                    plugin.delete()
                    try:
