...
from core.errors.alarm_backends.detect import InvalidAdvancedRingRatioConfig, InvalidAlgorithmsConfig, InvalidDataPoint
...
class TestAdvancedRingRatio(object):
    ...
    def test_anomaly_message(self):
        ...
        detect_engine = AdvancedRingRatio(config=algorithms_config, unit="percent")
        detect_engine.query_history_points([_datapoint200])
        anomaly_result = detect_engine.detect_records(_datapoint200, 1)
        assert len(anomaly_result) == 1
        assert anomaly_result[0].anomaly_message == "avg(测试指标)较前3个时间点的均值(67.0%)上升超过100.0%, 当前值200%"
        anomaly_result = detect_engine.detect_records(_datapoint_1, 1)
        assert len(anomaly_result) == 1
        assert anomaly_result[0].anomaly_message == "avg(测试指标)较前3个时间点的均值(67.0%)下降超过101.0%, 当前值-1%"
