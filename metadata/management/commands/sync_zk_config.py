...
class Command(BaseCommand):
    ...
    def sync_to_zk(cls):
        ...
        """
        try:
            mq_cluster = models.ClusterInfo.objects.get(
                cluster_type=models.DataSource.DEFAULT_MQ_TYPE, is_default_cluster=True
            )
        except models.ClusterInfo.DoesNotExist:
            msg = (
                "failed to get default MQ for cluster type->[%s],"
                "maybe admin set something wrong?" % models.DataSource.DEFAULT_MQ_TYPE
            )
            print(msg)
            logger.error(msg, exc_info=True)
            return

        mq_info = {"host": mq_cluster.domain_name, "port": mq_cluster.port}
        for zk_host_info in config.ZK_GSE_HOST_INFO_LIST:
            # 1. 确保zk路径存在
            zk_client = KazooClient(hosts=zk_host_info)
            zk_client.start()
            zk_client.ensure_path(config.ZK_GSE_DATA_CLUSTER_PATH)

            # 2. 将配置信息写入到zk中
            zk_config = json.dumps([mq_info]).encode("utf-8")
            zk_client.set(config.ZK_GSE_DATA_CLUSTER_PATH, zk_config)

            message = "[sync_zk_config] write to zk_path->[{}] config->[{}] success.".format(
                config.ZK_GSE_DATA_CLUSTER_PATH, zk_config
            )
            # 日志和标准输出都进行打印
            logger.info(message)
            print(message)
