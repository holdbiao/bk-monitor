...
from alarm_backends.constants import LATEST_NO_DATA_CHECK_POINT, NO_DATA_LEVEL
...
class CheckProcessor(object):
    ...
    def pull_data(self, item, check_timestamp, inputs=None):
        ...
        if total_points == 0:
            logger.info(
                _("[nodata] strategy({}) item({}) check_timestamp({}) 无待检测数据，可能触发无数据告警").format(
        ...
        if records:
            ...
            logger.info(
                _("[nodata] strategy({}) item({}) check_timestamp({}) pull records({})").format(
            ...
            if unexpected_record_count > 0:
                logger.error(
                    (_("[nodata] strategy({}) item({}) check_timestamp({}) 发现非期望格式的待检测数据{}条," "其中之一: {}")).format(
                        self.strategy_id, item.id, check_timestamp, unexpected_record_count, last_unexpected_record
                    )
                )

            logger.info(
                _("[nodata] strategy({}) item({}) check_timestamp({}) 拉取数据({})条").format(
    ...
    def push_data(self):
        ...
        if any(self.inputs.values()):
            logger.info(_("[nodata] strategy({}) 无数据检测完成: 无数据异常记录数({})").format(self.strategy_id, anomaly_count))
        ...
        with service_lock(key.SERVICE_LOCK_NODATA, strategy_id=self.strategy_id):
            ...
            for item in self.strategy.items:
                if item.no_data_config.get("is_enabled"):
                    ...
                    agg_interval = int(item.rt_query_config["agg_interval"])
