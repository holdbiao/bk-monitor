<template>
  <div class="collector-host" ref="addDel">
    <template v-if="taskReady.show">
      <task-ready :task-ready="taskReady.status" :target="target"></task-ready>
    </template>
    <template v-else>
      <div v-if="tables && tables.contents && tables.contents.length">
        <config-deploy :is-running="isRunning" :all-data="tables" @can-polling="handlePolling" @refresh="handleRefreshData"></config-deploy>
      </div>
      <div v-else>
      ...
      <div class="footer">
        <bk-button theme="primary" :disabled="hasRunning" @click="handleDone">{{hasRunning ? $t('执行中') : $t('完成')}}</bk-button>
        <bk-button v-if="showRollBack" :disabled="hasRunning" @click="handleRollback"> {{ $t('回滚') }} </bk-button>
      </div>
    </template>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    operationText() {
      ...
      return text[this.operationType]
    }
  },
  watch: {
    'taskReady.show': {
  ...
  methods: {
    ...
    async init() {
      ...
      this.objType = this.data.objectTypeEn
    ...
    async handleRollback() {
      this.$bkInfo({
        ...
        confirmFn: () => {
          ...
          rollbackDeploymentConfig({ id: this.id }, { needMessage: false }).then(async () => {
    ...
    getHosts(id) {
      return collectTargetStatus({ id, is_auto: this.data.autoStatus }, { needMessage: false }).then((data) => {
        this.tables = this.handleData(data)
        this.hasRunning = data.contents.some(item => item.child.some(set => (set.status === 'RUNNING'
        || set.status === 'PENDING')))
        this.$emit('update:hosts', this.tables)
      })
        .catch((err) => {
