...
from monitor_web.export_import.constant import DIRECTORY_LIST, ConfigType, ImportDetailStatus, ImportHistoryStatus
from monitor_web.export_import.parse_config import CollectConfigParse, StrategyConfigParse
...
from monitor_web.strategies.serializers import handel_target, is_validate_target
...
class GetAllConfigListResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        service_category_id = serializers.IntegerField(required=False, label=_("服务分类"))
        label = serializers.CharField(required=False, label=_("标签"))
        cmdb_node = serializers.CharField(required=False, label=_("节点信息"))
        search_value = serializers.CharField(required=False, label=_("模糊搜索字段"))
    ...
    def search_strategy_config_by_node(self, cmdb_node):
        ...
        for item in items:
            if not item.target or item.target[0]:
                continue
    ...
    def search_config_by_service_category_id(self, bk_biz_id, service_category_id):
        # 根据服务分类节点筛选配置
    ...
    def handle_return_data(self):
        ...
        for strategy_config in self.strategy_config_list:
            ...
            sql_instances = QueryConfig.from_models(
                QueryConfigModel.objects.filter(item_id__in=list({item.id for item in item_instances}))
            )
            collect_config_list = []
            for sql_instance in sql_instances:
                collect_config_list.extend(
                    [
                        ...
                        for condition in sql_instance.agg_condition
    ...
    def perform_request(self, validated_request_data):
        ...
        if service_category_id:
            self.search_config_by_service_category_id(bk_biz_id, service_category_id)
...
class ExportPackageRequestSerializer(serializers.Serializer):
    ...
    collect_config_ids = serializers.ListField(required=False, allow_empty=True, label=_("需要导出的采集配置ID列表"))
    strategy_config_ids = serializers.ListField(required=False, allow_empty=True, label=_("需要导出的策略配置ID列表"))
    list_data = serializers.ListField(required=False, allow_empty=True, label=_("需转为csv的列表数据"))
...
class ExportPackageResource(Resource):
    ...
    def perform_request(self, validated_request_data):
        ...
        if not any([self.collect_config_ids, self.strategy_config_ids, self.list_data]):
    ...
    def make_strategy_config_file(self):
        ...
        result = resource.strategies.backend_strategy_config_list(
            bk_biz_id=self.bk_biz_id, ids=self.strategy_config_ids
        )
        ...
        for result_data in result:
            ...
            for item_msg in result_data["item_list"]:
                item_msg["target"] = [[]]
                if item_msg.get("rt_query_config", {}).get("result_table_id", "").endswith("_cmdb_level"):
                    extend_msg = item_msg["rt_query_config"]["extend_fields"]["origin_config"]
                    strategy_instance = StrategyModel.objects.get(id=strategy_id)
                    target_type = strategy_instance.target_type
                    item_msg["rt_query_config"]["result_table_id"] = extend_msg["result_table_id"]
                    item_msg["rt_query_config"]["agg_dimension"] = extend_msg["agg_dimension"]
                    item_msg["rt_query_config"]["extend_fields"] = ""
                    item_msg["rt_query_config"]["agg_dimension"].extend(target_type_to_dimensions[target_type])
...
class HistoryDetailResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        import_history_id = serializers.IntegerField(required=True, label=_("导入历史ID"))
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
    ...
    def get_count_msg(self, config_list, action_type):
        ...
        type_to_count_map = {"plugin": plugin_count, "collect": collect_count, "strategy": strategy_count}
        ...
        return all_count, plugin_count, collect_count, strategy_count
    ...
    def perform_request(self, validated_request_data):
        ...
        all_count, plugin_count, collect_count, strategy_count = self.get_count_msg(
...
class UploadPackageResource(Resource):
    ...
    def __init__(self):
        ...
        self.type_map = {"plugin": self.plugin_count, "collect": self.collect_count, "strategy": self.strategy_count}
    ...
    class RequestSerializer(serializers.Serializer):
        file_data = serializers.FileField(required=True, label=_("文件内容"))
    ...
    def un_tar_package(self):
        ...
        try:
            ...
            logger.exception(_("压缩包解压失败: {}").format(e))
...
class ImportConfigResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        uuid_list = serializers.ListField(required=True, label=_("配置的uuid"))
        import_history_id = serializers.IntegerField(required=False, label=_("导入历史ID"))
    ...
    def perform_request(self, validated_request_data):
        ...
        if import_history_id:
            ...
            file_id = parse_instances.first().file_id
            plugin_name_list = [
                parse_obj.config["plugin_id"] for parse_obj in parse_instances.filter(type=ConfigType.COLLECT)
            ]
            plugin_parses = ImportParse.objects.filter(
                name__in=plugin_name_list, type=ConfigType.PLUGIN, file_id=file_id
            )
            create_detail_data = []
            ...
            for plugin_parse_obj in plugin_parses:
                create_detail_data.append(
                    ImportDetail(
                        name=plugin_parse_obj.name,
                        type=plugin_parse_obj.type,
                        label=plugin_parse_obj.label,
                        history_id=self.import_history_instance.id,
                        import_status=ImportDetailStatus.IMPORTING,
                        parse_id=plugin_parse_obj.id,
                    )
                )
        ...
        if not any([collect_config_list, strategy_config_list]):
            raise ImportConfigError({"msg": _("未选择任何配置")})

        if any([collect_config_list, strategy_config_list]):
            import_config.delay(
                username, bk_biz_id, self.import_history_instance, collect_config_list, strategy_config_list
            )
...
class AddMonitorTargetResource(Resource):
    class RequestSerializer(serializers.Serializer):
        import_history_id = serializers.IntegerField(required=True, label=_("导入历史ID"))
        target = serializers.ListField(required=True, label=_("监控目标"))
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
