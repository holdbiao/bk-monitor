<template>
  <div class="metric-dimension" v-bkloading="{ isLoading: loading, extCls: 'bk-loading-fixed' }">
    <div class="dialog-hearder">
      <div class="title" v-if="!isFromHome">{{ $t('设置指标&维度') }}</div>
      <!-- 顶部按钮 -->
      <div class="set-button">
        ...
        <bk-dropdown-menu :disabled="!canMoveBtn">
          <bk-button type="primary" slot="dropdown-trigger" class="move">
            <span>{{ $t('移动到...') }}</span>
        ...
        <bk-popover placement="top-start" :tippy-options="tippyOptions" :delay="200">
          <bk-button v-if="!isFromHome" class="mc-btn-add btn" style="margin-left: 8px;" @click="handleRefreshData">{{ $t('刷新') }}</bk-button>
          <div slot="content">
        ...
        <div class="set-right">
          ...
          <template v-if="!isFromHome">
            <monitor-import
              style="color: #3a84ff; margin-right: 10px;"
              :return-text="true"
            ...
            <span class="blue dataPreview" @click="handleShowData">{{ $t('数据预览') }}</span>
            <bk-switcher v-model="dataPreview" size="small" theme="primary"></bk-switcher>
          </template>
          <template v-else>
            <i class="bk-icon icon-exclamation-circle-shape risk-icon"></i>
            <span class="mr-10">{{ $t('编辑指标或维度存在风险') }}</span>
            <bk-popover class="change-name" placemnet="top-end">
            ...
            <monitor-export
              ...
              @click="cb => authority.MANAGE_AUTH ? handleExportMetric(cb) : handleShowAuthorityDetail()" />
            <span style="color: #dcdee5;">|</span>
            <span class="blue dataPreview" @click="handleHideStop">{{ $t('隐藏已停用') }}</span>
            <bk-switcher v-model="hideStop" size="small" theme="primary" @change="handleHideActive"></bk-switcher>
          </template>
        </div>
      </div>
    </div>
    <!-- 指标/维度表格 -->
    <metric-group
      v-for="(group, index) in tableData" :key="index"
    ...
    <div class="footer-btn mb10">
      ...
      <bk-button v-if="!isFromHome" class="mc-btn-add add-btn btn mr-r" theme="primary" @click="handleBackPlugin"> {{ $t('返回插件定义') }} </bk-button>
      <bk-button v-else-if="isFromHome || isShowCancel" class="mc-btn-add add-btn btn" @click="handleCancel"> {{ $t('取消') }} </bk-button>
    </div>
    <!-- 新建/编辑分组dialog -->
    <bk-dialog
      ...
      <div class="metric-name">
        <div class="hint">
          <i class="icon-monitor icon-tips"></i> {{ $t('指标分类的定义影响指标检索的时候,如试图查看，仪表盘添加视图和添加监控策略时选择指标的分类。') }} </div>
        <span class="item required">{{ $t('英文名') }}</span>
        <verify-input class="verify-input" :validator="{ content: $t('输入指标名,以字母开头,允许包含下划线和数字') }" :show-validate.sync="rule.isNameEmpty">
          <bk-input :placeholder="$t('英文名')" v-model.trim="groupDialog.name" @blur="rule.isNameEmpty = !regx.test(groupDialog.name)"></bk-input>
        </verify-input>
        <span class="item"> {{ $t('别名') }} </span>
        <verify-input>
          <bk-input :placeholder="$t('别名')" v-model="groupDialog.desc"></bk-input>
        </verify-input>
      </div>
      <div class="footer">
        <bk-button class="confirm-btn" theme="primary" @click="handleSetGroup"> {{ $t('确认') }} </bk-button>
...
import VerifyInput from '../verify-input.vue'
...
export default {
  ...
  props: {
    ...
    pluginData: { //  保存指标/维度必要的插件数据
      type: Object,
      default: () => ({})
    },
    isFromHome: { //  是否首页入口或者详情入口
      type: Boolean,
      default: false
    },
  ...
  computed: {
    ...
    canSave() {
      ...
      const metricExisted = []
      this.tableData.some(row => row.fields.filter(item => item.monitor_type === 'metric').some((item) => {
      ...
      this.tableData.some((row) => {
        const existed = []
        return row.fields.filter(item => item.monitor_type === 'dimension').some((item) => {
      ...
      const descExisted = []
      this.tableData.some(row => row.fields.filter(item => item.description !== '').some((item) => {
    ...
    typeList() {
      ...
      if (this.pluginData && ['JMX', 'Exporter', 'Pushgateway'].includes(this.pluginData.plugin_type)) {
  ...
  methods: {
    ...
    handleSetGroup() {
      ...
      // 编辑情况下未变名字
      if (group.isEdit && this.tableData[group.index].table_name === group.name) {
        group.isShow = false
        return
      }
      // 校验分组名字是否与关键字冲突
      ...
      if (res || res1) {
        this.$bkMessage({ theme: 'error', message: `${this.$t('指标分类不能同名且不能为')}${group.name}` })
        return
      }
      // 新增/编辑
      if (group.isEdit) {
              ...
              if (item.monitor_type === 'metric') {
                ...
                if (!this.checkDimensionRelevance(item, curMetrics)) {
                  ...
                  if (findIndex === -1 && Array.isArray(stopedData)) {
                    findIndex = stopedData.findIndex(row => row.monitor_type === 'dimension'
    ...
    async handleSave() {
      ...
      if (!this.haveData) {
        this.$bkMessage({ theme: 'error', message: this.$t('至少设置一个指标并且是启用状态') })
        return
      }
      if (!this.canSave) {
        this.$bkMessage({ theme: 'error', message: this.$t('所有的指标/维度的英文名和别名不能重名或与关键字冲突或为空') })
        return
      }
      // 前端业务逻辑字段，传给后端时删掉
      ...
      const params = {
        ...
        metric_json: (tableData || []).map(item => ({ ...item, fields: item.fields.map((set) => {
          const tmpSet = { ...set }
      ...
      if (data) {
        let result = true
        if (data.token) {
          result =  await releaseCollectorPlugin(this.pluginData.plugin_id, data).then(() => true)
            .catch(() => false)
        }
    ...
    handleExportMetric(cb) {
      typeof cb === 'function' && cb(
        ...
        this.tableData.filter(item => item.table_name).map(item => ({
          ...item,
          fields: item?.fields?.filter(item => item.name).map(({ description, monitor_type,
            is_diff_metric, name, type, unit,
            is_active, dimensions = [] }) => {
    ...
    handleImportMetric(data) {
      ...
      if (dataJson?.length) {
        dataJson.forEach((item, index) => {
          if (item.table_name) {
            const tableItem = {
              table_name: item.table_name,
              table_desc: item.table_name || item.table_name,
              fields: []
            }
            const fieldList = []
            ...
            item.fields.forEach((field, childIndex) => {
              if (!field.name) {
                errorList.push(this.$t(
                  '分组：{tableName} 第{index}个字段未填写名称'
                  , { tableName: item.table_name,  index: childIndex + 1 }
                ))
              }
              ...
              if (fieldList.some(set => set.description !== '' && set.description === field.description)) {
                errorList.push(this.$t(
                  '分组：{tableName} 别名：{fieldName}重复'
                  , { tableName: item.table_name,  fieldName: field.description }
                ))
              }
      ...
      if (errorList.length) {
        this.$bkMessage({
          theme: 'error',
          message: this.$createElement('ul', {}, errorList.map(message => this.$createElement('li', {}, message))),
          delay: 10000,
          ellipsisLine: 0
        })
        return
