    ...
    <div class="step-set-done-body">
      <div class="done-loading" v-if="loading">
        ...
        <div class="loading-message">
          <h4 class="message-title"> {{ $t('保存插件中，请稍后...') }} </h4>
        </div>
      </div>
      <div class="done-result" v-else>
        <div class="result-success" v-if="status">
          <svg class="success-svg" viewBox="0 0 64 64">
            <path
          ...
          <div class="success-message">
            <template v-if="data.type === 'edit'">
              <h4 class="message-title"> {{ $t('插件编辑完成') }} </h4>
              <p class="message-tip"> {{ $t('经过努力，新的插件生成了，恭喜') }} </p>
            </template>
            <template v-else>
              <h4 class="message-title"> {{ $t('插件创建完成') }} </h4>
              <p class="message-tip"> {{ $t('经过努力，终于拥有了完全属于你自己的插件，加油') }} </p>
            </template>
          </div>
        </div>
        <div class="result-fail" v-else>
          <svg viewBox="0 0 64 64" class="fail-svg">
            <g>
          ...
          <div class="fail-message">
            <h4 class="message-title">{{data.type === 'edit' ? $t('插件编辑失败') : $t('插件新建失败')}}</h4>
            <p class="message-tip">{{error}}</p>
          </div>
        </div>
      </div>
    </div>
    <div class="step-set-done-footer" v-if="!loading">
      <bk-button class="mr10" @click="close"> {{ $t('关闭') }} </bk-button>
      <bk-button class="mr10" theme="primary" v-if="needUpdataPlugin" @click="goToCollectConfig">{{ $t('前往采集配置升级插件') }}</bk-button>
      <bk-button theme="primary" @click="goToCollectConfigAdd">{{ $t('新增采集配置') }}</bk-button>
    </div>
  </div>
</template>
...
export default {
  ...
  async activated() {
    ...
    const { data } = this
    const { saveData } = data
    if (saveData.from === 'save') { // 从【保存插件】按钮进来
      if (data.type === 'create') { // 新建
