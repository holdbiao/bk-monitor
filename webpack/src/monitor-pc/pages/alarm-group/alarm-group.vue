<template>
  <div class="alarm-group" v-monitor-loading="{ 'isLoading': loading }">
    <!-- 列表页 -->
    <div class="alarm-group-tool">
      <bk-button
        ...
        theme="primary"
        @click="authority.MANAGE_AUTH ? handleShowAddView('add') : handleShowAuthorityDetail()"> {{ $t('新建') }}
      </bk-button>
      <bk-input
        class="tool-search"
    ...
    <div>
      <bk-table class="alarm-group-table" :empty-text="$t('查询无数据')" :data="table.data">
        <bk-table-column label="ID" prop="id" width="70">
        ...
        <bk-table-column :label="$t('名称')" min-width="100">
          <template slot-scope="scope">
            <span class="notice-group-name" @click="handleShowDetailsView(scope.row)">
              {{ scope.row.name }}
            </span>
          </template>
        </bk-table-column>
        ...
        <bk-table-column :label="$t('应用策略数')" width="200">
          <template slot-scope="scope">
        ...
        <bk-table-column :label="$t('说明')" prop="message" min-width="180">
          <template slot-scope="scope">
            {{scope.row.message || '--'}}
          </template>
        </bk-table-column>
...
import alarmGroupDetail from './alarm-group-detail/alarm-group-detail.vue'
  ...
  methods: {
    ...
    handleDeleteRow(id) {
      this.$bkInfo({
        ...
        confirmFn: () => {
          this.loading = true
          deleteNoticeGroup({ id_list: [id] }).then(() => {
            this.getNoticeGroupList()
