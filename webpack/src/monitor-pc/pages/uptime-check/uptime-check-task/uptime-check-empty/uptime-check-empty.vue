<template>
  <div class="no-tasks">
    <div class="desc">{{!isNode ? $t('暂无拨测任务') : $t('暂无拨测节点')}}</div>
    <div v-if="!isNode">
      <div class="create-task">
        <div class="create-title"> {{ $t('新建') }} </div>
        <div class="create-desc"> {{ $t('点击立即创建业务的拨测监控，化被动投诉为主动发现问题') }} </div>
        <span
      ...
      <div class="create-task">
        <div class="create-title"> {{ $t('导入') }} </div>
        <div class="create-desc"> {{ $t('根据官方提供的任务模板Excel，您可以快速批量导入多个拨测任务') }} </div>
        <span
          v-authority="{ active: !authority.MANAGE_AUTH }"
          class="create-btn"
    ...
    <div v-else>
      <div class="create-task">
        <div class="create-title"> {{ $t('新建') }} </div>
        <div class="create-desc"> {{ $t('创建拨测任务前先要至少有一个拨测节点,拨测节点负责拨测任务的执行.') }} </div>
        <span
          v-authority="{ active: !authority.MANAGE_AUTH }"
          class="create-btn"
