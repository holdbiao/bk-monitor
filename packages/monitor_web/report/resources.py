...
from core.drf_resource import resource, api, CacheResource
from core.drf_resource.base import Resource
from core.drf_resource.exceptions import CustomException
from monitor_web.models.custom_report import CustomTSTable
from monitor_web.report.serializers import (
    ReceiverSerializer,
    StaffSerializer,
    FrequencySerializer,
...
from bkmonitor.models.base import ReportItems, ReportContents, ReportStatus
from constants.report import StaffChoice, GroupId, return_replace_val_dict

logger = logging.getLogger(__name__)
GlobalConfig = apps.get_model("bkmonitor.GlobalConfig")


def create_report_dashboard(bk_biz_id):
    """
    创建运营报表默认仪表盘
    """
    try:
        ...
        tstable = CustomTSTable.objects.get(bk_data_id=bk_data_id)
        org_id = resource.grafana.get_org_id(bk_biz_id)
        with open("packages/monitor_web/grafana/dashboards/report.json", encoding="utf-8") as f:
            dashboard_config = json.loads(f.read())
        for panel in dashboard_config.get("panels", []):
            if panel.get("targets", []):
                new_rt_id = f"{int(settings.MAIL_REPORT_BIZ)}_bkmonitor_time_series_{bk_data_id}.base"
                panel["targets"][0]["data"]["metric"]["id"][0] = "custom.time_series"
                panel["targets"][0]["data"]["metric"]["id"][1] = tstable.time_series_group_id
                panel["targets"][0]["data"]["metric"]["id"][2] = new_rt_id
                if panel["targets"][0]["data"].get("metricDetail", {}):
                    panel["targets"][0]["data"]["metricDetail"]["resultTableId"] = new_rt_id
        ...
        logger.error(f"Create report dashboard failed: {e}")
...
class ReportListResource(Resource):
    ...
    def create_report_dashboard_if_need(self):
        """
        是否需要创建默认仪表盘
        """
        try:
            dashboard_uids = [
                dashboard["uid"]
                for dashboard in resource.grafana.get_dashboard_list(bk_biz_id=int(settings.MAIL_REPORT_BIZ))
            ]
        except Exception:
            return
        if settings.REPORT_DASHBOARD_UID not in dashboard_uids:
            # 创建订阅报表默认仪表盘
            create_report_dashboard(int(settings.MAIL_REPORT_BIZ))
    ...
    def perform_request(self, validated_request_data):
        ...
        # 检查是否需要创建仪表盘
        # self.create_report_dashboard_if_need()
...
class StatusListResource(Resource):
    ...
    def perform_request(self, validated_request_data):
        ...
        status_queryset = ReportStatus.objects.filter(report_item__in=list(items.keys())).order_by("-create_time")
...
class GroupListResource(CacheResource):
    ...
    def format_data(self, group_type, children):
        ...
        :return: 格式化后的数据
        """
        notify_groups = settings.NOTIRY_MAN_DICT
    ...
    def perform_request(self, validated_request_data):
        all_business = api.cmdb.get_business(all=True, bk_biz_ids=self.fetch_strategy_bizs())
        maintainers = []
        productors = []
        developers = []
        testers = []
