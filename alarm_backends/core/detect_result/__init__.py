...
from alarm_backends.constants import LATEST_NO_DATA_CHECK_POINT, LATEST_POINT_WITH_ALL_KEY
...
class CheckResult(Result):
    ...
    def last_check_point_field(self):
        return key.LAST_CHECKPOINTS_CACHE_KEY.get_field(
            strategy_id=self.strategy_id, item_id=self.item_id, dimensions_md5=self.dimensions_md5, level=self.level
        )
    ...
    @staticmethod
    def update_last_checkpoint_by_dimensions_md5(strategy_id, item_id, dimensions_md5, check_point, level):
        ...
        detect_service.hset(key.LAST_CHECKPOINTS_CACHE_KEY.get_key(), last_checkpoint_cache_field, check_point)
        return check_point

    @staticmethod
    def expire_last_checkpoint_cache():
        key.LAST_CHECKPOINTS_CACHE_KEY.expire()
    ...
    # ----- Func of other ----- #
    @staticmethod
    def get_all_last_checkpoints_cache():
        return key.LAST_CHECKPOINTS_CACHE_KEY.client.hgetall(key.LAST_CHECKPOINTS_CACHE_KEY.get_key())

    @staticmethod
    def get_all_last_checkpoints_cache_key():
        return key.LAST_CHECKPOINTS_CACHE_KEY.client.hkeys(key.LAST_CHECKPOINTS_CACHE_KEY.get_key())

    def clean_check_result_cache_key(self):
        """
        clean CHECK_RESULT_CACHE_KEY LAST_CHECKPOINTS_CACHE_KEY and with level
        """
        self.CHECK_RESULT.delete(self.check_result_cache_key)
        self.CHECK_RESULT.hdel(key.LAST_CHECKPOINTS_CACHE_KEY.get_key(), self.last_check_point_field)

    def clean_no_data_last_checkpoints_cache_key(self):
        """
        :summary: clean no data last_checkpoints_cache_key
        :return:
        """
        self.clean_check_result_cache_key()
        self.CHECK_RESULT.hdel(self.md5_to_dimension_key, self.dimensions_md5)
        self.CHECK_RESULT.hdel(key.LAST_CHECKPOINTS_CACHE_KEY.get_key(), self.latest_no_data_check_point_field)

    def clean_last_checkpoints_cache_key(self):
        """
        clean all check_result_cache_key
        """
        self.clean_check_result_cache_key()
        self.CHECK_RESULT.hdel(self.md5_to_dimension_key, self.dimensions_md5)
        # delete __ALL__ key in key.LAST_CHECKPOINTS_CACHE_KEY
        self.CHECK_RESULT.hdel(key.LAST_CHECKPOINTS_CACHE_KEY.get_key(), self.latest_point_with_all_field)
