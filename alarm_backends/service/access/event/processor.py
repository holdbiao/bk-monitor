...
from alarm_backends.service.access.event.filters import ExpireFilter, ConditionFilter
...
from alarm_backends.service.access.event.records.custom_event import GseCustomStrEventRecord
    ...
    def push_to_check_result(self):
        ...
        for event_record in self.record_list:
            for item in event_record.items:
                ...
                item_id = item.id
                if not event_record.is_retains[item_id]:
                    continue
        ...
        for md5_dimension_last_point_key, point_timestamp in list(last_checkpoints.items()):
            try:
                ...
                CheckResult.update_last_checkpoint_by_dimensions_md5(
                    strategy_id, item_id, md5_dimension, point_timestamp, level
                )
            except Exception as e:
...
class AccessCustomEventGlobalProcess(BaseAccessEventProcess):
    ...
    def pull(self):
        ...
        # 获取topic信息
        topic_info = api.metadata.get_data_id(bk_data_id=self.data_id)
        ...
        try:
            ...
            kafka_queue.set_topic(topic, group_prefix="access.event.%s" % self.data_id)
