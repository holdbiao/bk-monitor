...
from bkmonitor.strategy.new_strategy import Action, Algorithm, Detect, Item, QueryConfig, Strategy
...
class TestStrategy:
    ...
    def test_to_dict(self, clean_model):
        ...
        assert len(strategy.actions) == 1 and isinstance(strategy.actions[0], Action)
    ...
    def test_save(self, clean_model):
        ...
        assert ActionModel.objects.filter(id=strategy.actions[0].id).exists()
        ...
        strategy.actions[0].type = "threshold"
        ...
        assert ActionModel.objects.get(id=strategy.actions[0].id).action_type == strategy.actions[0].type
