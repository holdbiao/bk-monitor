...
class CollectConfigListResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        refresh_status = serializers.BooleanField(required=False, label=_("是否刷新状态"))
        search = serializers.DictField(required=False, label=_("搜索字段"))
        order = serializers.CharField(required=False, label=_("排序字段"))
        disable_service_type = serializers.BooleanField(default=True, label=_("不需要服务分类"))
    ...
    def get_realtime_data_nodeman_v2_1(self, config_data_list):
        ...
        # 节点管理返回的状态数量
        for subscription_status in collect_statistics_data:
            status_number = {}
            ...
            if config.last_operation != OperationType.STOP:
                # 开启订阅，这里需要考虑挪个地方做
                config.switch_subscription("enable")
    ...
    def need_upgrade(self, conf):
        ...
        if conf.task_status == TaskStatus.STOPPED or conf.get_cache_data("total_instance_count", 0) == 0:
            ...
            if not config_version:
                config_version = conf.plugin.release_version.config_version
    ...
    def perform_request(self, validated_request_data):
        try:
            ...
            if bk_biz_id:
                config_list = config_list.filter(bk_biz_id=bk_biz_id)
            else:
            ...
            if search_dict:
                for item in search_list:
                    if any([v in item[k] for k, v in list(search_dict.items())]):
            ...
            return {"type_list": type_list, "config_list": result_list}
        except Exception:
            import traceback
...
class CollectConfigDetailResource(Resource):
    ...
    def perform_request(self, validated_request_data):
        ...
        if (
            ...
            collect_config_meta.target_object_type == TargetObjectType.HOST
            and collect_config_meta.deployment_config.target_node_type
            in [TargetNodeType.SERVICE_TEMPLATE, TargetNodeType.SET_TEMPLATE]
        ):
            target_result = []
            templates = {
            ...
            collect_config_meta.target_object_type == TargetObjectType.SERVICE
            and collect_config_meta.deployment_config.target_node_type
            in [TargetNodeType.SERVICE_TEMPLATE, TargetNodeType.SET_TEMPLATE]
        ):
            target_result = []
        ...
        release_version = collect_config_meta.plugin.get_release_version_by_config_version(config_version)
        result = {
...
class ToggleCollectConfigStatusResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        id = serializers.IntegerField(required=True, label=_("采集配置ID"))
        action = serializers.ChoiceField(required=True, choices=["enable", "disable"], label=_("启停配置"))
...
class RetryTargetNodesResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        id = serializers.IntegerField(required=True, label=_("采集配置ID"))
        target_nodes = serializers.ListField(required=True, label=_("需要重试的实例"))
        steps = serializers.DictField(required=True, label=_("重试步骤"))
    ...
    def perform_request(self, validated_request_data):
        ...
        if collect_config.target_object_type == TargetObjectType.HOST:
            target_instances = [
                {"ip": item["ip"], "bk_cloud_id": item["bk_cloud_id"], "bk_supplier_id": item["bk_supplier_id"]}
                for item in target_nodes
            ]
        else:
            target_instances = [{"id": item["service_instance_id"]} for item in target_nodes]

        scope = {"node_type": TargetNodeType.INSTANCE, "nodes": target_instances}

        # 主动触发节点管理订阅，更新采集配置信息
        if collect_config.deployment_config.subscription_id:
            task_id = collect_config.trigger_subscription(scope=scope, steps=validated_request_data["steps"])
...
class RevokeTargetNodesResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        id = serializers.IntegerField(required=True, label=_("采集配置ID"))
        instance_ids = serializers.ListField(required=True, label=_("需要终止的实例ID"))
...
class GetCollectLogDetailResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        id = serializers.IntegerField(required=True, label=_("采集配置ID"))
        instance_id = serializers.CharField(required=True, label=_("主机/实例id"))
        task_id = serializers.IntegerField(required=True, label=_("任务id"))
...
class BatchRetryConfigResource(Resource):
    ...
    def get_failed_instances(self):
        ...
        result = api.node_man.task_result(**params)

        # 所有失败的实例
        failed_instances = [item for item in result if item["status"] == CollectStatus.FAILED]
    ...
    def perform_request(self, validated_request_data):
        ...
        failed_instances = self.get_failed_instances()
        ...
        for instance in failed_instances:
            ...
            for group in step_groups:
                # 把相同的步骤的实例放到一起
                if steps == group["steps"]:
                    group["instances"].append(self.get_node(instance))
                    break
            else:
                step_groups.append({"instances": [self.get_node(instance)], "steps": steps})

        # 根据step的不同分类，依次发送重试请求
        for group in step_groups:
            scope = {"node_type": TargetNodeType.INSTANCE, "nodes": group["instances"]}
            task_id = self.config.trigger_subscription(scope=scope, steps=group["steps"])
...
class SaveCollectConfigResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        class RemoteCollectingSlz(serializers.Serializer):
            ...
            bk_supplier_id = serializers.IntegerField(required=True)
            is_collecting_only = serializers.BooleanField(required=True)

        id = serializers.IntegerField(required=False, label=_("采集配置ID"))
        name = serializers.CharField(required=True, label=_("采集配置名称"))
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        collect_type = serializers.ChoiceField(
            required=True, label=_("采集方式"), choices=CollectConfigMeta.COLLECT_TYPE_CHOICES
        )
        target_object_type = serializers.ChoiceField(
            required=True, label=_("采集对象类型"), choices=CollectConfigMeta.TARGET_OBJECT_TYPE_CHOICES
        )
        target_node_type = serializers.ChoiceField(
            required=True, label=_("采集目标类型"), choices=DeploymentConfigVersion.TARGET_NODE_TYPE_CHOICES
        )
        plugin_id = serializers.CharField(required=True, label=_("插件ID"))
        target_nodes = serializers.ListField(required=True, label=_("节点列表"))
        remote_collecting_host = RemoteCollectingSlz(required=False, allow_null=True, default=None, label=_("远程采集配置"))
        params = serializers.DictField(required=True, label=_("采集配置参数"))
        label = serializers.CharField(required=True, label=_("二级标签"))
        operation = serializers.ChoiceField(default="EDIT", choices=["EDIT", "ADD_DEL"], label=_("操作类型"))

        def validate(self, attrs):
            # 校验采集对象类型和采集目标类型搭配是否正确，且不同类型的节点列表字段正确
            # 校验业务拓扑和服务拓扑
            if (attrs["target_object_type"], attrs["target_node_type"]) == (
                TargetObjectType.HOST,
                TargetNodeType.TOPO,
            ) or (attrs["target_object_type"], attrs["target_node_type"]) == (
            ...
            # 校验主机实例
                ...
                for node in attrs["target_nodes"]:
                    if not ("ip" in node and "bk_cloud_id" in node and "bk_supplier_id" in node):
                        raise serializers.ValidationError("target_nodes needs ip, bk_cloud_id and bk_supplier_id")
            # 校验服务模板、集群模板
    ...
    def perform_request(self, data):
        ...
        if data["target_node_type"] in [
            ...
            # 动态拓扑、集群模板、服务模板
            target_nodes = [
                {"bk_inst_id": node["bk_inst_id"], "bk_obj_id": node["bk_obj_id"]} for node in data["target_nodes"]
            ]
        else:
            # 静态
            target_nodes = [
...
class GraphPointResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        id = serializers.IntegerField(required=True, label=_("采集配置id"))
        host_list = IpListSlz(required=False, many=True, label=_("主机信息"))
        instance_list = serializers.ListField(required=False, label=_("实例信息"))
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
...
class BaseCollectTargetStatusResource(Resource):
    class RequestSerializer(serializers.Serializer):
        id = serializers.IntegerField(required=True, label=_("采集配置id"))
        auto_running_tasks = serializers.ListField(required=False, label=_("自动运行的任务"))
    ...
    def get_target_status(self):
        ...
        if self.is_task_result:
            ...
            task_result = api.node_man.task_result(**params)
    ...
    def generate_host_status_dict(self):
        # 生成主机状态信息的字典格式
        host_status_dict = {}
        for item in self.host_instance_status:
            host_key = self.get_host_key(item)
            host_status_dict[host_key] = item
    ...
    def fetch_latest_version_by_config_version(self):
        """
        根据主配置拿最新子配置版本号
        """
        config_version = self.config.deployment_config.plugin_version.config_version
        ...
        if self.is_task_result and self.parent_deployment_config:
            ...
            for instance in instance_list:
                for key, label in list(self.diff_label_mapping.items()):
                    if [x for x in diff_node[key] if x.get("ip") == instance["ip"]]:
            ...
            if target_object_type == "host":
                ...
                for host in extra_info["host_list"]:
                    if not set(host.bk_module_ids) & contained_modules:
                        continue
                    status = extra_info["host_status_dict"].get(host.host_id)
                    if status:
    ...
    def _handle_host_topo_data(self, target_nodes=None):
        ...
        for node in node_scope:
            ...
            for host in host_list:
                ...
                status = host_status_dict.get(host.host_id)
                if status:
...
class CollectTargetStatusTopoResource(BaseCollectTargetStatusResource):
    ...
    def nodata_test(self, target_list):
        ...
        if self.config.plugin.plugin_type == PluginType.LOG or self.config.plugin.plugin_type == PluginType.SNMP_TRAP:
            ...
            if self.config.plugin.plugin_type == PluginType.PROCESS:
                ...
                latest_info_version = self.fetch_latest_version_by_config_version()
    ...
    def handle_host_topo_data(self):
        ...
        for host in hosts:
            # 标识主机所对应的状态
            host_key = "{}|{}".format(host.bk_host_innerip, host.bk_cloud_id)
            status = host_status_dict.get(host_key)
...
class UpdateConfigStatusResource(Resource):
    ...
    def perform_request(self, data):
        ...
        config_list = CollectConfigMeta.objects.filter(operation_result=OperationResult.DEPLOYING)
        ...
        for config in config_list:
            try:
                ...
                errors.append("Unknown error for CollectConfigMeta {}: {}".format(config.id, traceback.format_exc()))
        if errors:
...
class UpdateConfigInstanceCountResource(Resource):
    ...
    def perform_request(self, data):
        ...
        try:
            ...
            logger.error(_("请求节点管理主机运行状态接口失败: {}").format(e))
            return
        ...
        for item in result:
            total_instance_count = len(item["instances"])
            error_instance_count = [instance["status"] for instance in item["instances"]].count(CollectStatus.FAILED)
...
def update_config_operation_result(config):
    """
    更新采集配置的任务执行结果
    """
    ...
    for item in status_result:
        ...
        logger.info(_("采集配置id:{}, 实例状态:{}").format(config.id, instances_status))
        if error_count == 0:
...
class CleanLegacySubscription(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        subscription_id = serializers.ListField(required=True, label=_("节点管理订阅ID"), child=serializers.IntegerField())
        action_type = serializers.CharField(default="STOP", label=_("动作类型"))
...
class ListRelatedStrategy(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        collect_config_id = serializers.IntegerField(required=True, label=_("采集配置ID"))
...
class EncryptPasswordResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        password = serializers.CharField(required=True, label=_("明文密码"))
...
class DecryptPasswordResource(Resource):
    class RequestSerializer(serializers.Serializer):
        encrypted_text = serializers.CharField(required=True, label=_("密文"))
