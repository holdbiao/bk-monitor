...
from core.errors.event import AggmethodIsRealtimeError, EventNotExist, NotTimeSeriesError
...
from monitor_web.constants import AlgorithmType, EventLevel, EVENT_FIELD_CHINESE
...
class QueryEventsResource(Resource):
    class RequestSerializer(serializers.Serializer):
        bk_biz_ids = serializers.ListField(required=True, label=_("业务id列表"))
        time_range = serializers.CharField(required=False, allow_blank=True, label=_("时间范围"))
        receiver = serializers.CharField(required=False, allow_blank=True, label=_("通知接受人"))
        conditions = serializers.ListField(default=[], label=_("搜索条件"))
        days = serializers.IntegerField(required=False, label=_("天数时间范围"))
...
class ListEventResource(EventPermissionResource, EventDimensionMixin):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        status = serializers.ChoiceField(
            required=False, allow_blank=True, choices=["ABNORMAL", "SHIELD_ABNORMAL"], label=_("事件状态")
        )
        conditions = serializers.ListField(default=[], label=_("搜索条件"))
        page_size = serializers.IntegerField(default=10, label=_("获取的条数"))
        page = serializers.IntegerField(default=1, label=_("页数"))
        order = serializers.CharField(required=False, label=_("排序"), default=None)
        export = serializers.BooleanField(required=False, label=_("导出事件"), default=None)
...
class StackedChartResource(EventPermissionResource):
    class RequestSerializer(serializers.Serializer):
        bk_biz_ids = serializers.ListField(required=True, label=_("业务id列表"))
        time_range = serializers.CharField(required=True, allow_blank=True, label=_("时间范围"))
        receiver = serializers.CharField(required=False, allow_blank=True, label=_("通知接受人"))
        conditions = serializers.ListField(default=[], label=_("搜索条件"))
...
class EventGraphQueryResource(EventPermissionResource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        id = serializers.IntegerField(required=True, label=_("事件ID"))
    ...
    def perform_request(self, params: Dict):
        ...
        if (item["data_source_label"], item["data_type_label"]) not in self.AvailableDataLabel:
            return []
        ...
        if not params.get("start_time") or not params.get("end_time"):
            ...
            end_time = min(
                end_time + params["interval"] * 5, start_time + 1440 * params["interval"], datetime2timestamp(now())
            )
            diff = 1440 * params["interval"] - (end_time - start_time)
            if diff < params["interval"] * 5:
                diff = params["interval"] * 5
            elif diff > params["interval"] * 60:
                diff = params["interval"] * 60
            start_time -= diff
        ...
        data = resource.grafana.time_series_query(params)
        if not data:
            return []
        ...
        alert_algorithm_list = [algorithm for algorithm in item["algorithm_list"] if algorithm["level"] == event.level]
        threshold_line = []
        if len(alert_algorithm_list) == 1 and alert_algorithm_list[0]["algorithm_type"] == AlgorithmType.Threshold:
            threshold_config = alert_algorithm_list[0]["algorithm_config"]
            if len(threshold_config) == 1 and len(threshold_config[0]) == 1:
                # 算法的值转换为数值单位
                algorithm_unit = alert_algorithm_list[0].get("algorithm_unit", "")
                threshold_value = threshold_config[0][0]["threshold"]
...
class DetailEventResource(EventPermissionResource, EventDimensionMixin):
    ...
    def perform_request(self, validated_request_data):
        ...
        item = event.origin_config["item_list"][0]
        # 日志平台检索索引ID
        log_index_id = None
        if (item["data_source_label"], item["data_type_label"]) not in EventGraphQueryResource.AvailableDataLabel:
            query_config = {}
        else:
            dimensions = event.origin_alarm.get("data", {}).get("dimensions", {})
            dimensions = {
                key: value for key, value in dimensions.items() if key in item["rt_query_config"]["agg_dimension"]
            }
            ...
            if (
                ...
                == (DataSourceLabel.BK_MONITOR_COLLECTOR, DataTypeLabel.LOG)
                and "bk_obj_id" in dimensions
                and "bk_inst_id" in dimensions
            ):
                bk_obj_id = dimensions.pop("bk_obj_id")
                bk_inst_id = dimensions.pop("bk_inst_id")
                dimensions[f"bk_{bk_obj_id}_id"] = str(bk_inst_id)

            where = GetGraphQueryConfig.create_where_with_dimensions(
                item["rt_query_config"]["agg_condition"], dimensions
            )
            agg_dimension = list(set(item["rt_query_config"]["agg_dimension"]) & set(dimensions.keys()))

            # 扩展指标（针对智能异常检测，需要根据敏感度来）
            extend_metric_fields = item["rt_query_config"].get("extend_fields", {}).get("values", [])
            algorithm_list = item.get("algorithm_list", [])
            intelligent_algorithm_list = [
                algorithm
                for algorithm in algorithm_list
                if algorithm["level"] == event.level
                and algorithm["algorithm_type"] == AlgorithmModel.AlgorithmChoices.IntelligentDetect
            ]
            if intelligent_algorithm_list:
                extend_metric_fields = ["is_anomaly", "lower_bound", "upper_bound"]

            query_config = {
                "query_string": item["rt_query_config"].get("keywords_query_string", ""),
                "bk_biz_id": validated_request_data["bk_biz_id"],
                "data_source_label": item["data_source_label"],
                "data_type_label": item["data_type_label"],
                "group_by": agg_dimension,
                "result_table_id": item["rt_query_config"]["result_table_id"],
                "metric_field": item["rt_query_config"].get("metric_field"),
                "extend_metric_fields": extend_metric_fields,
                "method": item["rt_query_config"]["agg_method"],
                "interval": item["rt_query_config"]["agg_interval"],
                "where": where,
                "function": {"time_compare": ["1d", "1w"]},
                "extend_fields": item["rt_query_config"].get("extend_fields", {}),
            }
            ...
            if item["data_type_label"] == DataTypeLabel.LOG:
                if item["data_source_label"] == DataSourceLabel.BK_LOG_SEARCH:
                    ...
                    query_config["result_table_id"] = (
                        item["rt_query_config"].get("extend_fields", {}).get("index_set_id")
                    )
                    if not query_config["result_table_id"]:
                        query_config = {}
                    log_index_id = query_config.get("result_table_id")
                else:
                    query_config["metric_field"] = "event.count"

            # 处理自定义事件型指标
            if (item["data_source_label"], item["data_type_label"]) == (DataSourceLabel.CUSTOM, DataTypeLabel.EVENT):
                query_config["metric_field"] = "_index"
                event_name = query_config["extend_fields"]["custom_event_name"]
                if event_name:
                    query_config["filter_dict"] = {"event_name": event_name}
        ...
        result_data = {
            ...
            "log_index_id": log_index_id,
        }

        if query_config:
            is_bar = (query_config["data_source_label"], query_config["data_type_label"]) in (
                (DataSourceLabel.BK_LOG_SEARCH, DataTypeLabel.LOG),
                (DataSourceLabel.CUSTOM, DataTypeLabel.EVENT),
            )
            ...
            result_data["graph_panel"] = {
                ...
                "title": item.get("name") or f"{query_config['result_table_id']}.{query_config['metric_field']}",
                "subTitle": f"{query_config['result_table_id']}.{query_config['metric_field']}",
                "targets": [
                    {"data": query_config, "datasourceId": "time_series", "name": _("时序数据"), "alias": "$time_offset"}
                ],
            }
...
class StrategySnapshotResource(EventPermissionResource):
    ...
    def perform_request(self, validated_request_data):
        ...
        try:
            strategy_config = StrategyModel.objects.get(bk_biz_id=bk_biz_id, id=event.strategy_id)
            is_enabled = strategy_config.is_enabled
        except StrategyNotExist:
            changed_status = ConfigChangedStatus.DELETED
        else:
            if int(strategy_config.update_time.timestamp()) != event.origin_config["update_time"]:
                changed_status = ConfigChangedStatus.UPDATED

        item_dict = event.origin_config["item_list"][0]
        exclude_fields = ["id", "update_time", "create_time"]
        for k, v in list(item_dict["rt_query_config"].items()):
            if k not in exclude_fields:
                item_dict[k] = v

        event.origin_config.update(strategy_status=changed_status)
        return_data = resource.strategies.strategy_config_detail.handle_result_data(event.origin_config)
        return_data.update(**resource.strategies.strategy_config_detail.get_target_detail(return_data))
        return_data["create_time"] = utc2datetime(return_data["create_time"])
        return_data["update_time"] = utc2datetime(return_data["update_time"])
        return_data["is_enabled"] = is_enabled
        return return_data
...
class DetailAlertNoticeResource(Resource):
    class RequestSerializer(serializers.Serializer):
        action_id = serializers.CharField(required=True, label=_("通知时间"))
    ...
    def perform_request(self, validated_request_data):
        ...
        for alert in alert_list:
            user_alert_info = alert_data.setdefault(
                alert.username,
                {"receiver": alert.username, "notice_group": settings.NOTIRY_MAN_DICT.get(alert.role, "")},
            )
...
class AckEventResource(EventPermissionResource):
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        id = serializers.IntegerField(required=False, label=_("事件ID"))
        message = serializers.CharField(required=True, allow_blank=True, label=_("确认信息"))
    ...
    def perform_request(self, validated_request_data):
        ...
        events = Event.objects.filter(
            id__in=event_ids, bk_biz_id=validated_request_data["bk_biz_id"], status=EventStatus.ABNORMAL, is_ack=False
        )

        message = {"message": validated_request_data["message"]}

        # 添加确认操作日志
        event_actions = []
        for event in events.only("event_id"):
            action_dict = {
                "username": get_request().user.username,
                "operate": EventOperate.ACK,
                "extend_info": message,
                "event_id": event.event_id,
            }
            event_actions.append(EventAction(**action_dict))
        EventAction.objects.bulk_create(event_actions, batch_size=100)
...
class GraphPointResource(Resource):
    ...
    def perform_request(self, validated_request_data):
        ...
        strategy_config = event.origin_config
        ...
        if item["rt_query_config"]["agg_method"] == AGG_METHOD_REAL_TIME:
            raise AggmethodIsRealtimeError({"event_id": event_unique_id})
        detect_algorithm_list = item["algorithm_list"]
        # 阈值线,暂时只支持静态阈值,以后显示同比环比
        # 用列表是因为同一level下,可以既有静态阈值,又有同比策略
        alert_algorithm_list = [algorithm for algorithm in detect_algorithm_list if algorithm["level"] == event.level]
        threshold_line = []
        if len(alert_algorithm_list) == 1 and alert_algorithm_list[0]["algorithm_type"] == AlgorithmType.Threshold:
            threshold_config = alert_algorithm_list[0]["algorithm_config"]
            if len(threshold_config) == 1 and len(threshold_config[0]) == 1:
        ...
        # 拼上产生告警的维度
        event_dimension_detail = event.origin_alarm["data"]["dimensions"]
        event_config_dimension = item["rt_query_config"]["agg_dimension"]
        event_config_condition = item["rt_query_config"]["agg_condition"]
        ...
        # 获取图表查询时间段
        plot_bands = {"from": event.origin_alarm["data"]["time"] * 1000, "to": None}
        ...
        graph_point_request_data = {
            "bk_biz_id": bk_biz_id,
            "monitor_field": item["rt_query_config"]["metric_field"],
            "method": item["rt_query_config"]["agg_method"],
            "result_table_id": item["rt_query_config"]["result_table_id"],
            "filter_dict": filter_dict,
            "unit": unit.suffix,
            "conversion": item["rt_query_config"].get("unit_conversion", 1.0),
            "use_short_series_name": True,
            "interval": math.ceil(float(item["rt_query_config"]["agg_interval"]) / 60.0),
            "time_step": math.ceil(float(item["rt_query_config"]["agg_interval"]) / 60.0),
            "time_start": None,
            "time_end": None,
            "extend_fields": item["rt_query_config"].get("extend_fields", {}),
            "data_source_label": item["data_source_label"],
        ...
        now_time = now()
        start_timestamp = int(datetime2timestamp(event.begin_time - datetime.timedelta(hours=1)))
        max_end_time = int(datetime2timestamp(event.begin_time + datetime.timedelta(hours=1)))
        if event.status == EventStatus.RECOVERED:
            graph_end_time = event.end_time + datetime.timedelta(hours=1)
            plot_bands["to"] = int(datetime2timestamp(event.end_time)) * 1000
            end_timestamp = min(datetime2timestamp(graph_end_time), datetime2timestamp(now_time), max_end_time)
        else:
            end_timestamp = datetime2timestamp(now_time)
        ...
        # 获取图表title
        title = item["rt_query_config"]["metric_field"]
        ...
        if metric_info:
            if metric_info.data_target == DataTarget.HOST_TARGET:
                dimension = event.origin_alarm["dimension_translation"]
                ...
                service_str = (
                    event.origin_alarm["dimension_translation"].get("service_instance", {}).get("display_value", "")
                )
        ...
        if plot_bands["from"] not in point_time_list and first_anomaly_in_time_range:
            point = [plot_bands["from"], event.origin_alarm["data"]["value"]]
...
class GetSolutionResource(EventPermissionResource):
    ...
    def perform_request(self, validated_request_data):
        ...
        metric_id = event.origin_config["item_list"][0]["metric_id"]
...
class SaveSolutionResource(EventPermissionResource):
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        id = serializers.IntegerField(required=True, label=_("事件ID"))
        solution = serializers.CharField(required=True, allow_blank=True, label=_("解决方法"))
...
class ListEventLogResource(EventPermissionResource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        id = serializers.IntegerField(required=True, label=_("事件ID"))
        offset = serializers.IntegerField(required=False, label=_("上一页最后一条日志ID"))
        limit = serializers.IntegerField(default=10, label=_("获取的条数"))
        operate = serializers.ListField(default=[], label=_("流转状态"))

    def get_ack_content(self, log):
        return {"contents": [_("{}确认了该告警事件并备注：").format(log.username), log.extend_info["message"]]}

    def get_notice_content(self, log):
        notice_action = self.event.origin_config["action_list"][0]
...
class ShieldSnapshotResource(EventPermissionResource):
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        id = serializers.IntegerField(required=True, label=_("事件ID"))
        shield_snapshot_id = serializers.IntegerField(required=True, label=_("事件ID"))
...
class EventRelatedInfoResource(EventPermissionResource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_biz_id = serializers.IntegerField(required=True, label=_("业务ID"))
        ids = serializers.ListField(child=serializers.IntegerField(), required=True, label=_("事件ID"))
    ...
    def get_log_related_info(bk_biz_id, events: List[Event]) -> Dict[str, Dict]:
        ...
        for event in events:
            ...
            if not item["rt_query_config"].get("extend_fields", {}).get("index_set_id"):
                continue

            related_infos[event.id] = {
                "type": "log_search",
                "index_set_id": item["rt_query_config"]["extend_fields"]["index_set_id"],
                "query_string": item["rt_query_config"].get("keywords_query_string", "*"),
                "agg_condition": item["rt_query_config"]["agg_condition"],
            }
    ...
    def get_custom_event_related_info(bk_biz_id, events: List[Event]) -> Dict[str, Dict]:
        ...
        for event in events:
            ...
            if "bk_event_group_id" in item["rt_query_config"]:
                related_infos[event.id]["type"] = "custom_event"
                related_infos[event.id]["bk_event_group_id"] = item["rt_query_config"]["bk_event_group_id"]
    ...
    def get_bkdata_related_info(bk_biz_id, events: List[Event]) -> Dict[str, Dict]:
        ...
        for event in events:
            if not event.origin_config["item_list"]:
                continue

            item = event.origin_config["item_list"][0]
            if item["data_source_label"] != DataSourceLabel.BK_DATA:
                continue
                ...
                "query_configs": [
                    {
                        "data_type_label": item["data_type_label"],
                        "data_source_label": item["data_source_label"],
                        "metric_field": item["rt_query_config"]["metric_field"],
                        "group_by": item["rt_query_config"]["agg_dimension"],
                        "result_table_id": item["rt_query_config"]["result_table_id"],
                        "method": item["rt_query_config"]["agg_method"],
                        "where": item["rt_query_config"]["agg_condition"],
                        "interval": item["rt_query_config"]["agg_interval"],
                    }
                ],
            }
...
class ListIndexByHost(EventPermissionResource):
    class RequestSerializer(serializers.Serializer):
        ip = serializers.CharField(required=True, label=_("主机IP"))
        bk_cloud_id = serializers.IntegerField(required=False, label=_("云区域ID"), default=0, allow_null=True)
