...
def mock_arrow(mocker):
    mocker.patch("alarm_backends.service.access.event.records.gse_event.arrow.now", return_value=now)
    mocker.patch("alarm_backends.service.access.event.records.gse_event.arrow.utcnow", return_value=utc_now)
...
class TestGseEventRecords(object):
    def test_agent(self, mocker):
        ...
        r = AgentEvent(AGENT_LOSE_DATA, {2: {31: strategy}})
        r.check()
        records = r.flat()
