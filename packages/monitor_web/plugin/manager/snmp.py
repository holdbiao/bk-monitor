...
from monitor_web.plugin.constant import MAX_METRIC_NUM
...
class SNMPPluginManager(PluginManager):
    ...
    def get_deploy_steps_params(self, plugin_version, param, target_nodes):
        ...
        plugin_data = param["plugin"]
        default_port = str(collector_params["port"])
        ...
        deploy_steps = [
            {
                ...
                "params": {"port_range": "%s,10000-65535" % default_port, "context": plugin_params},
            },
            self.get_bkmonitorbeat_deploy_step("bkmonitorbeat_prometheus_remote.conf", {"context": collector_params}),
        ]
        return deploy_steps
    ...
    def get_collector_json(self, plugin_params):
        ...
        content = yaml.load(self.read_file(config_yaml_path))
    ...
    def create_version(self, data):
        ...
        for group, item in yaml.load(config_yaml).items():
    ...
    def parse_snmp_yaml_to_metric(self, config_yaml):
        ...
        for item in config:
            ...
            if len(metric_json) > MAX_METRIC_NUM:
