<template>
  <div class="tendency-chart">
    ...
    <monitor-echarts
      ...
      :show-legend="true" />
    <div class="no-data" v-else>{{loading ? $t('数据加载中...') : $t('暂无数据')}}</div>
    <!-- 时间选择器 -->
    <datetime-picker
      :title="pickerTitle"
      :max-date="maxDate"
...
import { ISeriesData, ISelectGroup, ICompare, ICompareData } from '../../types/tendency-chart'
...
export default class TendencyChart extends Mixins(HideChartTooltipMixin) {
  ...
  async handleGetChartData() {
    ...
    if (chartSeries) {
      ...
      const coverList = []
      const upBoundary = data.find(item => item.dimensions.bk_target_ip === dimensions.bk_target_ip
      ...
      const lowBoundary = data.find(item => item.dimensions.bk_target_ip === dimensions.bk_target_ip
            && item.dimensions.bk_target_cloud_id === dimensions.bk_target_cloud_id
  ...
  getSemanticsTime(semantics: string) {
    if (!semantics || !semantics.endsWith('d')) {
      return semantics
    }
