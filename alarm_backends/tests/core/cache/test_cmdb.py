...
class TestHostManager(TestCase):
    ...
    def test_clear(self):
        ...
        self.assertEqual(len(keys), len(ALL_HOSTS))
    ...
    def test_refresh_exception(self, get_host_by_topo_node):
        ...
        self.assertSetEqual({"10.0.0.1|1", "10.0.0.2|2"}, set(json.loads(all_host_id_list["2"])))
        self.assertSetEqual({"10.0.0.3|3", "10.0.0.4|4"}, set(json.loads(all_host_id_list["3"])))
        self.assertSetEqual({"10.0.0.5|5", "10.0.0.6|6"}, set(json.loads(all_host_id_list["4"])))
        ...
        all_host_id_list = HostManager.cache.hgetall(HostManager.get_biz_cache_key())
        self.assertSetEqual({"10.0.0.1|1", "10.0.0.3|3", "10.0.0.4|4"}, set(json.loads(all_host_id_list["2"])))
    ...
    def test_remove_biz(self, get_business):
        get_business.return_value = ALL_BUSINESS
        HostManager.refresh()
        self.assertEqual(6, len(HostManager.all()))
        new_business_list = ALL_BUSINESS[:2]
        get_business.return_value = new_business_list
        # 删除业务4，减少2台主机
        HostManager.refresh()
        self.assertEqual(4, len(HostManager.all()))
