  ...
  <div class="title-wrapper">
    <div
      ...
      <div class="main-title">
        ...
        <span class="title-interval" v-if="hasMetric && extendMetricData.collect_interval">{{extendMetricData.collect_interval}}m</span>
        <i v-show="showMore"
           v-if="hasMetric"
           class="icon-monitor icon-tishi tips-icon"
           @mouseenter.self.stop="handleShowTips"
           @mouseleave.self.stop="handleHideTips"></i>
        <i :style="{ marginLeft: hasMetric ? '0px' : 'auto' }" v-show="showMore" class="icon-monitor icon-mc-more more-icon"></i>
      </div>
      <div v-if="subtitle"
...
export default class ChartTitle extends Vue {
  ...
  handleTips() {
    ...
    const elList = {
      ...
      bk_log_search_time_series: [ // 日志采集
        ...options,
