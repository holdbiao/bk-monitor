<template>
  <div class="dimension-table-wrapper">
    <div class="operation-upload-download">
      <div class="upload">
        <input type="file" multiple accept=".json" ref="uploadMetricFile" @change="getFileInfo" style="display: none;"> {{ $t('在下方自定义添加或') }} <span class="uploadt-btn" @click="handleUpload"> {{ $t('点击上传文件') }} </span>
      </div>
      <div class="download">
        <span v-if="isEdit"><span class="bk-icon icon-exclamation-circle risk"></span> {{ $t('编辑指标/维度有风险') }} </span>
        <a class="download-example" href="javascript:void(0);" @click="handleDownloadMetricJson"> {{ $t('下载样例') }} </a>
      </div>
    </div>
    <div style="margin-bottom: 10px;" v-for="(table, index) in tables" :key="index">
      <div class="table-title">
        ...
        <span class="operator">
          <svg @click="editTableName(index, table)">
            <g>
              <line x1="0" y1="20" x2="16" y2="20" stroke-width="2px" stroke="#63656E"></line>
              <line x1="3" y1="17" x2="14" y2="10" stroke-width="2px" stroke="#63656E"></line>
              <line x1="15" y1="9" x2="17" y2="8" stroke-width="2px" stroke="#63656E"></line>
            </g>
          </svg>
          <span v-if="tables.length > 1" class="bk-icon icon-close" @click="handelRemoveTable(index)"></span>
        </span>
      </div>
      <bk-table :data="table.data">
        <bk-table-column :label="$t('指标/维度')" prop="dimension">
          <template slot-scope="props">
            <div class="row-header">{{props.row.rowHeader}}</div>
          </template>
        </bk-table-column>
        <bk-table-column label-class-name="require" class-name="small-padding" :label="$t('英文名')" prop="englishName">
          <template slot-scope="props">
            <div class="name">
              <verify-input position="right" :validator="{ content: props.row.nameErrorMsg }" :show-validate.sync="props.row.nameError">
                <bk-input :disabled="Boolean(props.row.sourceName)" v-model.trim="props.row.name" :maxlength="256" :placeholder="$t('英文')" @blur="checkInput(props.row, ['name', 'alias'])" @change="checkInput(props.row, ['name', 'alias'])"></bk-input>
              </verify-input>
              <bk-popover class="change-name" placemnet="top-end" trigger="mouseenter" v-if="!(props.row.name.length || Boolean(props.row.sourceName))">
                <span class="icon-monitor icon-change" @click="rename(props.row)" v-show="props.row.isReservedWord"></span>
                <div slot="content">
                  <p style="margin-bottom: 0; font-size: 12px;"> {{ $t('名字有冲突,可点击该图') }} </p>
                  <p style="margin-bottom: 0; font-size: 12px;"> {{ $t('标转换成非冲突名称') }} </p>
                </div>
              </bk-popover>
            </div>
          </template>
        </bk-table-column>
        <bk-table-column class-name="small-padding" :label="$t('别名')" prop="alias">
          <template slot-scope="props">
            <verify-input position="right" :validator="{ content: props.row.aliasErrorMsg }" :show-validate.sync="props.row.aliasError">
              <bk-input v-model.trim="props.row.alias" :maxlength="128" :placeholder="$t('别名')" @blur="checkInput(props.row, ['name', 'alias'])" @change="checkInput(props.row, ['name', 'alias'])"></bk-input>
            </verify-input>
          </template>
        </bk-table-column>
        <bk-table-column class-name="small-padding" :label="$t('类型')" prop="type">
          <template slot-scope="props">
            <verify-input class="error-icon-right" position="right" v-if="props.row.monitorType === 'metric'" :show-validate.sync="props.row.typeError">
              <bk-select v-model="props.row.type.value" :clearable="false">
                <bk-option v-for="(dataType, typeIndex) in props.row.type.list" :key="typeIndex" :id="dataType.id" :name="dataType.name"></bk-option>
              </bk-select>
            </verify-input>
            <bk-select v-else v-model="props.row.type.value" :clearable="false" :disabled="props.row.monitorType === 'dimension'">
              <bk-option v-for="(dataType, typeIndex) in props.row.type.list" :key="typeIndex" :id="dataType.id" :name="dataType.name"></bk-option>
            </bk-select>
          </template>
        </bk-table-column>
        <bk-table-column class-name="small-padding" :label="$t('单位')" prop="unit">
          <template slot-scope="props">
            <bk-input v-if="props.row.monitorType === 'metric'" v-model="props.row.unit"></bk-input>
        ...
        <bk-table-column :label="$t('启/停')">
          <template slot-scope="props">
            <bk-switcher v-if="props.row.monitorType === 'metric'" v-model="props.row.switch" size="small"></bk-switcher>
            <div v-else style="text-align: center;">
              <span>--</span>
            </div>
          </template>
        ...
        <bk-table-column :label="$t('增/删')">
          <template slot-scope="props">
            <span class="bk-icon icon-plus-circle" @click.stop="handleAdd(props.row.monitorType, table.data, props.$index)"></span>

            <span @click.stop="handleDel(table.data, props.$index, index)" class="bk-icon icon-minus-circle"></span>
          </template>
        </bk-table-column>
      </bk-table>
    </div>
    <div class="add-category" @click="create">
      <span class="bk-icon icon-plus"></span><span> {{ $t('增加指标分类')}} </span>
    </div>
    <bk-dialog
      v-model="isShow"
      :theme="'primary'"
      ...
      <div class="metric-name">
        <div class="hint">
          ...
          <span> {{ $t('指标分类的定义影响指标检索的时候,如试图查看，仪表盘添加视图和添加监控策略时选择指标的分类。') }} </span>
        </div>
        <verify-input :validator="{ content: $t('输入指标名,以字母开头,允许包含下划线和数字') }" :show-validate.sync="metricForm.isNameEmpty">
          <bk-input :placeholder="$t('英文名')" v-model.trim="metricForm.name" @blur="metricForm.isNameEmpty = !regx.test(metricForm.name)"></bk-input>
        </verify-input>
        <verify-input>
          <bk-input :placeholder="$t('别名')" v-model="metricForm.alias"></bk-input>
        </verify-input>
      </div>
      <div class="footer">
        <bk-button class="confirm-btn" theme="primary" @click="handleConfirm" @keyup.enter="handleConfirm"> {{ $t('确认') }} </bk-button><bk-button @click="isShow = false"> {{ $t('取消') }} </bk-button>
      </div>
    </bk-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    insertTableData(tableIndex, tableObj) {
      ...
      if (this.import) {
        ...
        if (tableIndex > -1) {
          this.$bkMessage({ theme: 'error', message: this.$t('指标分类不能同名') })
        } else {
          this.tables.push(tableObj)
        }
      }
    ...
    handleConfirm() {
      if (this.regx.test(this.metricForm.name) && !this.metricForm.isEdit) {
        if (!this.tables.find(item => item.name === this.metricForm.name)
    ...
    rename(row) {
      ...
      if (rows.find(item => item.name === `_${row.name}`)) {
        this.$bkMessage({ theme: 'error', message: this.$t('已存在相同名称的字段，请修改') })
      } else {
      ...
      for (let i = rows.length - 1; i > -1; i--) {
        ...
        for (let k = 0; k < copyRows.length; k++) {
          ...
          if (i !== k && rows[i].name && rows[i].name === copyRows[k].name) {
            if ((rows[i].monitorType === 'metric' && copyRows[k].monitorType === 'metric')
             || tableNameOne === tableNameTwo) {
              hasDuplicateName = true
              rows[i][key1] = true
              rows[i][key2] = this.$t('名字重复')
              copyRows[k][key1] = true
              copyRows[k][key2] = this.$t('名字重复')
            }
          }
        }
      }
      ...
      const rules = {
        name: (val) => {
          ...
          if ((obj.monitorType === 'metric' || obj.alias) || (obj.monitorType === 'dimension' && val)) {
            ...
            if (!regx.test(val)) {
              obj.nameError = true
              obj.nameErrorMsg = this.$t('请输入名称，以字母开头，仅支持字母、下划线和数字')
            } else if (val.length > 100) {
              obj.nameError = true
              obj.nameErrorMsg = this.$t('名字不能超过100个字符')
            }
          }
        ...
        alias: (val) => {
          if (val.length > 100) {
            obj.aliasError = true
            obj.aliasErrorMsg = this.$t('名字不能超过100个字符')
          } else {
    ...
    valiDateTables() {
      ...
      if (this.isExistReservedWord) {
        this.$bkMessage({ theme: 'error', message: this.$t('指标维度表存在冲突名称') })
      }
