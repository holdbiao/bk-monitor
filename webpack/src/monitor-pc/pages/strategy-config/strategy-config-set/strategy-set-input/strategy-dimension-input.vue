<template>
  <div class="strategy-dimension-input">
    ...
    <bk-popover class="help-tips">
      <i class="icon-monitor icon-mc-help-fill" v-if="alarmDimensionList.length"></i>
      <div slot="content">
        <div class="help-text">{{ $t('没找到相关的维度？') }}
          <span class="help-icon" @click="handleGotoLink(paramsMap[monitorType])">
            {{ $t('查看文档') }}
            <i class="icon-monitor icon-mc-wailian"></i>
          </span>
        </div>
      </div>
    </bk-popover>
...
import { deepClone, random, getCookie } from '../../../../../monitor-common/utils/utils'
import documentLinkMixin from '../../../../mixins//documentLinkMixin'


export default {
  name: 'strategy-dimension-input',
  components: {
    SetInput,
    SetAdd
  },
  ...
  computed: {
    ...
    alarmDimensionList() {
      return this.alarmConditionList.filter(item => item.type !== 'add').map(item => ({
        ...item.value
      }))
    }
  },
  watch: {
    dimensionList: {
      handler() {
        const arr = deepClone(this.dimensionList).map(item => ({ ...item, show: !this.dimensions.includes(item.id) }))
  ...
  methods: {
    ...
    getDefaultAdd() {
      ...
      return {
        ...
        addType: this.alarmConditionList.length > 0
            && this.alarmConditionList.some(item => item.type !== 'add') ? 'common' : 'character'
      }
    },
    ...
    handleSetHide(item) {
      ...
      const len = this.alarmConditionList.length
      this.alarmConditionList[this.alarmConditionList.length - 1].show = true
      this.alarmConditionList[len - 1].addType = len > 1
        && this.alarmConditionList.filter(item => item.show).some(item => item.type !== 'add') ? 'common' : 'character'
    },
