...
from alarm_backends.management.story.base import BaseStory, CheckStep, register_step, register_story, Problem
        ...
        for row in cursor.fetchall():
            ...
            if info["auto_increment"] and info["auto_increment"] > self.problem_increased_id_limit:
                cursor.execute(
                    ...
                    where table_name="{info['table']}" and table_schema="{settings.DATABASES["default"]["NAME"]}"
                    and COLUMN_KEY="PRI";
                    """
                )
                for row in cursor.fetchall():
