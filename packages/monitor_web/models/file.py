...
class UploadedFileInfo(OperateRecordModelBase):
    original_filename = models.CharField(_lazy("原始文件名"), max_length=255)
    actual_filename = models.CharField(_lazy("文件名"), max_length=255)
    relative_path = models.TextField(_lazy("文件相对路径"))
    file_data = models.FileField(_lazy("文件内容"), upload_to=generate_upload_path)
    file_md5 = models.CharField(_lazy("文件内容MD5"), max_length=50)
    is_dir = models.BooleanField(_lazy("是否为目录"), default=False)
