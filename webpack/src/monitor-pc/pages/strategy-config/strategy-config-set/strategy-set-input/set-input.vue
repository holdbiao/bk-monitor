<template>
  <div class="set-input-wrapper">
    <div class="set-input"
         v-if="!multiple"
         :class="{ 'is-condition': isCondition || isMethod, 'is-method': isMethod,'input-placeholder': !inputValue.length && !(select[displayKey] + '').length }"
         ...
         @blur="handleSetBlur"
    ></div>
    <div class="set-tag" v-else>
      <bk-tag-input trigger="focus"
    ...
    <div style="display: none;" v-if="!multiple">
      <slot name="list">
        <div class="set-list-wrapper" ref="setList">
          <ul class="set-list" :style="{ 'min-width': listMinWidth + 'px' }">
          ...
          <div class="set-remove" @mousedown="handleRemoveClick" v-if="isKey && (select[displayKey] + '').length">
            <i class="icon-monitor icon-chahao" :class="{ 'set-remove-icon': !isMethod }"></i>
            {{ !isMethod ? $t('删除') : ''}}
          </div>
        </div>
      </slot>
  ...
  methods: {
    ...
    handleSelectChange(v) {
      ...
      if (!this.filterList.length) {
        this.popoverInstance && this.popoverInstance.hide(100)
      } else if (!this.isItemClick && this.popoverInstance
        && !this.popoverInstance.state.isVisible && this.list.length) {
