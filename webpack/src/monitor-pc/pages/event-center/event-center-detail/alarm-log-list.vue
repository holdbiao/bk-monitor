<template>
  <ul class="log-list">
    <li class="log-list-item" v-for="(item, index) in list" :key="item.logId" v-show="item.show">
      <div class="item-title">
        <span class="item-title-set icon-monitor"
              :class="[item.expand ? 'icon-mc-minus-plus' : 'icon-mc-plus-fill']"
              @click="beforeCollapseChange(item, index)"
              v-if="item.collapse"></span>
        <span class="item-title-icon"><i class="icon-monitor" :class="item.logIcon"></i></span>
        ...
        <div class="item-content-desc">
          ...
          <span v-if="item.contents.length === 1">
            <span
              ...
              {{ item.count > 1 ? `${$t('当前事件流水过多，收敛')}${item.count}${$t('条。')}` : (item.contents[0] || '--') }}
            </span>
            <span v-if="item.operate === 'ANOMALY_NOTICE' && item.shieldType === 'saas_config'"
                  class="can-click"
          ...
          <template v-else-if="item.operate === 'ANOMALY_NOTICE'">
            {{item.contents[0]}}
            <span class="notice-group" v-for="text in item.contents[1]" :key="text">{{text}}</span>
            {{item.contents[2]}}
            <span class="notice-status">{{alertStatusMap[item.contents[3]]}}</span>
            <span class="can-click" @click="handleNoticeDetail(item.actionId)"> {{ $t('点击查看明细') }} </span>
          </template>
          ...
          <template v-else-if="item.contents.length > 1 && (item.operate === 'CREATE' || item.operate === 'CONVERGE')">
            <span
              ...
              :key="i"
              v-bk-tooltips.top="i === item.index && item.sourceTime && (item.operate === 'CREATE' || item.operate === 'CONVERGE') ? `${$t('数据时间：')}${item.sourceTime}` : ''"
              :class="{ 'tip-dashed': i === item.index && item.sourceTime && (item.operate === 'CREATE' || item.operate === 'CONVERGE') }">
              {{content || '--'}}
            </span>
          </template>
        </div>
        <div class="item-border" :style="{ borderColor: item.border ? '#979BA5' : '#DCDEE5' }"></div>
      </div>
    </li>
    <li class="log-list-loading" v-show="loading">
      <img class="loading-img" src="../../../static/images/svg/spinner.svg" /> {{ $t('正加载更多内容…') }} </li>
  </ul>
</template>
<script>
