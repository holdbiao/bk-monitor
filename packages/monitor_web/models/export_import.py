...
from monitor_web.export_import.constant import ConfigType, ImportDetailStatus, ImportHistoryStatus
...
class ImportHistory(OperateRecordModelBase):
    ...
    status = models.CharField(max_length=100, choices=STATUS_TYPE_CHOICES, verbose_name=_lazy("导入状态"))
...
class ImportDetail(OperateRecordModelBase):
    ...
    name = models.CharField(max_length=100, verbose_name=_lazy("配置名称"))
    type = models.CharField(max_length=100, choices=TYPE_CHOICES, verbose_name=_lazy("配置类型"))
    label = models.CharField(max_length=100, blank=True, verbose_name=_lazy("标签"))
    history_id = models.IntegerField(verbose_name=_lazy("对应的导入历史ID"))
    config_id = models.CharField(max_length=100, null=True, verbose_name=_lazy("生成的配置ID"))
    import_status = models.CharField(max_length=100, choices=IMPORT_STATUS_CHOICES, verbose_name=_lazy("导入状态"))
    error_msg = models.TextField(blank=True, verbose_name=_lazy("文件错误信息"))
    parse_id = models.IntegerField(verbose_name=_lazy("解析ID"))
...
class ImportParse(OperateRecordModelBase):
    ...
    type = models.CharField(max_length=100, choices=TYPE_CHOICES, verbose_name=_lazy("配置类型"))
    label = models.CharField(max_length=100, blank=True, verbose_name=_lazy("标签"))
    uuid = models.CharField(max_length=100, verbose_name=_lazy("文件解析内容对应uuid"))
    file_status = models.CharField(max_length=100, choices=FILE_STATUS_CHOICES, verbose_name=_lazy("是否为正确配置"))
    error_msg = models.TextField(blank=True, verbose_name=_lazy("文件错误信息"))
    file_id = models.IntegerField(verbose_name=_lazy("文件ID"))
    config = JsonField(null=True, verbose_name=_lazy("配置详情"))
