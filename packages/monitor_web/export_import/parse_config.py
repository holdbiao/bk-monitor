...
from monitor_web.models import CollectorPluginMeta, Signature, CollectConfigMeta
from monitor_web.plugin.manager import PluginManagerFactory
from monitor_web.strategies.resources import BackendStrategyConfigResource
...
class StrategyConfigParse(BaseParse):
    def check_msg(self):
        ...
        serializers = BackendStrategyConfigResource.RequestSerializer(data=self.file_content)
        try:
        ...
        action_list = return_data["config"]["action_list"]
        for action_detail in action_list:
            for notice_detail in action_detail.get("notice_group_list", []):
                if notice_detail.get("name") is None:
                    return_data.update(file_status=ImportDetailStatus.FAILED, error_msg=_("缺少通知组名称"))

        agg_condition = return_data["config"]["item_list"][0].get("rt_query_config", {}).get("agg_condition", [])
