...
//     '/api/'
// ]
const setCache = (cacheName, request) => caches.open(cacheName).then(cache => cache.match(request).then(res => res || fetch(request).then((res) => {
...
self.addEventListener('install', (e) => {
  const promiseList = []
  const cacheJsUtil = caches.open(cacheJsName).then(cache => cache.addAll(self.assetData.filter(url => url.match(/\.js$/))))
  promiseList.push(cacheJsUtil)
  const cacheCssUtil = caches.open(cacheCssName).then(cache => cache.addAll(self.assetData.filter(url => url.match(/\.css$/))))
  promiseList.push(cacheCssUtil)
  const cacheFontUtil = caches.open(cacheFontName).then(cache => cache.addAll(self.assetData.filter(url => url.match(/\.(ttf|woff|eot)$/))))
  promiseList.push(cacheFontUtil)
  const cacheImgUtil = caches.open(cacheImgName).then(cache => cache.addAll(self.assetData.filter(url => url.match(/\.(png|jpe?g|gif|svg)$/))))
...
self.addEventListener('fetch', (e) => {
  ...
  const needCache = requestUrl.match(/\.(css|js|ttf|woff|eot|png|jpe?g|gif|svg)$/) && self.assetData.some(url => requestUrl.includes(url))
  if (needCache) {
