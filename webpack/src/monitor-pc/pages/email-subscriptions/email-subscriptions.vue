<template>
  <div class="email-subscriptions-wrap">
    ...
    <list-collapse
      ...
      <span class="header-btn" slot="header-btn" @click.stop="handleRouterTo('email-subscriptions-add')">
        <span class="icon-monitor icon-mc-plus-fill"></span>
        <span>{{$t('新增订阅')}}</span>
      </span>
      <div slot="content" class="list-content">
        <bk-table
          v-bkloading="{ isLoading: subscribedLoading }"
              ...
              <template slot-scope="scope">
                <div class="receivers-status">
                  ...
                  <template v-else>
                    ...
                    <i class="icon-monitor icon-audit" @click="handleReceiversList($event, scope.row)"></i>
                  </template>
                </div>
              </template>
            </bk-table-column>
            ...
            <template slot-scope="scope">
              <div class="btn-wrap">
                <bk-button
                  ...
                  title="primary"
                  :disabled="isPermissionDenied(scope.row)"
                  @click="handleToEdit(scope.row)">{{$t('编辑')}}</bk-button>
                <bk-button
                  v-show="isAllowSubscriptions(scope.row)"
                  :text="true"
                  title="primary"
                  @click="handleSubscriptions(scope.row, true)">{{$t('订阅')}}</bk-button>
                <bk-button
                  v-show="!isAllowSubscriptions(scope.row)"
                  :text="true"
                  title="primary"
                  @click="handleSubscriptions(scope.row, false)">{{$t('取消订阅')}}</bk-button>
                <bk-button
                  :text="true"
                  title="primary"
                  :disabled="isPermissionDenied(scope.row)"
                  @click="handleDele(scope.row, scope.$index)">{{$t('删除')}}</bk-button>
                <bk-button
                  :text="true"
                  title="primary"
                  :disabled="isPermissionDenied(scope.row)"
                  @click="handleToClone(scope.row)">{{$t('克隆')}}</bk-button>
              </div>
            </template>
          </bk-table-column>
        </bk-table>
        ...
        <bk-table
          ...
          <template v-for="(item, index) in sendListColumnsMap">
            <bk-table-column v-if="item.key === 'isSuccess'" :key="index" :label="$t('发送状态')" prop="isSuccess">
              <template slot-scope="scope">
                <span :class="scope.row.isSuccess ? 'is-success' : 'is-fail'">{{scope.row.isSuccess ? $t('成功') : $t('失败')}}</span>
              </template>
            </bk-table-column>
            <bk-table-column
              v-else :key="index"
              :label="item.label"
              :prop="item.key"
              :formatter="item.formatter"></bk-table-column>
          </template>
        </bk-table>
      </div>
    </list-collapse>
...
import { transformDataKey, deepClone } from '../../../monitor-common/utils/utils'
// import { getReceiver } from '../../../monitor-api/modules/notice_group'
import ReceiverList from './components/receiver-list.vue'

const { i18n } = window
const frequencyMap: string[] = [, i18n.t('仅一次'), i18n.t('每天'), i18n.t('每周'), i18n.t('每月')]
// let groupList: any = []
/**
 * 邮件订阅列表页
 */
...
export default class EmailSubscriptions extends Vue {
  ...
  private subListColumnsMap: ITableColumnItem[] = [
    ...
    { label: i18n.t('邮件标题'), key: 'mailTitle' },
    { label: i18n.t('发送频率'), key: 'frequency', width: 100,
      formatter: row => frequencyMap[row.frequency.type]
    },
    { label: i18n.t('发送时间'), key: 'lastSendTime',
      formatter: (row) => {
        const weekMap = [
          i18n.t('周一'),
          i18n.t('周二'),
    ...
    {
      label: i18n.t('订阅状态'), key: 'receivers'
    },
    { label: i18n.t('启用/停止'), key: 'isEnabled' }
  ]
  ...
  private managerFormatter(row) {
    return row.managers.filter(item => !item.group).map(item => (item.type === 'group'
   ...
   */
  private getReceiver() {
    return groupList({ bk_biz_id: +window.cc_biz_id > 0 ? null : 0 }).then((res) => {
      this.groupList = res
    })
  }
  ...
  private handleSubscriptions(row, bool) {
    ...
    let params = {
      reportItemId: row.id,
      receivers: curReceivers ? row.receivers.map((item) => {
    ...
    this.$bkInfo({
      title: this.$t(bool ? '恢复订阅' : '取消订阅该邮件，且不再显示？'),
      confirmLoading: true,
      confirmFn: async () => {
        try {
  ...
  private handleToClone(row) {
    ...
    reportClone({ report_item_id: row.id }).then(() => {
