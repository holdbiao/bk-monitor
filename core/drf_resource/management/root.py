...
自动发现项目下resource和adapter和api
    ...
    使用:
        ...
        # resource: 基于业务逻辑的封装
        resource.plugin -> plugin/resource.py
...
def install_resource(rs_path):
    ...
    for p in dotted_path.split("."):
        if isinstance(_resource, ResourceShortcut):
            logger.info("ignored: {}".format(dotted_path))
            rs_path.ignored()
            return
...
def install_adapter(rs_path):
    ...
    if is_api(dotted_path):
        ...
        rs, ada = dotted_path[(len(API_DIR) + 1) :].split(".", 1)
    ...
    if not ada.startswith(settings.PLATFORM):
        logger.info("unsupported: {}".format(dotted_path))
        rs_path.ignored()
        return
