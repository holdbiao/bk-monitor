...
def refresh_agent_status_by_biz_id(cc_biz_id):
    """
    定时缓存agent状态信息
    """
    set_client_user(business.maintainer(cc_biz_id))
    resource.cc.raw_hosts.refresh(cc_biz_id)


@task(ignore_resule=True)
@task_decorator(random_range=5, task_interval=30)
def refresh_agent_data_report_status_by_biz_id(cc_biz_id):
    """
    定时缓存agent数据上报状态
    """
    set_client_user(business.maintainer(cc_biz_id))
    ...
    host_index_backend.data_report_info.refresh(cc_biz_id)
...
def refresh_host_status_cache(biz_id=None):
    ...
    for biz_id in biz_id_set:
        refresh_agent_status_by_biz_id.apply_async(args=(biz_id,), expires=DEFAULT_TASK_EXPIRES)
        refresh_agent_data_report_status_by_biz_id.apply_async(args=(biz_id,), expires=DEFAULT_TASK_EXPIRES)
