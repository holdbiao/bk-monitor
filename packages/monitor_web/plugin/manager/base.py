...
from monitor_web.models.plugin import CollectorPluginConfig, CollectorPluginInfo, PluginVersionHistory
...
class PluginManager(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def update_metric(self, data):
        ...
        if old_metric_md5 != new_metric_md5:
            is_change = True
                ...
                for os_type, file_list in list(add_files.items()):
                    ...
                    for index, config in enumerate(context["config_json"]):
                        if config.get("type") == "file":
                            with open(
                                os.path.join(dest_dir, "etc", "{{file" + str(index + 1) + "}}.tpl"), "w"
                            ) as template_file:
                                template_file.write("{{file" + str(index + 1) + "_content}}")
