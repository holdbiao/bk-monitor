...
from constants.strategy import TargetFieldType, TargetMethodList
...
class Ipserializer(serializers.Serializer):
    ip = serializers.IPAddressField(required=True, label=_("IP地址"))
    bk_cloud_id = serializers.IntegerField(required=True, label=_("云区域ID"))
    # bk_supplier_id = serializers.IntegerField(required=True, label=_("供应商ID"))


class TopoSerializer(serializers.Serializer):
    bk_inst_id = serializers.IntegerField(required=True, label=_("节点ID"))
    bk_obj_id = serializers.CharField(required=True, label=_("节点层级"))


class HostTargetIpSerializer(serializers.Serializer):
    bk_target_ip = serializers.IPAddressField(required=True, label=_("目标ip地址"))
    bk_target_cloud_id = serializers.IntegerField(required=True, label=_("目标云区域ID"))


class TemplateSerializer(serializers.Serializer):
    bk_inst_id = serializers.IntegerField(required=True, label=_("模板id"))
    bk_obj_id = serializers.CharField(required=True, label=_("模板对象名"))
...
def is_validate_target(value):
    ...
    for value_msg in value:
        if not isinstance(value_msg, list):
            return ValidationError(_("target结构不符合要求"))
        for target_value in value_msg:
            target_value_is_validate(target_value)


def target_value_is_validate(target_value):
    error_list = []
    if not isinstance(target_value, dict):
        raise ValidationError(_("target结构不符合要求"))
    if not target_value.get("field"):
        error_list.append({"field": _("target需要field字段且不能为空")})
    # if target_value.get("field") not in TargetFieldList:
    #     error_list.append({"field": _("target需要field字段且为%s") % ("、".join(TargetFieldList))})
    if target_value.get("method") not in TargetMethodList:
        error_list.append({"method": _("target需要method字段且为%s") % ("、".join(TargetMethodList))})
    if not all([target_value.get("value"), isinstance(target_value.get("value"), list)]):
        error_list.append({"value": _("target需要value字段且不能为空且为列表")})

    if error_list:
        raise ValidationError(error_list)

    # 校验target中的传值格式
    if target_value.get("field") == TargetFieldType.host_ip:
        for real_value in target_value.get("value"):
            serializer_obj = Ipserializer(data=real_value)
            serializer_obj.is_valid(raise_exception=True)
    elif target_value.get("field") == TargetFieldType.host_target_ip:
        for real_value in target_value.get("value"):
            serializer_obj = HostTargetIpSerializer(data=real_value)
            serializer_obj.is_valid(raise_exception=True)
    elif target_value.get("field") in [TargetFieldType.host_topo, TargetFieldType.service_topo]:
        for real_value in target_value.get("value"):
            serializer_obj = TopoSerializer(data=real_value)
            serializer_obj.is_valid(raise_exception=True)
    elif target_value.get("field") in [TargetFieldType.host_topo, TargetFieldType.service_topo]:
        for real_value in target_value.get("value"):
            serializer_obj = TopoSerializer(data=real_value)
            serializer_obj.is_valid(raise_exception=True)
    elif target_value.get("field") in [
        ...
        for real_value in target_value.get("value"):
            serializer_obj = TemplateSerializer(data=real_value)
            serializer_obj.is_valid(raise_exception=True)
    elif target_value.get("value"):
        if not isinstance(target_value["value"][0], dict):
            # 如果不是字典，说明不是内置数据结构，跳过校验
            return


def handel_target(value):
    for value_list in value:
        for value_detail in value_list:
            if value_detail["field"] == TargetFieldType.host_ip and isinstance(value_detail["value"], list):
                ...
                value_detail["value"] = [
                    {"bk_target_ip": x["ip"], "bk_target_cloud_id": x["bk_cloud_id"]} for x in value_detail["value"]
                ]
        ...
        for algorithm_key, algorithm_value in list(value_msg.items()):
            if algorithm_key in DETECT_ALGORITHM_INT_LIST:
                ...
                if algorithm_value is None:
                    continue
                try:
                    value_msg[algorithm_key] = round(float(algorithm_value), 4) if str(algorithm_value) else None
                except (ValueError, TypeError):
                    raise ValidationError(_("检测算法不符合要求，周期为正整数，值为正数"))
