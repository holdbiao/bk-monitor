...
def check_permission(obj, request_cc_biz_id):
    ...
    if cc_biz_id and int(cc_biz_id) != int(request_cc_biz_id):
        logger.exception(
            _("权限不通过！ 当前请求的业务ID为%s，而对象[%s](%s)所属业务ID为%s")
...
def check_rt_reserved_word(word):
    ...
    :return 是否通过校验
    """
    from monitor.constants import RT_RESERVED_WORD_EXACT, RT_RESERVED_WORD_FUZZY
...
def fetch_biz_id_from_dict(data, default=None):
    ...
    from monitor.constants import BIZ_ID_FIELD_NAMES
