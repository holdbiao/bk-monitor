...
@pytest.fixture()
def redis_mocked(mock):
    redis = mock.patch("alarm_backends.service.action.message_queue.client.redis")
    redis_mocker = mock.MagicMock()
    redis.Redis.return_value = redis_mocker
    return redis_mocker
...
class TestRedisClient(object):
    ...
    def test_send(self, setup, redis_mocked):
        settings.ENABLE_MESSAGE_QUEUE = True
        settings.MESSAGE_QUEUE_DSN = "redis://:123456@locahost:3307/1/actions"
        processor = MessageQueueProcessor(setup.id)
        processor.execute()

        event = Event.objects.get(event_id=setup.event_id)
        assert redis_mocked.lpush.call_count == 1
        assert redis_mocked.lpush.call_args[0][0] == "actions"
        result = json.loads(redis_mocked.lpush.call_args[0][1])
        assert result["event"]["id"] == event.id

    def test_no_password(self, setup, redis_mocked):
        settings.ENABLE_MESSAGE_QUEUE = True
        settings.MESSAGE_QUEUE_DSN = "redis://locahost:3307/1/actions"
        processor = MessageQueueProcessor(setup.id)
        processor.execute()

    def test_error(self, setup):
        settings.MESSAGE_QUEUE_DSN = "redis://:123456@locahost:3307/w/actions"
        try:
            MessageQueueProcessor(setup.id)
