...
from bkmonitor.utils.db.fields import JsonField, AESJsonField
...
class CollectConfigMeta(OperateRecordModelBase):
    ...
    name = models.CharField(_lazy("配置名称"), max_length=128)

    # 采集插件相关配置
    collect_type = models.CharField(_lazy("采集方式"), max_length=32, choices=COLLECT_TYPE_CHOICES, db_index=True)
    plugin = models.ForeignKey(CollectorPluginMeta, verbose_name=_lazy("关联插件"), related_name="collect_configs")
    ...
    # HOST                   TOPO                 业务拓扑
    # HOST                   INSTANCE             主机实例
    target_object_type = models.CharField(_lazy("采集对象类型"), max_length=32, choices=TARGET_OBJECT_TYPE_CHOICES)

    deployment_config = models.ForeignKey("DeploymentConfigVersion", verbose_name=_lazy("当前的部署配置"))

    cache_data = JsonField(_lazy("缓存数据"), default=None)

    last_operation = models.CharField(_lazy("最近一次操作"), max_length=32, choices=OPERATION_TYPE_CHOICES)
    operation_result = models.CharField(_lazy("最近一次任务结果"), max_length=32, choices=OPERATION_RESULT_CHOICES)
    ...
    def task_status(self):
        ...
        if self.operation_result == OperationResult.PREPARING:
            ...
            if cache_data:
                if (
                    cache_data.get("error_instance_count") is not None
    ...
    def need_upgrade(self):
        ...
        if self.task_status == TaskStatus.STOPPED or self.get_cache_data("total_instance_count", 0) == 0:
            ...
            return self.deployment_config.plugin_version.config_version < self.plugin.release_version.config_version
    ...
    def switch_config_version(self, target_deployment_config):
        ...
        if operate_type == "rebuild":
            with transaction.atomic():
                ...
                self.switch_subscription("disable")
                if prev_sub_id != 0:
    ...
    def update_subscription(self, target_deployment_config):
        ...
        for step in deploy_params.get("steps", []):
            update_params["steps"].append({"id": step["id"], "params": step["params"]})
...
class DeploymentConfigVersion(OperateRecordModelBase):
    ...
    plugin_version = models.ForeignKey(
        PluginVersionHistory, verbose_name=_lazy("关联插件版本"), related_name="deployment_versions"
    )

    parent_id = models.IntegerField(_lazy("父配置ID"), default=None, null=True)
    config_meta_id = models.IntegerField(_lazy("所属采集配置ID"))
    subscription_id = models.IntegerField(_lazy("节点管理订阅ID"), default=0)

    target_node_type = models.CharField(_lazy("采集目标类型"), max_length=32, choices=TARGET_NODE_TYPE_CHOICES)
    ...
    #     }
    # }
    params = AESJsonField(_lazy("采集参数配置"), default=None)
    ...
    #     }
    # ]
    target_nodes = JsonField(_lazy("采集目标节点"), default=[])
    ...
    # }
    remote_collecting_host = JsonField(_lazy("远程采集机器"), default=None)
    task_ids = JsonField(_lazy("任务id列表"), default=None)
    ...
    def metrics(self):
        ...
        获取表结构
        """
        last_version = self.plugin_version.plugin.get_release_version_by_config_version(
            self.plugin_version.config_version
        )
