<template>
  <article class="import-config" v-bkloading="{ isLoading: loading }">
    <!--按钮组（统计信息）-->
    <section class="import-config-tag" v-show="historyId">
      <div class="config-tag">
        ...
        <bk-button
          ...
          <span class="icon-monitor icon-mc-retry"></span> {{ $t('失败批量重试') }} </bk-button>
      </div>
    </section>
    <!--折叠内容-->
        ...
        <bk-collapse-item
          ...
          <template #default>
            ...
            <div class="collapse-item-right" v-show="table.statistics && table.statistics[item.name]">
              ...
              <span v-html="getStatusHtml(item.name)"></span>
            </div>
          </template>
          <!--折叠表格-->
          <template #content>
              ...
              <bk-table-column :label="$t('任务状态')" width="153" :render-header="renderHeader">
                <template #default="{ row }">
                  <div class="status-col" v-if="statusMap[row.status]">
                    <span v-if="row.status === 'importing'" class="status-runing icon-monitor icon-loading"></span>
                    <span :class="'status-' + row.status" v-else></span>
                ...
                <template #default="{ row }">
                  <div class="detail-col">
                    ...
                    <bk-button
                      ...
                      text
                      @click="handleRetry(row.uuid)"> {{ $t('重试') }} </bk-button>
                  </div>
                </template>
              </bk-table-column>
            </bk-table>
...
export default {
  ...
  computed: {
    disabledBatchRetryBtn() {
      return this.table.runingQueue.length !== 0
            || this.table.list.filter(item => item.status === 'failed').length === 0
    },
    disabledConfirmBtn() {
      ...
      const hasRepeat = {}
      return this.table.list && !this.table.list.every((item, index) => {
        if (index === 0) {
  ...
  methods: {
    ...
    handleChangeStatus(data) {
      if (!data || !data.configList) return
    ...
    async handleBatchRetry() {
      ...
      // 获取失败任务ID并设置任务状态
      const uuids = this.table.list.filter(item => item.status === 'failed').map((item) => {
    ...
    getItemTitle(item) {
      if (this.table.statistics[item.name] && this.table.statistics[item.name].total) {
        ...
        return `${item.title}（${this.id ? this.$t('共') : this.$t('已选')}${total}${this.$t('个')}）`
      }
      return `${item.title}`
    },
    getStatusHtml(collapseName) {
      const statusHtml = []
