<template>
  <div ref="stepPlugin" class="step-plugin" v-bkloading="{ 'isLoading': data.isEdit ? pluginLoading : false }">
    <div class="step-plugin-item">
      <div class="item-label item-required"> {{ $t('所属业务') }} </div>
      <div class="item-container">
        <bk-select class="biz" :disabled="isPublicPlugin || data.isEdit || ccBizId !== 0" :clearable="false" v-model="bizList.value">
          <bk-option v-for="(biz, index) in bizList.list" :key="index" :id="biz.id" :name="biz.text"></bk-option>
        </bk-select>
        <div class="plugin-logo">
          <logo :logo.sync="pluginBasicInfo.logo"></logo>
        </div>
      </div>
    </div>
    <div class="step-plugin-item">
      <div class="item-label item-required"> {{ $t('插件ID') }} </div>
      <div class="item-container plugin-id">
        <verify-input :show-validate.sync="pluginBasicInfo.isNameLegal" :validator="{ content: pluginBasicInfo.nameErrorMsg }">
          <bk-input :placeholder="$t('英文')" :disabled="disabledPluginId" class="item-input" v-model="pluginBasicInfo.name" @blur="checkPluginId"></bk-input>
        </verify-input>
        <bk-checkbox
          ...
          @click.native="!authority.MANAGE_PUBLIC_AUTH && handleShowAuthorityDetail(pluginManageAuth.MANAGE_PUBLIC_AUTH)"
    ...
    <div class="step-plugin-item">
      ...
      <div class="item-container">
        <bk-input :placeholder="$t('别名')" class="item-input" v-model="pluginBasicInfo.alias"></bk-input>
      </div>
    </div>
    <div class="step-plugin-item plugin-type">
      <div class="item-label item-required"> {{ $t('插件类型') }} </div>
      <div class="item-container" ref="containerWidth">
        <div class="bk-button-group">
          <bk-button
            ...
            v-for="(pluginType, index) in pluginBasicInfo.type.list" :key="index"
            :class="{ 'is-selected': pluginType.id === pluginBasicInfo.type.value }"
            @click="handlePluginChange(pluginType.id)">
            {{pluginType.name}}
          </bk-button>
        </div>
        <div class="description" v-if="pluginTypeDes">
          {{pluginTypeDes}}
          <span class="doc-link" @click="handleGotoLink(pluginBasicInfo.type.value)">{{$t('前往文档中心')}}  <span class="icon-monitor icon-mc-link"></span></span>
        </div>
      </div>
    </div>
    <div class="step-plugin-item label-bottom upload-package" v-show="['Exporter', 'DataDog'].includes(pluginBasicInfo.type.value)">
      <div class="item-label item-required">{{$t('上传内容')}}</div>
      <div class="item-container">
        <div class="upload-container" style="margin: -8px 0 0 -10px;">
          <div class="upload-item" v-for="(system, index) in systemTabs.list" :key="`${index}-Exporter`" v-show="pluginBasicInfo.type.value === 'Exporter'">
            <mo-upload :ref="`uploadFile-exporter-${index}`"
                       :collector="pluginBasicInfo.exporterCollector[system.name]" :is-edit="data.isEdit"
                       :plugin-id="pluginBasicInfo.name" :system="system.name" :plugin-type="pluginBasicInfo.type.value">
            </mo-upload>
          </div>
          <div class="upload-item" v-for="(system, index) in systemTabs.list" :key="`${index}-DataDog`" v-show="pluginBasicInfo.type.value === 'DataDog'">
            <mo-upload :ref="`uploadFile-datadog-${index}`"
                       @change="getCheckNameChange"
                       @yaml="getYaml"
                       :collector="pluginBasicInfo.dataDogCollector[system.name]" :is-edit="data.isEdit"
                       :plugin-id="pluginBasicInfo.name" :system="system.name" :plugin-type="pluginBasicInfo.type.value">
            </mo-upload>
          </div>
        </div>
    ...
    <div class="step-plugin-item label-bottom" v-show="['Script', 'JMX', 'DataDog'].includes(pluginBasicInfo.type.value)">
      <div class="item-label item-required">{{$t('采集配置')}}</div>
    ...
    <div v-show="pluginBasicInfo.type.value === 'Exporter'" class="step-plugin-item">
      <div class="item-label item-required"> {{ $t('绑定端口') }} </div>
      <div class="item-container">
        <verify-input :show-validate.sync="pluginBasicInfo.isPortLegal" :validator="{ content: $t('请输入合法端口') }" class="run-port">
          <param-card
            title="${port}"
    ...
    <div class="step-plugin-item" v-show="pluginBasicInfo.type.value === 'SNMP'">
      <div class="item-label item-required">{{$t('SNMP版本')}}</div>
      <div class="item-container editor-wrapper">
        <bk-select v-model="pluginBasicInfo.snmpVersion" :clearable="false">
          <bk-option
            v-for="i in 3"
            :key="i"
            :name="`v${i}`"
    ...
    <div ref="itemParam" :class="['step-plugin-item', 'define-param', hasDefineParam ? 'has-param' : '', pluginBasicInfo.type.value === 'Exporter' ? 'plugin-exporter' : '']">
      <div :class="{ 'item-label': true, 'param-label': true, 'item-required': pluginBasicInfo.type.value === 'Exporter' }"> {{ $t('定义参数') }} </div>
      <div class="item-container">
        <div class="item-param" v-for="(param, index) in params" :key="index" @click="handleEditParam(param, index)">
          <div class="wrapper">
            <span class="param-name">{{['JMX', 'Pushgateway'].includes(pluginBasicInfo.type.value) ? param.description : param.name}}</span>
            <span v-show="!param.disabled" class="bk-icon icon-close-circle-shape" @click.stop="handleDeleteParam(index)"></span>
          </div>
        </div>
        <template v-if="!['JMX', 'Pushgateway', 'SNMP'].includes(pluginBasicInfo.type.value)">
          <span
    ...
    <div v-show="pluginBasicInfo.type.value === 'Exporter'" class="step-plugin-item">
      ...
      <div class="item-container">
        <p class="exporter-tips">{{$t('Exporter的类型插件需要使用${host} ${port}来定义启动参数。 如 --listen=${host}:${port}')}}</p>
      </div>
    </div>
    <div class="step-plugin-item">
      <div class="item-label item-required"> {{ $t('分类') }} </div>
      <div class="item-container">
        <verify-input :show-validate="pluginBasicInfo.noLabel" :validator="{ content: $t('请选择分类') }">
          <bk-select :clearable="false" v-model="pluginBasicInfo.label.value" @change="validateLabel">
            <bk-option-group
    ...
    <div class="step-plugin-item remote-collector">
      ...
      <div class="item-container">
        <bk-switcher v-model="remoteCollector" size="small"></bk-switcher><span>{{$t('支持远程采集')}}</span>
        <span class="description">{{$t('（功能开启并保存成功后，将无法关闭，请确认是否支持远程采集后，谨慎开启！）')}}</span>
      </div>
    </div>
    <div class="step-plugin-item" style="align-items: normal;">
      <div class="item-label"> {{ $t('描述') }} </div>
      <div class="item-container">
        <div class="markdown-editor">
          <editor v-model="pluginBasicInfo.desc" mode="wysiwyg"></editor>
        </div>
      </div>
    </div>
    ...
    <config-param :param-list="params" :plugin-type="pluginBasicInfo.type.value" :show.sync="configParam.show" :param="configParam.value" @confirm="handleConfirm"></config-param>
    <polling-loading :status="registerDialog.status" :show.sync="registerDialog.show"></polling-loading>
    <bk-dialog
      :close-icon="false"
      ...
      <div class="change-description">
        ...
        <bk-input type="textarea" v-model="pluginChangeMsg.msg"></bk-input>
      </div>
      <div slot="footer">
        <bk-button theme="primary" :title="$t('确认')" @click="handleUpdateDesc" @keyup.enter="handleUpdateDesc"> {{ $t('确定') }} </bk-button>
        <bk-button @click="handleCancelDesc" style="margin-left: 8px;"> {{ $t('取消') }} </bk-button>
      </div>
    </bk-dialog>
  </div>
</template>
...
import { retrieveCollectorPlugin, checkIdCollectorPlugin, createCollectorPlugin,
  editCollectorPlugin } from '../../../../../monitor-api/modules/model'
  ...
  data() {
    ...
    return {
      ...
      pluginBasicInfo: {
        ...
        relatedConfCount: 0,
        bindUrl: '127.0.0.1',
        nameErrorMsg: this.$t('请输入插件名称，仅以字母开头，仅支持字母、下划线和数字'),
        alias: '',
        logo: '',
        type: {
          list: [
        ...
        jmxParams: [
          {
            ...
            type: 'text',
            default: '',
            description: this.$t('监听端口'), disabled: true
          },
          {
            name: 'jmx_url',
            mode: 'opt_cmd',
            type: 'text',
            default: '',
            description: this.$t('连接字符串'), disabled: true
          },
          {
            name: 'username',
            mode: 'opt_cmd',
            type: 'text',
            default: '',
            description: this.$t('用户名'), disabled: true
          },
          {
            name: 'password',
            mode: 'opt_cmd',
            type: 'password',
            default: '',
            description: this.$t('密码'), disabled: true
          }
        ],
        dataDogParams: [
          {
            ...
            mode: 'collector',
            default: 'python',
            description: 'Python 程序路径', disabled: true
          }
        ],
        pushGatewayParams: [
          {
      ...
      portRgx: /^([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-5]{2}[0-3][0-5])$/,
  ...
  computed: {
    ...
    params() {
      ...
      const param = {
        ...
        Pushgateway: info.pushGatewayParams,
...
${this.$t('采集器将定期访问 http://127.0.0.1/server-status 以获取Apache的指标数据')}。`
  ...
  watch: {
    'authority.MANAGE_PUBLIC_AUTH': {
      handler(v) {
        if (!this.data.isEdit && v) {
          this.handlePublicPluginChange(true)
        }
      }
    }
  },
  async created() {
    ...
    const resultList = await Promise.all([
      this.getOsList(),
      this.getLabels(),
      this.getReservedWords()]).catch(() => false)
    if (resultList) {
  ...
  methods: {
    ...
    handleDeleteParam(index) {
      this.pluginBasicInfo.params.splice(index, 1)
    },
    /**
             * @description 切换插件
             * @param { String } val
    ...
    async checkPluginId() {
      ...
      if (pluginBasicInfo.name && !pluginBasicInfo.isNameLagal) {
        await checkIdCollectorPlugin({ plugin_id: this.pluginBasicInfo.name.trim() }).catch(() => {
          this.pluginBasicInfo.isNameLegal = true
          this.pluginBasicInfo.nameErrorMsg = this.$t('名字重复')
        })
      } else if (!this.pluginBasicInfo.name) {
        this.pluginBasicInfo.nameErrorMsg = this.$t('请输入插件名称，仅以字母开头，仅支持字母、下划线和数字, 长度不能超过50个字符')
        this.pluginBasicInfo.isNameLegal = true
      }
    ...
    /**
         * @description 编辑参数
         */
    handleEditParam(param, index) {
      this.configParam.value = param
      this.configParam.index = index
      this.configParam.show = true
    ...
    async getPluginInfo() {
      ...
      if (result) {
        ...
        const msg = `${this.$t('上次变更，版本号')}${result.config_version}.${result.info_version},
                        ${this.$t('修改人')}:${result.update_user},${this.$t('修改时间')}:${result.update_time}`
    ...
    backFillData(plugin) {
      ...
      this.bizList.value = plugin.bk_biz_id === undefined || !this.$store.getters.isSuperUser
        ? this.$store.getters.bizId : plugin.bk_biz_id
      ...
      if (plugin.plugin_type === 'Exporter') {
        ...
        this.pluginMonaco.selectedSystemList = Object.keys(plugin.collector_json)
          .filter(item => this.systemTabs.list.some(sys => sys.name === item))
      ...
      plugin.config_json.forEach((item) => {
        if (typeof item.visible === 'undefined' && (plugin.plugin_type !== 'DataDog' || item.name !== 'python_path')) {
    ...
    generationPluginParams() {
      ...
      const params = {
        ...
        plugin_display_name: pluginBasicInfo.alias ? pluginBasicInfo.alias : '',
    ...
    getConfigParam(params, pluginBasicInfo) {
      ...
      if (params.plugin_type === 'JMX') {
        ...
        params.config_json = this.deleteDisabled(pluginBasicInfo.pushGatewayParams)
    ...
    getUploadFile(yaml) {
      ...
      this.systemTabs.list.forEach((system, index) => {
        ...
        const { fileDesc } = this.$refs[componentName][0]
        if (fileDesc) {
          hasfile = true
          ...
          pluginRegister(params, { isAsync: true }).then((data) => {
            polling(data, (data) => {
              if (data.is_completed && data.state === 'SUCCESS') {
                ...
                this.registerDialog.status.msg = this.$t('插件包生成失败')
            ...
            .catch((err) => {
              this.registerDialog.status.msg = this.$t('插件包生成失败')
              this.registerDialog.status.failMsg = err.message || this.$t('插件包生成失败')
              reject(err)
            })
        } else {
    ...
    createPlugin() {
      ...
      return ajax(...params).then((data) => {
        ...
        if (this.data.isEdit && (!this.pluginData.sameConfig || !this.pluginData.sameInfo)
            && this.pluginBasicInfo.stage === 'release' && this.pluginChangeMsg.isChange) {
          this.pluginChangeMsg.show = true
          // 编辑关键选项，返回reject，阻止注册，让用户选填修改信息，等再次确认时注册插件
          return Promise.reject(data)
        }
        this.registerDialog.status.msg = this.$t('插件信息保存中')
    ...
    getPluginVersion() {
      ...
      this.pluginData.is_completed = this.pluginData.sameConfig
        && this.pluginBasicInfo.stage === 'release'// true表示插件不需要注册
    },
    /**
    ...
    validateRequired(key, empty) {
      ...
      if (key === 'name') {
        ...
        if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(name)) {
          ...
          this.pluginBasicInfo.nameErrorMsg = this.$t('输入超过50字符，禁止输入。')
          this.pluginBasicInfo[empty] = true
        }
      } else {
    ...
    validateUploadFile() {
      ...
      if (!hasCollectorJson) {
        const timer = setTimeout(() => {
          this.bkMessage('error', this.$t('请上传文件') + this.pluginData.plugin_type)
          clearTimeout(timer)
        }, 300)
      }
    ...
    validateExporterPlugin() {
      ...
      if (!hasHostPort) {
        this.bkMessage('error', `Exporter${this.$t('类型插件参数默认值必须包含')}\${host}${this.$t('和')}\${port}${this.$t('变量模板')}`)
      }
    ...
    validateExporterParam() {
      ...
      this.params.forEach((item) => {
        if (item.default.includes('${host}')) {
          hasHost = true
        }
        if (item.default.includes('${port}')) {
          hasPort = true
        }
      })
    ...
    async validateAllOptions(pluginType) {
      ...
      if (pluginType === 'SNMP' && !this.pluginData.collector_json?.filename) {
        this.$bkMessage({ theme: 'error', message: this.$t('请上传采集配置文件') })
        result.push(false)
      }
  ...
  .step-plugin {
    ...
    &-item {
      ...
      .item-container {
        ...
        /deep/ .step-verify-input .tooltips-icon {
          top: 8px;
        }
        .upload-container {
          display: flex;
