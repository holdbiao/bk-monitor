export default class TableStore {
  ...
  public constructor(originData, bizList) {
    ...
    while (i < len) {
      ...
      const doingStatus = ['STARTING', 'STOPPING',
        'DEPLOYING', 'AUTO_DEPLOYING', 'PREPARING'].includes(status) || taskStatus === 'TBC'
  ...
  public getTableData(type = 'All', typeLabel = '') {
    ...
    if (!(type === 'All' && !typeLabel.length)) {
      ret = ret.filter(item => (item.collectType === type || type === 'All')
      && (!typeLabel.length || (typeLabel.length
        && (
    ...
    if (this.keyword.length) {
      ...
      if (keyword.includes(window.i18n.tc('插件ID：'))) {
        ...
        ret = ret.filter(item => item.name.toLocaleLowerCase().includes(keyword)
