...
from core.drf_resource.tools import get_serializer_fields, get_underscore_viewset_name, render_schema
from core.drf_resource.viewsets import ResourceViewSet as OldResourceViewSet
from bkmonitor.utils.common_utils import camel_to_underscore, underscore_to_camel, uniqid
...
{% for module in resource_modules %}
  {{ module.name }}: {
     ...
     * @apiGroup {{ module.name }}
     ...
     {% endfor %}*
    */
    {{ resource.function_name }}: RestListAPI('{{ resource.method }}', '{{ resource.request_url | safe }}'),
    {% endfor %}
  },
{% endfor %}
}
...
def generate_js_api():
    ...
    code = template.render({"resource_modules": resource_modules, "models": models})
    with open(
        os.path.join(settings.PROJECT_ROOT, "webpack", "src", "monitor-pc", "common", "resource.js"),
        "w+",
        encoding="utf-8",
    ) as fp:
        fp.write(code)
...
def _generate_js_api_by_module(views, app_name=""):
    ...
    for attr, val in list(views.__dict__.items()):
        ...
        if isinstance(val, type):
            ...
            elif issubclass(val, GenericViewSet) and val is not GenericViewSet:
